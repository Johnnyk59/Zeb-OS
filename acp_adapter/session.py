"""ACP session manager — maps ACP sessions to Zeb AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.zeb/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Zeb itself is running in WSL.

    Windows ACP clients can launch ``zeb acp`` inside WSL while still sending
    ...
    """
    from zeb_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Zeb from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["zeb-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Zeb agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Zeb AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Zeb runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.zeb/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ZEB_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_zeb_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=zeb_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Zeb daemon's process cwd.
