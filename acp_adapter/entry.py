"""CLI entry point for the zeb-agent ACP adapter.

Loads environment variables from ``~/.zeb/.env``, configures logging
...
try:
    ...
    # shadowing Zeb's own modules — ``zeb acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    zeb_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from ZEB_HOME (default ``~/.zeb``)."""
    from zeb_cli.env_loader import load_zeb_dotenv

    zeb_home = get_zeb_home()
    loaded = load_zeb_dotenv(zeb_home=zeb_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zeb_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="zeb-acp",
        description="Run Zeb Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Zeb version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Zeb provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.zeb/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ZebACPAgent  # noqa: F401

    print("Zeb ACP check OK")


def _run_setup() -> None:
    from zeb_cli.main import main as zeb_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "zeb", "model"]
        zeb_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``zeb postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting zeb-agent ACP adapter")
    ...
    import acp
    from .server import ZebACPAgent
