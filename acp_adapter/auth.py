"""ACP auth helpers — detect and advertise Zeb authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "zeb-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Zeb runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Zeb can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Zeb.
    ...
    not have Zeb provider credentials configured yet, so Zeb always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Zeb using the currently configured "
