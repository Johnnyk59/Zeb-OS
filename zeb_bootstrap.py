"""Windows UTF-8 bootstrap for Zeb entry points.
...
should be imported at the very top of every Zeb entry point
(``zeb``, ``zeb-agent``, ``zeb-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Zeb modules.

    Zeb ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Zeb source root
    ...
    """
    root = src_root or os.environ.get("ZEB_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ZEB_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ZEB_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import zeb_bootstrap``
# (or ``from zeb_bootstrap import apply_windows_utf8_bootstrap``) at
