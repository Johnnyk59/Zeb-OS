"""
Backup and import commands for zeb CLI.

`zeb backup` creates a zip archive of the entire ~/.zeb/ directory
(excluding the zeb-agent repo and transient files).

`zeb import` restores from a backup zip, overlaying onto the current
ZEB_HOME root.
"""
...
from zeb_constants import get_default_zeb_root, get_zeb_home, display_zeb_home
...
# ``zeb-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/zeb-agent/``
...
# ZEB_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "zeb-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ZEB_HOME) —
...
# File names that ``zeb import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ZEB_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ZEB_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zeb root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``zeb-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/zeb-agent/`` — must be preserved.
        if part == "zeb-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ZEB_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Zeb home directory."""
    zeb_root = get_default_zeb_root()

    if not zeb_root.is_dir():
        print(f"Error: Zeb home directory not found at {zeb_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zeb-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zeb-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zeb_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zeb_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zeb_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``zeb-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "zeb-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zeb_root)
    ...
    # outside ZEB_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_zeb_home()}."
        )
    ...
    print(f"\nRestore with: zeb import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Zeb backup.
    ...
    # Look for telltale files that a zeb home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Zeb backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zeb/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zeb dir name
        if prefix in {".zeb", "zeb"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zeb_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zeb_root / "config.yaml").exists()
        has_env = (zeb_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Zeb configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ZEB_HOME.
            ...
            try:
                target.resolve().relative_to(zeb_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_zeb_home()}."
            )
        ...
        profiles_dir = zeb_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zeb_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: zeb profile list  (after installing zeb)")

        # Guidance
        print()
        if not (zeb_root / "zeb-agent").is_dir():
            print("Note: The zeb-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: zeb update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zeb -p {pname} gateway install")

        print("Done. Your Zeb configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zeb backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZEB_HOME).
...
# — `zeb update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ZEB_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ZEB_HOME and the entry is silently
...
def _quick_snapshot_root(zeb_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zeb_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ZEB_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``zeb update``.
    ...
    Args:
        ...
        zeb_home: Override for the Zeb home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zeb_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zeb backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zeb_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zeb_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zeb_root`` to ``out_path``.
...
def _pre_update_backup_dir(zeb_home: Optional[Path] = None) -> Path:
    home = zeb_home or get_zeb_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZEB_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZEB_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``zeb update``) should continue even if the backup fails.
    """
    zeb_root = zeb_home or get_default_zeb_root()
    if not zeb_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zeb_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zeb_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zeb claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZEB_HOME under ``backups/`` before a
    ``zeb claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``zeb import <archive>``.  Writes to
    ``<ZEB_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `zeb import` and the
