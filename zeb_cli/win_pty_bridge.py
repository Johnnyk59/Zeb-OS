"""Windows ConPTY bridge for the `zeb dashboard` chat tab.

Drop-in counterpart to ``zeb_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``zeb_cli.web_server`` consumes: ``spawn``, ``read``,
