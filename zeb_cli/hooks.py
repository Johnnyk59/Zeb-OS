"""zeb hooks — inspect and manage shell-script hooks.

Usage::

    zeb hooks list
    zeb hooks test <event> [--for-tool X] [--payload-file F]
    zeb hooks revoke <command>
    zeb hooks doctor

Consent records live under ``~/.zeb/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.zeb/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zeb hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zeb hooks {list|test|revoke|doctor}")
        print("Run 'zeb hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.zeb/config.yaml.")
        print("See `zeb hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `zeb hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `zeb hooks test` and `zeb hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Zeb wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `zeb hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `zeb hooks doctor` would execute
