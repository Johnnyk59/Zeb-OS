"""Migrate Zeb' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Zeb' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Zeb mcp_servers.<n>.command/args/env  → codex stdio transport
  Zeb mcp_servers.<n>.url/headers       → codex streamable_http transport
  Zeb mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Zeb mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Zeb-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Zeb' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Zeb config.")
...
# Zeb keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Zeb' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, zeb_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Zeb MCP server config to the codex inline-table dict
    ...
    if not isinstance(zeb_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(zeb_cfg.get("command"))
    has_url = bool(zeb_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(zeb_cfg["command"])
        args = zeb_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = zeb_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(zeb_cfg["url"])
        headers = zeb_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Zeb' transport: sse hint is informational; codex auto-negotiates
        if zeb_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in zeb_cfg:
        try:
            out["tool_timeout_sec"] = float(zeb_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in zeb_cfg:
        try:
            out["startup_timeout_sec"] = float(zeb_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if zeb_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in zeb_cfg:
        ...
        elif key not in _KNOWN_ZEB_KEYS:
            skipped.append(f"{key} (unknown Zeb key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ZEB_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Zeb)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Zeb' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Zeb' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Zeb
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="zeb-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ZEB_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed zeb-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.zeb`` symlink that happens to live under ``/private/var/folders``
...
def _build_zeb_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Zeb' own
    ...
    so a zeb installed under /opt/, /usr/local/, or a venv all work.
    ZEB_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ZEB_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_zeb_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ZEB_HOME its launcher sets
    ...
    # — that would override the launcher's ZEB_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ZEB_HOME", tmp_path) would
    ...
    zeb_home = os.environ.get("ZEB_HOME") or ""
    if zeb_home and _looks_like_test_tempdir(zeb_home):
        zeb_home = ""
    if zeb_home:
        env["ZEB_HOME"] = zeb_home
    # PYTHONPATH passes through so a worktree-launched zeb finds the
    ...
    env["ZEB_QUIET"] = "1"
    env["ZEB_REDACT_SECRETS"] = env.get("ZEB_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_zeb_tools: bool = True,
) -> MigrationReport:
    """Translate Zeb mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        zeb_config: full ~/.zeb/config.yaml dict
    ...
    zeb_servers = (zeb_config or {}).get("mcp_servers") or {}
    if not isinstance(zeb_servers, dict):
        report.errors.append(
            "mcp_servers in Zeb config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in zeb_servers.items():
    ...
    # Inject Zeb' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Zeb for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/zeb_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_zeb_tools:
        translated["zeb-tools"] = _build_zeb_tools_mcp_entry()
        if "zeb-tools" not in report.migrated:
            report.migrated.append("zeb-tools")
