"""
Zeb Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``zeb desktop`` / ``zeb gui``)
     Built inside the agent checkout under ``$ZEB_HOME/zeb-agent/``:
       ...
       - ``$ZEB_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Zeb.app`` or ``~/Applications/Zeb.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Zeb`` (NSIS per-user)
...
the app name ("Zeb"), separate from ``$ZEB_HOME``:
  - macOS:   ``~/Library/Application Support/Zeb``
  - Windows: ``%APPDATA%\\Zeb``
  - Linux:   ``$XDG_CONFIG_HOME/Zeb`` (default ``~/.config/Zeb``)
...
``zeb uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Zeb"
...
def source_built_gui_artifacts(zeb_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``zeb desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``zeb-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(zeb_home)
    ...
    return [
        ...
        zeb_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Zeb" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Zeb.app"),
            home / "Applications" / "Zeb.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Zeb).
            local_base / "Programs" / "Zeb",
            # Older / alternate layout some builds used.
            local_base / "zeb-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Zeb")
    else:
        ...
        paths += [
            data_base / "applications" / "zeb.desktop",
            data_base / "applications" / "Zeb.desktop",
        ]
    return paths


def agent_is_installed(zeb_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ZEB_HOME.
...
def gui_is_installed(zeb_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(zeb_home):
        if p.exists():
            return True
...
def gui_install_summary(zeb_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = zeb_home if zeb_home is not None else get_zeb_home()
...
def uninstall_gui(zeb_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``zeb-agent/zeb_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ZEB_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove zeb' or "
            "'sudo dnf remove zeb'). AppImage builds are a single file you "
