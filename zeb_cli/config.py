"""
Configuration management for Zeb Agent.

Config files are stored in ~/.zeb/ for easy access:
- ~/.zeb/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zeb/.env         - API keys and secrets

This module provides:
- zeb config          - Show current configuration
- zeb config edit     - Open config in editor
- zeb config set      - Set a specific value
- zeb config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``zeb config set`` (which rewrites
    ...
    ``config.yaml`` in place: zeb never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.zeb/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``zeb logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  zeb config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Zeb makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Zeb itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``zeb update``, the TUI build.
...
#   on every plugin install / ``zeb update``.
...
# * ``ZEB_HOME`` / ``ZEB_PROFILE`` / ``ZEB_CONFIG`` /
#   ``ZEB_ENV`` — Zeb runtime location flags. Writing these into
...
#
# IMPORTANT: ``ZEB_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ZEB_LANGFUSE_PUBLIC_KEY,
# ZEB_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Zeb runtime location — never via dashboard env writer.
    # NOT a ZEB_* blanket: integration credentials (ZEB_GEMINI_*,
    # ZEB_LANGFUSE_*, ZEB_SPOTIFY_*, ...) ARE allowed.
    "ZEB_HOME", "ZEB_PROFILE", "ZEB_CONFIG", "ZEB_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Zeb runtime location "
            "(ZEB_HOME, ZEB_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ZEB_TOOL_PROGRESS", "ZEB_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `zeb plugins enable
    # observability/langfuse` or `zeb tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ZEB_LANGFUSE_ENV",
    "ZEB_LANGFUSE_RELEASE",
    "ZEB_LANGFUSE_SAMPLE_RATE",
    "ZEB_LANGFUSE_MAX_CHARS",
    "ZEB_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from zeb_cli.colors import Colors, color
from zeb_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZEB_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zeb_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zeb is running in package-manager-managed mode.

    Two signals: the ZEB_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZEB_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``zeb_cli/`` — i.e. the git checkout for source
    installs, ``/opt/zeb`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ZEB_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Zeb was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ZEB_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ZEB_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.zeb``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ZEB_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.zeb:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ZEB_HOME``, a
    ...
    ``docker`` and ``zeb update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/zeb-agent`` image bakes a ``docker``
        stamp into ``/opt/zeb`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ZEB_HOME.
    try:
    ...
    #    host install whose shared $ZEB_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``zeb update``.
    try:
        method = (
            (get_zeb_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zeb_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ZEB_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/zeb`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Zeb lives in a ``uv tool`` layout.

    ``uv tool install zeb-agent`` places the install at
    ``.../uv/tools/zeb-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``zeb-agent`` happens to be uv-tool-installed on the machine while
    the *active* Zeb is a regular pip/venv install, causing
    ``zeb update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/zeb-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade zeb-agent"
    if method == "docker":
        return "docker pull nousresearch/zeb-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade zeb-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade zeb-agent"
        return "pip install --upgrade zeb-agent"
    return "zeb update"
...
# Long-form text for ``zeb update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# zeb_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Zeb, it doesn't update
...
✗ ``zeb update`` doesn't apply inside the Docker container.

Zeb Agent runs as a published image (nousresearch/zeb-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/zeb-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate zeb-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/zeb-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/zeb-agent/tags
  • Your config and session history live under ``$ZEB_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``zeb update`` inside Docker.
...
def format_managed_message(action: str = "modify this Zeb installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZEB_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zeb installation is managed by NixOS "
            f"(ZEB_MANAGED={env_hint}).\n"
            "Edit services.zeb-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zeb installation is managed by Homebrew "
            f"(ZEB_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zeb-agent"
        )

    return (
        f"Cannot {action}: this Zeb installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zeb."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZEB_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zeb_bin
    or None if container mode is not active, we're already inside the
    container, or ZEB_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZEB_DEV") == "1":
        return None
    ...
    container_mode_file = get_zeb_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zeb-agent")
    exec_user = info.get("exec_user", "zeb")
    zeb_bin = info.get("zeb_bin", "/data/current-package/bin/zeb")
...
# Re-export from zeb_constants — canonical definition lives there.
from zeb_constants import get_zeb_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zeb_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zeb_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_zeb_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ZEB_UID / ZEB_GID env vars set by Docker deployments.

    Docker containers running Zeb commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ZEB_HOME once, but
    subdirectories created at runtime by ``ensure_zeb_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ZEB_UID", "").strip()
    gid_str = os.environ.get("ZEB_GID", "").strip()
    try:
...
def _chown_to_zeb_uid(path) -> None:
    """Chown ``path`` to ``ZEB_UID:ZEB_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_zeb_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_zeb_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zeb group can
    share state with the gateway service.

    The mode can be overridden via the ZEB_HOME_MODE environment variable
    (e.g. ZEB_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZEB_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ZEB_UID``/``ZEB_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZEB_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_zeb_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Zeb runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZEB_CONTAINER") or os.environ.get("ZEB_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZEB_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZEB_HOME, upgrading legacy empty templates.
...
def ensure_zeb_home():
    """Ensure ~/.zeb directory structure exists with secure permissions.
    ...
    """
    home = get_zeb_home()
    # Named profiles must be created explicitly (e.g. ``zeb profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ZEB_HOME would resurrect an empty skeleton
...
def _ensure_zeb_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZEB_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Zeb-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Zeb
        ...
        # identity slot (SOUL.md). Empty by default. The ZEB_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Zeb adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ZEB_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ZEB_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Zeb auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Zeb sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Zeb
        ...
        # bundled Zeb image, which drops to the `zeb` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Zeb sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Zeb should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``zeb chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zeb/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: zeb sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .zeb.md, or .cursorrules before Zeb applies
    ...
    # single read_file page exceeds these limits, Zeb truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Zeb' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); zeb =
                                      # Zeb' compression threshold triggers
    ...
    # See tools/kanban_tools.py and zeb_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``zeb dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``zeb kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``zeb kanban decompose`` and the kanban
        ...
        # ``zeb profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `zeb model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `zeb` (and `zeb chat`) launches by default:
        ...
        # REPL and `--tui` (or ZEB_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `zeb --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zeb -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZEB_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `zeb --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to ZEB_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/zeb`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `zeb pets`. Disabled until
        ...
        "pet": {
            ...
            # get_zeb_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ZEB_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ZEB_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ZEB_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ZEB_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ZEB_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/zeb``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Zeb appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zeb/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zeb/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.zeb/cache/delegation/
    ...
    # response. If not, Zeb feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Zeb auto-pauses the goal and
    ...
    "moa": {
        ...
        # <zeb_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.zeb/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZEB_SKILL_DIR} and ${ZEB_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `zeb curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `zeb curator run --consolidate` overrides
        ...
        # normally restored on every `zeb update`, so pruning them only
        ...
        # skipped), snapshot ~/.zeb/skills/ into
        # ~/.zeb/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `zeb curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for zeb-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Zeb Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ZEB_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Zeb' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.zeb/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZEB_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `zeb doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``zeb_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Zeb to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ZEB_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ZEB_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `zeb -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `zeb` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `zeb kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     zeb-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Zeb tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.zeb/logs/.
    ...
    # update model picker lists without shipping a zeb-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://zeb-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `zeb model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal ZEB_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.zeb/sessions/sessions.json entirely.
        ...
        # the ZEB_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.zeb.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.zeb/.env,
        ...
        # behavior: files must live under the Zeb cache, under
        ...
        # user. Bridged to ZEB_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Zeb
        # cache (~/.zeb/cache/{documents,images,audio,video,screenshots})
        ...
        # to ZEB_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ZEB_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ZEB_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.zeb/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.zeb/sessions/session_{sid}.json`` on every turn
    ...
    # ``zeb update`` behaviour.
    "updates": {
        # Run a full ``zeb backup``-style zip of ZEB_HOME before every
        # ``zeb update``.  Backups land in ``<ZEB_HOME>/backups/`` and
        # can be restored with ``zeb import <path>``.  Off by default:
        # zipping a large ZEB_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``zeb update
        ...
        "backup_keep": 5,
        # What `zeb update` does with uncommitted local changes to the
    ...
    # `zeb tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.zeb/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.zeb/.env (or your shell) and never in config.yaml.
            ...
            # ~/.zeb/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.zeb/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Zeb disables it for our users unless they
        # explicitly opt in here. When false (default), Zeb sets
    ...
    # Zeb Desktop (Electron app) launch options. These only affect
    # `zeb desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ZEB_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Zeb mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zeb model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Zeb auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Zeb, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zeb:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZEB_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #zeb)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: zeb-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zeb-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Zeb API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Zeb API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Zeb API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZEB_TOOL_PROGRESS and ZEB_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zeb.config`` entries, then checks
    ...
    try:
        ...
        # should never break `zeb update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Zeb' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Zeb versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zeb_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zeb setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Zeb won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zeb doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ZEB_HOME", "~/.zeb")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "zeb update / zeb -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZEB_TOOL_PROGRESS")
            old_mode = get_env_value("ZEB_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZEB_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZEB_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZEB_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZEB_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZEB_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_zeb_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `zeb plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `zeb curator status` has no
    ...
    #   3. Creates `~/.zeb/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_zeb_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_zeb_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `zeb config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `zeb config set`"
                    )
    ...
    # behavior set it to true explicitly via `zeb config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from zeb_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from zeb_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: zeb config set <key> <value>")
    ...
    # `zeb update`, but only the version bump is persisted.
    ...
    # metadata.zeb.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_zeb_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``zeb config set``
    ...
    backends) — while display paths (``zeb status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _zeb_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zeb/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zeb/config.yaml.
    ...
    (which change ``ZEB_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/zeb/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_zeb_dotenv()
...
#   openai-codex (OAuth — zeb auth) — OpenAI Codex
#   nous         (OAuth — zeb auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.zeb/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Zeb writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zeb/.env.
    ...
    interactive menu render (`zeb tools`, `zeb setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `zeb tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zeb env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zeb/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zeb/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zeb/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zeb/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zeb — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zeb vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zeb/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.zeb/.env`` over ``os.environ``.

    Used for Zeb-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Zeb Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from zeb_cli.auth import get_anthropic_key
    ...
    # Warn on stale ZEB_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ZEB_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ZEB_MAX_ITERATIONS={_env_ghost} "
                f"(run 'zeb doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  zeb config edit     # Edit config file", Colors.DIM))
    print(color("  zeb config set <key> <value>", Colors.DIM))
    print(color("  zeb setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `zeb config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `zeb config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zeb config set <key> <value>")
            print()
            print("Examples:")
            print("  zeb config set model anthropic/claude-sonnet-4")
            print("  zeb config set terminal.backend docker")
            print("  zeb config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zeb config migrate' to add them")
        ...
        print("  zeb config           Show current configuration")
        print("  zeb config edit      Open config in editor")
        print("  zeb config set <key> <value>   Set a config value")
        print("  zeb config check     Check for missing/outdated config")
        print("  zeb config migrate   Update config with new options")
        print("  zeb config path      Show config file path")
        print("  zeb config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``zeb config`` UI so users
