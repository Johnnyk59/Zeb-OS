...
Usage:
    zeb pairing list              # Show all pending + approved users
    zeb pairing approve <platform> <code>  # Approve a pairing code
    zeb pairing revoke <platform> <user_id> # Revoke user access
    zeb pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle zeb pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: zeb pairing {list|approve|revoke|clear-pending}")
        print("Run 'zeb pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.zeb/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'zeb pairing list' to see pending codes.\n")
