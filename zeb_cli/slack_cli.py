"""``zeb slack ...`` CLI subcommands.

Today only ``zeb slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ zeb slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Zeb. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Zeb deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from zeb_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Zeb in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Zeb agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``zeb_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ZEB_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Zeb")
    ...
    """
    name = getattr(args, "name", None) or "Zeb"
    description = getattr(args, "description", None) or "Your Zeb agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ZEB_HOME") or str(Path.home() / ".zeb")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Zeb app\n"
