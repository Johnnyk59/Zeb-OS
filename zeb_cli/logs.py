"""``zeb logs`` — view and filter Zeb log files.
...
Usage examples::
    ...
    zeb logs                    # last 50 lines of agent.log
    zeb logs -f                 # follow agent.log in real time
    zeb logs errors             # last 50 lines of errors.log
    zeb logs gateway -n 100    # last 100 lines of gateway.log
    zeb logs gui -f            # follow gui.log (dashboard/pty/ws)
    zeb logs desktop -f        # follow desktop.log (Electron app boot/backend)
    zeb logs --level WARNING    # only WARNING+ lines
    zeb logs --session abc123   # filter by session ID substring
    zeb logs --component tools  # only tool-related lines
    zeb logs --since 1h         # lines from the last hour
    zeb logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zeb_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Zeb runs — try 'zeb chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zeb_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zeb_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zeb_home()}/logs/")
        return

    print(f"Log files in {display_zeb_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zeb chat' to generate logs)")
