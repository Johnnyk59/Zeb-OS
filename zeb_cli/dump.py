"""
Dump command for zeb CLI.

Outputs a compact, plain-text summary of the user's Zeb setup
...
from zeb_cli.config import get_zeb_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.zeb/.env.
    ...
    exports. ``zeb debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.zeb_build_sha`` by the Dockerfile's
    ``ZEB_GIT_SHA`` build-arg (see ``zeb_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``zeb dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zeb_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zeb_home: Path) -> int:
...
def _cron_summary(zeb_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zeb_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zeb_home:      {display_zeb_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.zeb/.env: a managed
        ...
        # A credential added via `zeb auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `zeb auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["zeb-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zeb_home)}")
    lines.append(f"  skills:             {_count_skills(zeb_home)}")
