"""``zeb project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$ZEB_HOME/projects.db`` store
(see :mod:`zeb_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Zeb Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``zeb project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: zeb project <action> [options]\n"
                "Run 'zeb project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `zeb project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `zeb project add-folder`.",
            file=sys.stderr,
        )
        return 1
