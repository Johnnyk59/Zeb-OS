...
# xAI's public API is OpenAI-compatible for the endpoints Zeb commonly
# uses. The Responses endpoint is included because Zeb' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Zeb-managed OAuth credentials."""

    auth_hint = "zeb auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`zeb auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`zeb auth reset xai-oauth` or re-authenticate with "
                    "`zeb auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `zeb auth add xai-oauth --type oauth`."
            )
