"""MCP picker — interactive `zeb mcp picker` (also the default `zeb mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``zeb mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `zeb plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from zeb_cli.cli_output import prompt_yes_no
from zeb_cli.curses_ui import curses_single_select
from zeb_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Zeb session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from zeb_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `zeb mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: zeb mcp install <name>    Picker: zeb mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Zeb is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Zeb — run `zeb update` "
...
def show_catalog() -> None:
    """`zeb mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`zeb mcp picker` (and default `zeb mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`zeb mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `zeb mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
