...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zeb -c.",
    ...
    "Ctrl+Z suspends Zeb to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zeb -c resumes your most recent CLI session. zeb -c \"project name\" resumes by title.",
    "zeb -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zeb -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zeb chat -t web,terminal enables only specific toolsets for a focused session.",
    "zeb chat -s github-pr-workflow preloads a skill at launch.",
    "zeb chat -q \"query\" runs a single non-interactive query and exits.",
    "zeb chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zeb chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zeb --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zeb chat --source telegram tags the session for filtering in zeb sessions list.",
    "zeb -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zeb doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zeb dump outputs a compact setup summary — great for bug reports.",
    "zeb config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zeb config edit opens config.yaml in your default editor.",
    "zeb config check scans for missing or stale configuration options.",
    "zeb sessions browse opens an interactive session picker with search.",
    "zeb sessions stats shows session counts by platform and database size.",
    "zeb sessions prune --older-than 30 cleans up old sessions.",
    "zeb skills search react --source skills-sh searches the skills.sh public directory.",
    "zeb skills check scans installed hub skills for upstream updates.",
    "zeb skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zeb skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zeb mcp add github --command npx adds MCP servers from the command line.",
    "zeb mcp serve runs Zeb itself as an MCP server for other agents.",
    "zeb auth add lets you add multiple API keys for credential pool rotation.",
    "zeb completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zeb logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zeb backup creates a zip backup of your entire Zeb home directory.",
    "zeb profile create coder creates an isolated profile that becomes its own command.",
    "zeb profile create work --clone copies your current config and keys to a new profile.",
    "zeb update syncs new bundled skills to ALL profiles automatically.",
    "zeb gateway install sets up Zeb as a system service (systemd/launchd).",
    "zeb memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zeb webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: zeb tools disables unused tools, zeb skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "zeb models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zeb -w).",
    ...
    "ZEB_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zeb tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zeb profile create coder' creates the 'coder' command.",
    "zeb profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zeb prints a resume command with session ID and stats.",
    "zeb sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zeb -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zeb skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zeb/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zeb cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zeb runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "zeb gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zeb via a local macOS server.",
    ...
    "Context files (.zeb.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Zeb loads project context from .zeb.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "zeb mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "zeb mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "zeb mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — zeb tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zeb/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zeb chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zeb plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zeb acp runs Zeb as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZEB_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zeb auth add nous or zeb auth add openai-codex sets up OAuth-based providers.",
    ...
    "zeb status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.zeb/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zeb your own.",
    ...
    "Before a gateway session reset, Zeb auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zeb/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Zeb state via ZEB_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ZEB_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZEB_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.zeb/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zeb profile create ls' would be rejected.",
    "zeb profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zeb claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZEB_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zeb tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.zeb/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.zeb/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Zeb auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Zeb config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Zeb is working.',
    ...
    '/reload reloads ~/.zeb/.env into the running session — pick up new API keys without restarting.',
    ...
    "ZEB_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.zeb/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.zeb/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'zeb curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "zeb curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'zeb curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'zeb auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ZEB_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ZEB_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'zeb dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'zeb dashboard embeds the full Zeb TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.zeb/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.zeb/dashboard-plugins/ — no npm build required.',
    ...
    'ZEB_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ZEB_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ZEB_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ZEB_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.zeb/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.zeb/skills/ so drop-in skills appear without restarting the session.',
    ...
    'zeb -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'zeb chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'zeb chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'zeb chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "zeb chat --source tool tags programmatic chats so they don't clutter zeb sessions list.",
    'zeb dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'zeb sessions rename <ID> "new title" renames any past session; zeb sessions delete <ID> removes one.',
    'zeb import restores a session export or profile archive produced by sessions export or profile export.',
    'zeb fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'zeb pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'zeb setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'zeb status --deep runs the full health sweep across every component; plain zeb status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ZEB_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ZEB_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.zeb/plugins/ — trust-gated by design.',
    "ZEB_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ZEB_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ZEB_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ZEB_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ZEB_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ZEB_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ZEB_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ZEB_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ZEB_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ZEB_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ZEB_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Zeb block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from zeb sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.zeb/dashboard-plugins/.',
]
