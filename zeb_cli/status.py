"""
Status command for zeb CLI.

Shows the status of all Zeb Agent components.
"""
...
from zeb_cli.auth import AuthError, resolve_provider
from zeb_cli.colors import Colors, color
from zeb_cli.config import get_env_path, get_env_value, get_zeb_home, load_config
from zeb_cli.models import provider_label
from zeb_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from zeb_cli.nous_subscription import get_nous_subscription_features
from zeb_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``zeb config``'s
...
def show_status(args):
    """Show status of all Zeb Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Zeb Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: zeb portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: zeb model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: zeb auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: zeb auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zeb model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from zeb_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from zeb_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_zeb_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_zeb_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zeb doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zeb setup' to configure", Colors.DIM))
    print()
