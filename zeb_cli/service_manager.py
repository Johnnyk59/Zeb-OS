...
the existing module-level functions in zeb_cli.gateway and
zeb_cli.gateway_windows directly. This protocol is a thin facade
...
``zeb gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``zeb gateway start/stop/restart``).
    """
    ...
    # `zeb gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged zeb
    ...
    detection always fails. Since every Zeb runtime call inside the
    container drops to zeb via ``s6-setuidgid``, that silent failure
...
# in ``zeb_cli.gateway`` (systemd/launchd) and ``zeb_cli.gateway_windows``
...
# active (set via the ``zeb -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in zeb_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from zeb_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from zeb_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from zeb_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from zeb_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in zeb_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from zeb_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from zeb_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from zeb_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from zeb_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``zeb_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `zeb profile create`
# runs inside the container (Phase 4). Static services (main-zeb, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ZEB_HOME blindly;
    derive the shared profile root from the current ZEB_HOME and map the
    ...
    zeb_home = Path(os.environ.get("ZEB_HOME", "/opt/data"))
    if zeb_home.parent.name == "profiles":
        root = zeb_home.parent.parent
    else:
        root = zeb_home
...
# UID/GID of the in-image ``zeb`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the zeb user.
    ...
    (the ``zeb`` user — UID 10000 — running every Zeb runtime
    ...
    with zeb ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           zeb:zeb, 0755 (parent must already exist)
    ``svc_dir/event/``                     zeb:zeb, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 zeb:zeb, 0755
    ``svc_dir/supervise/event/``           zeb:zeb, 03730
    ``svc_dir/supervise/control``          zeb:zeb, 0660    (FIFO)
    ...
    slot's supervise/ was zeb-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ZEB_UID, _ZEB_GID)
        except PermissionError:
            # Running as the zeb user already — directory is zeb-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ZEB_UID, _ZEB_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is zeb-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ZEB_UID, _ZEB_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``zeb -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`zeb profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-zeb, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ZEB_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ZEB_HOME=/data/zeb`` is honored at run
          ...
          4. Drops to the zeb user and exec's
             ``zeb -p <profile> gateway run`` (or just ``zeb
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``zeb gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ZEB_HOME profile" (the implicit profile that exists at
        the top of $ZEB_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``zeb_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ZEB_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ZEB_HOME``,
        ...
        allocator in ``zeb_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export ZEB_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's ZEB_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``zeb gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ZEB_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The ZEB_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own ZEB_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "zeb gateway run --replace"
        else:
            gateway_cmd = f"zeb -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid zeb {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ZEB_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ZEB_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ZEB_HOME=/data/zeb``
        gets its logs under /data/zeb/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``zeb logs`` reads and what
        ...
        return (
            ...
            f': "${{ZEB_HOME:=/opt/data}}"\n'
            f'log_dir="$ZEB_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as zeb and crash-loops on
            ...
            f'chown zeb:zeb "$ZEB_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R zeb:zeb "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid zeb s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``zeb gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``zeb -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with zeb ownership
            ...
            # the zeb user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is zeb-owned (see ``_seed_supervise_skeleton``), and on
