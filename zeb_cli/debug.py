"""``zeb debug`` debug tools for Zeb Agent.

Currently supports:
    zeb debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.zeb/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[zeb debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.zeb/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``zeb debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``zeb debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `zeb debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "zeb-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``zeb debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.zeb/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``zeb debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zeb-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zeb_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_zeb_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``zeb dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``zeb dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "zeb-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``zeb debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "zeb debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `zeb debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  zeb debug delete <url>")

    print("\nShare these links with the Zeb team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Zeb version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``zeb debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from zeb_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "zeb debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `zeb debug share --local` to print the report instead, "
            "or `zeb debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: zeb debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'zeb debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``zeb debug`` call.
    ...
    # best-effort — any failure is swallowed so ``zeb debug`` stays
