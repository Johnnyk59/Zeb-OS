...
the tool registry or launching Zeb (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``zeb-cli`` to the non-existent ``zeb``. ``resolve_toolset('zeb')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``zeb-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``zeb`` instead of ``zeb-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `zeb tools` to reconfigure."
        )
    return warnings
