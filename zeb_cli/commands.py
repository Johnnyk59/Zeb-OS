"""Slash command definitions and autocomplete for the Zeb CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Zeb config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Zeb works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Zeb is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zeb/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Zeb Agent to the latest version", "Info"),
    CommandDef("version", "Show Zeb Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zeb_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zeb`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Zeb ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Zeb
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``zeb skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``zeb skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``zeb skills list`` and
...
# unrelated command landed. These claim their slots right after /zeb,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /zeb <command>).
...
# native slot, the alias spelling stays reachable via /zeb reset).
...
# ``/zeb <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/zeb" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /zeb credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /zeb billing.
#   - moa: high-cost slash mode, available through /zeb moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /zeb debug on Slack.
_SLACK_VIA_ZEB_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zeb <verb>`` subcommand.
    ...
    can still reach them via ``/zeb <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/zeb`` is always reserved as the first entry so the
    legacy ``/zeb <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zeb as the catch-all top-level command.
    entries.append(("zeb", "Talk to Zeb or run a subcommand", "[subcommand] [args]"))
    seen.add("zeb")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ZEB_ONLY:
            # Intentionally Slack-via-/zeb only (see _SLACK_VIA_ZEB_ONLY).
            return
    ...
    # everything except /zeb, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://zeb-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zeb handler.

    Maps both canonical names and aliases so /zeb bg do stuff works
    the same as /zeb background do stuff.

    Plugin-registered slash commands are included so ``/zeb <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zeb_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
