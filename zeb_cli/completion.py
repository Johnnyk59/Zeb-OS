"""Shell completion script generation for zeb CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zeb_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Zeb Agent bash completion
# Add to ~/.bashrc:
#   eval "$(zeb completion bash)"

_zeb_profiles() {{
    local profiles_dir="$HOME/.zeb/profiles"
...
_zeb_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zeb_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zeb_completion zeb
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef zeb
# Zeb Agent zsh completion
# Add to ~/.zshrc:
...
_zeb_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zeb/profiles" ]]; then
        profiles+=($HOME/.zeb/profiles/*(N/:t))
    fi
...
_zeb() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zeb_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Zeb Agent fish completion",
        "# Add to your config:",
        "#   zeb completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zeb_profiles",
        "    echo default",
        "    if test -d $HOME/.zeb/profiles",
        "        for d in $HOME/.zeb/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zeb -f -s p -l profile"
        " -d 'Profile name' -xa '(__zeb_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__zeb_profiles)' -d 'Profile name'"
                )
