#!/usr/bin/env python3
"""
Zeb CLI - Main entry point.

Usage:
    zeb                     # Interactive chat (default)
    zeb chat                # Interactive chat
    zeb gateway             # Run gateway in foreground
    zeb gateway start       # Start gateway as service
    zeb gateway stop        # Stop gateway service
    zeb gateway status      # Show gateway status
    zeb gateway install     # Install gateway service
    zeb gateway uninstall   # Uninstall gateway service
    zeb setup               # Interactive setup wizard
    zeb logout              # Clear stored authentication
    zeb status              # Show status of all components
    zeb cron                # Manage cron jobs
    zeb cron list           # List cron jobs
    zeb cron status         # Check if cron scheduler is running
    zeb doctor              # Check configuration and dependencies
    zeb honcho setup                    # Configure Honcho AI memory integration
    zeb honcho status                   # Show Honcho config and connection status
    zeb honcho sessions                 # List directory → session name mappings
    zeb honcho map <name>               # Map current directory to a session name
    zeb honcho peer                     # Show peer names and dialectic settings
    zeb honcho peer --user NAME         # Set user peer name
    zeb honcho peer --ai NAME           # Set AI peer name
    zeb honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zeb honcho mode                     # Show current memory mode
    zeb honcho mode [hybrid|honcho|local]  # Set memory mode
    zeb honcho tokens                   # Show token budget settings
    zeb honcho tokens --context N       # Set session.context() token cap
    zeb honcho tokens --dialectic N     # Set dialectic result char cap
    zeb honcho identity                 # Show AI peer identity representation
    zeb honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zeb honcho migrate                  # Step-by-step migration guide: OpenClaw native → Zeb + Honcho
    zeb version             Show version
    zeb update              Update to latest version
    zeb uninstall           Uninstall Zeb Agent
    zeb acp                 Run as an ACP server for editor integration
    zeb sessions browse     Interactive session picker with search

    zeb claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: zeb_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zeb_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``zeb update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``zeb_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, zeb crashes on import and the user can't run
# ``zeb update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import zeb_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'zeb' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``zeb tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``zeb.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("zeb")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"zeb", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"zeb")
        # Windows: the .exe name is already ``zeb.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* zeb_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ZEB_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".zeb", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ZEB_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ZEB_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ZEB_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ZEB_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`zeb --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from zeb_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Zeb Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``zeb --version`` before config/logging imports on Termux."""
    if os.environ.get("ZEB_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from zeb_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from zeb_cli.subcommands.cron import build_cron_parser
from zeb_cli.subcommands.gateway import build_gateway_parser
from zeb_cli.subcommands.profile import build_profile_parser
from zeb_cli.subcommands.model import build_model_parser
from zeb_cli.subcommands.setup import build_setup_parser
from zeb_cli.subcommands.postinstall import build_postinstall_parser
from zeb_cli.subcommands.whatsapp import build_whatsapp_parser
from zeb_cli.subcommands.slack import build_slack_parser
from zeb_cli.subcommands.login import build_login_parser
from zeb_cli.subcommands.logout import build_logout_parser
from zeb_cli.subcommands.auth import build_auth_parser
from zeb_cli.subcommands.status import build_status_parser
from zeb_cli.subcommands.webhook import build_webhook_parser
from zeb_cli.subcommands.hooks import build_hooks_parser
from zeb_cli.subcommands.doctor import build_doctor_parser
from zeb_cli.subcommands.security import build_security_parser
from zeb_cli.subcommands.dump import build_dump_parser
from zeb_cli.subcommands.debug import build_debug_parser
from zeb_cli.subcommands.backup import build_backup_parser
from zeb_cli.subcommands.import_cmd import build_import_cmd_parser
from zeb_cli.subcommands.config import build_config_parser
from zeb_cli.subcommands.console import build_console_parser
from zeb_cli.subcommands.version import build_version_parser
...
from zeb_cli.subcommands.memory import build_memory_parser
from zeb_cli.subcommands.acp import build_acp_parser
from zeb_cli.subcommands.tools import build_tools_parser
from zeb_cli.subcommands.insights import build_insights_parser
from zeb_cli.subcommands.skills import build_skills_parser
from zeb_cli.subcommands.pairing import build_pairing_parser
from zeb_cli.subcommands.plugins import build_plugins_parser
from zeb_cli.subcommands.mcp import build_mcp_parser
from zeb_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (zeb tools, zeb setup, zeb model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zeb {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zeb module import.
#
# Many modules cache ZEB_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZEB_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zeb/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZEB_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `zeb mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Zeb' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo zeb -p <name>` against the invoking user's home.
        ...
        candidate = home / ".zeb" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`zeb chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors zeb_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ZEB_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zeb/profiles/coder or
    # /opt/data/profiles/coder).  If ZEB_HOME points to the zeb root
    # instead (e.g. systemd hardcodes ZEB_HOME=/root/.zeb), we must
    # still read active_profile — the user may have switched profiles via
    # `zeb profile use` and the gateway should honour that choice.
    # See issue #22502.
    zeb_home_env = os.environ.get("ZEB_HOME", "")
    if profile_name is None and zeb_home_env:
        if Path(zeb_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the zeb root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ZEB_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``zeb gateway run`` to mean
    # "the root ZEB_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ZEB_S6_SUPERVISED_CHILD"):
        try:
            from zeb_constants import get_default_zeb_root

            active_path = get_default_zeb_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZEB_HOME
    if profile_name is not None:
        try:
            from zeb_cli.profiles import resolve_profile_env

            zeb_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            zeb_home = _resolve_sudo_user_profile_env(profile_name)
            if not zeb_home:
            ...
            # A bug in profiles.py must NEVER prevent zeb from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ZEB_HOME"] = zeb_home
...
# Load .env from ~/.zeb/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zeb_cli.config import get_zeb_home
from zeb_cli.env_loader import load_zeb_dotenv

load_zeb_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZEB_REDACT_SECRETS env
# var BEFORE zeb_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_zeb_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from zeb_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ZEB_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ZEB_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `zeb` subcommands
...
try:
    from zeb_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from zeb_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if zeb_constants not importable yet
...
from zeb_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to zeb_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (zeb_cli.main._model_flow_*) keep resolving unchanged.
from zeb_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_zeb_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ZEB_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ZEB_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from zeb_cli.config import get_env_path, get_zeb_home, load_config
    from zeb_cli.auth import get_auth_status

    # Determine whether Zeb itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zeb_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zeb_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_zeb_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Zeb has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Zeb to use their tokens.
    if _has_zeb_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zeb_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zeb_bin
        cli_args: the original CLI arguments (everything after 'zeb')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zeb_bin = container_info["zeb_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zeb {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zeb {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  zeb --tui --resume {target}")
    if title:
        print(f'  zeb --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zeb/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@zeb" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/zeb-ink/package.json",
    "packages/zeb-ink/index.js",
    "packages/zeb-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ZEB_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ZEB_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ZEB_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZEB_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zeb_home = os.environ.get("ZEB_HOME") or str(Path.home() / ".zeb")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZEB_HOME": zeb_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(zeb_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(zeb_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if zeb_cli_dir is None:
        zeb_cli_dir = Path(__file__).parent
    bundled = zeb_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``zeb update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ZEB_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Zeb checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `zeb update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Zeb checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `zeb --tui`\n"
        "If the checkout is still inconsistent, run `zeb update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZEB_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZEB_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from zeb_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ZEB_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZEB_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZEB_TUI_DIR (e.g. `unset ZEB_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @zeb/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @zeb/ink resolves through
        # packages/zeb-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "zeb-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zeb_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from zeb_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zeb-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZEB_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ZEB_PYTHON_SRC_ROOT"] = os.environ.get(
        "ZEB_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ZEB_PYTHON", sys.executable)
    env.setdefault("ZEB_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ZEB_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ZEB_MODEL"] = model
        env["ZEB_INFERENCE_MODEL"] = model
    if provider:
        env["ZEB_TUI_PROVIDER"] = provider
        env["ZEB_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZEB_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZEB_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZEB_TUI_SKILLS"] = value
    if query:
        env["ZEB_TUI_QUERY"] = query
    if image:
        env["ZEB_TUI_IMAGE"] = image
    if checkpoints:
        env["ZEB_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZEB_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZEB_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZEB_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZEB_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZEB_ACCEPT_HOOKS"] = "1"
    ...
    # ZEB_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `zeb --tui` try to
    ...
    # still set ZEB_TUI_RESUME themselves.
    env.pop("ZEB_TUI_RESUME", None)
    if resume_session_id:
        env["ZEB_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `zeb update` so
    ...
    if code == 42:
        from zeb_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZEB_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``zeb kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``zeb kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ZEB_KANBAN_BOARD"):
        return
    try:
        from zeb_cli.kanban_db import get_current_board

        os.environ["ZEB_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zeb/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Zeb — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Zeb still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ZEB_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ZEB_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zeb sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from zeb_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from zeb_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'zeb doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Zeb isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zeb setup")
        print()

        from zeb_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'zeb setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zeb_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZEB_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zeb/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZEB_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZEB_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZEB_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zeb_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from zeb_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zeb_cli.config import get_env_value, save_env_value
    from zeb_constants import find_node_executable, with_zeb_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Zeb?")
        print()
    ...
    # has no creds.json.  Every subsequent `zeb gateway` then paid a 30s
    ...
    session_dir = get_zeb_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: zeb gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `zeb gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zeb gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Zeb Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zeb gateway")
        ...
        print()
        print("  Or install as a service: zeb gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zeb whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``zeb whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``zeb_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from zeb_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from zeb_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from zeb_cli.config import stamp_install_method
    from zeb_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Zeb post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from zeb_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zeb model``) and the setup wizard
    ...
    from zeb_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from zeb_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZEB_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zeb_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zeb model' for "
                "available providers, or run 'zeb doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in zeb_cli/models.py). A multi-member group shows one
    ...
    elif selected_provider == "local-model":
        _model_flow_local_model(config, current_model)
    ...
    # a leftover OPENAI_BASE_URL in ~/.zeb/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zeb/.env if the active provider is not 'custom'.
    ...
    """
    from zeb_cli.config import get_env_value, save_env_value, load_config
...
#
# Zeb uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zeb model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zeb model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`zeb_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from zeb_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Zeb only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``zeb model`` flow, then route aux tasks to them here.
    """
    from zeb_cli.config import load_config
    from zeb_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zeb_cli.auth import _prompt_model_selection
    from zeb_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from zeb_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zeb_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from zeb_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Zeb URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from zeb_cli.curses_ui import curses_radiolist
...
# downstream call sites read `zeb_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `zeb` invocation — including
# fast paths like `zeb --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zeb_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``zeb setup`` / ``zeb model``.
    ...
    recover from a malformed paste without editing ``~/.zeb/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from zeb_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from zeb_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `zeb setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zeb_constants import display_zeb_home as _dhh_fn

            print(
                f"    Zeb will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Zeb CLI with a provider."""
    from zeb_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zeb_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zeb_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zeb_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zeb_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zeb_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zeb slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: zeb slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zeb slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zeb_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zeb_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from zeb_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from zeb_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zeb_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `zeb security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from zeb_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from zeb_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zeb_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zeb_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Zeb home directory to a zip file."""
    if getattr(args, "quick", False):
        from zeb_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zeb_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Zeb backup from a zip file."""
    from zeb_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from zeb_cli.banner import format_banner_version_label
    ...
    try:
        from zeb_cli.banner import check_for_updates
        from zeb_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Zeb Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from zeb_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zeb_cli.uninstall import run_gui_uninstall
    ...
    from zeb_cli.uninstall import run_uninstall
...
# Critical files that Zeb must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``zeb`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="zeb-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zeb update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``zeb_cli/web_dist/`` (per vite.config.ts
    outDir: "../zeb_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "zeb_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``zeb`` launcher present but ``zeb_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the zeb venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — zeb venv python3, no nix-shell overhead
    ...
    # a zeb venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``zeb update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zeb web``).
    ...
    # build path usable on a stock `py -m zeb_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (zeb web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``zeb update`` can unconditionally call ``zeb desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``zeb desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ZEB_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ZEB_HOME."""
    from zeb_constants import get_zeb_home
    return get_zeb_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``zeb update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Zeb.app/Contents/MacOS/Zeb"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Zeb.exe",
            release_dir / "win-ia32-unpacked" / "Zeb.exe",
            release_dir / "win-arm64-unpacked" / "Zeb.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "zeb",
            release_dir / "linux-unpacked" / "Zeb",
            release_dir / "linux-arm64-unpacked" / "zeb",
            release_dir / "linux-arm64-unpacked" / "Zeb",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Zeb'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Zeb.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Zeb.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Zeb.exe: Access is
    ...
    an unrelated "Zeb" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Zeb is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Zeb.app/Contents/MacOS/Zeb  ->  app bundle = .../Zeb.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``zeb desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Zeb Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Zeb Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ZEB_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from zeb_logging import setup_logging as _setup_logging_gui
    ...
    # with_zeb_node_path() copies os.environ when called with no arg.
    env = with_zeb_node_path()
    if getattr(args, "fake_boot", False):
        env["ZEB_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ZEB_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "zeb_root", None):
        env["ZEB_DESKTOP_ZEB_ROOT"] = str(Path(args.zeb_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ZEB_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ZEB_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ZEB_DESKTOP_DISABLE_GPU=... zeb desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ZEB_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ZEB_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  zeb gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Zeb.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Zeb.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Zeb.exe, close any")
                    print("  running Zeb desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> zeb desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Zeb is
    ...
    if source_mode:
        print("→ Launching Zeb Desktop from source build...")
    ...
    print(f"→ Launching packaged Zeb Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``zeb dashboard`` processes other than ourselves.

    ``zeb dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zeb update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Zeb Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``zeb serve`` as
    ...
    environment variable ``ZEB_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "zeb dashboard",
        "zeb_cli.main dashboard",
        "zeb_cli/main.py dashboard",
        # The headless backend (`zeb serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "zeb serve",
        "zeb_cli.main serve",
        "zeb_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from zeb_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "zeb.*dashboard"`) keeps us consistent
            # with `zeb_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `zeb update`.
    ...
    print("  Preview now:  zeb curator run --dry-run")
    print("  Pause it:     zeb curator pause")
    print(
        "  Docs:         https://zeb-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``zeb update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``zeb update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: zeb curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``zeb dashboard`` processes.

    Called at the end of ``zeb update`` (default ``reason``) and also
    from ``zeb dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Zeb Desktop Electron app spawns this dashboard as a
    # backend child, it sets ZEB_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ZEB_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.zeb-update-staging"
    backup = f"{dst}.zeb-update-old"
...
def _update_via_zip(args):
    """Update Zeb Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `zeb update "
            f"--branch {branch}`, or update against main with `zeb update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/zeb-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="zeb-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"zeb-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for zeb-agent itself
        ...
        # GitHub ZIPs extract to zeb-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"zeb-agent-{branch}")
    ...
    from zeb_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: zeb skills list-modified  "
    ...
    try:
        from zeb_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "zeb-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Zeb behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zeb completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Zeb couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Zeb couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Zeb behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Zeb couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Zeb couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `zeb update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/zeb-agent.git",
    "git@github.com:NousResearch/zeb-agent.git",
    "https://github.com/NousResearch/zeb-agent",
    "git@github.com:NousResearch/zeb-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/zeb-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zeb_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zeb_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Zeb repository.")
        print("  This means you may miss updates from NousResearch/zeb-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/zeb-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/zeb-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``zeb update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zeb_constants import get_default_zeb_root

    default_home = get_default_zeb_root()
...
# Install-scoped breadcrumb dropped right before ``zeb update`` mutates the
...
# survives and the next ``zeb`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ZEB_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``zeb update`` left half-done.
    ...
    protocol stream (``zeb acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if zeb.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _zeb_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        print(
            "⚠ A previous `zeb update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from zeb_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``zeb update`` is still progressing even if pip/uv itself is silent.
    """
...
def _zeb_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_zeb_instances(
    ...
    The Zeb Desktop Electron app spawns ``zeb.EXE`` as a backend child,
    so during ``zeb update`` the user-invoked process and the desktop's
    ...
    shims (``zeb.exe`` / ``zeb-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``zeb update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``zeb.exe``) is a separate process from the Python
    ...
    for shim in _zeb_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated zeb.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``zeb update``
    ...
    #      zeb.exe sitting *under* a non-Zeb parent (e.g. a Zeb
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "zeb.exe"
    lines = ["✗ Another zeb.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Zeb Desktop, exit any open `zeb` REPLs, and")
    lines.append("  stop the gateway (`zeb gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `zeb update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_zeb_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``zeb.exe``.
    ...
    when ``zeb update`` runs, ``zeb.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``zeb.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next zeb invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Zeb Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Zeb Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _zeb_exe_shims(scripts_dir):
        ...
        print(
            "    Close Zeb Desktop, exit other `zeb` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `zeb update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_zeb_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``zeb.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``zeb.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``zeb`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_zeb_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``zeb.exe.old.*`` left by prior updates.

    Called early on every zeb invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``zeb update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `zeb update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``zeb.exe`` / ``zeb-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_zeb_exe`` for the rationale.
    """
    ...
    # hours later inside ``zeb update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``zeb.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``zeb.exe`` shim is locked during ``zeb update``, or when uv/distlib
    skips a launcher write. The symptom is ``zeb-agent.exe`` and
    ``zeb-acp.exe`` present but ``zeb.exe`` missing, so ``zeb`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `zeb update --force` after "
            "closing other zeb processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m zeb_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``zeb update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``zeb.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``zeb`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `zeb update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `zeb update --force` after closing other zeb processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zeb_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ZEB_HOME/bin/uv``, but on Termux the official
    ...
    """
    from zeb_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `zeb update`, so we install root-only first
    ...
    nixos_env = with_zeb_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `zeb update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``zeb update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.zeb/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``zeb update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zeb update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zeb/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``zeb update --gateway``) the update is already
    ...
    try:
        ...
        # zeb_cli.config.get_zeb_home to simulate setup failure.
        from zeb_cli.config import get_zeb_home as _get_zeb_home

        logs_dir = _get_zeb_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.zeb/logs/update.log`` only, never the terminal.

    During ``zeb update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``zeb update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from zeb_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``zeb update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from zeb_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from zeb_cli.config import recommended_update_command
        from zeb_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zeb update`` without requiring a reinstall.  The
    ...
    ``zeb`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zeb.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zeb'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/zeb, code at /usr/local/lib/zeb-agent).
    fhs_link = Path("/usr/local/bin/zeb")
    ...
    path_comment = (
        "# Zeb Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ZEB_HOME before running the update.
    ...
    ZEB_HOME directories.  The ``--backup`` flag on ``zeb update``
    ...
    # ``zeb_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # ZEB_HOME can add minutes to every update. Users who want the
    ...
    try:
        from zeb_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_zeb_home so the user sees ~/.zeb/...
    try:
        from zeb_constants import get_zeb_home, display_zeb_home

        home = get_zeb_home()
        try:
            display_path = f"{display_zeb_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zeb import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``zeb update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``zeb update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run zeb from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.zeb-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".zeb-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The zeb.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m zeb_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``zeb
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Zeb processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Zeb Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Zeb desktop app / other Zeb terminals, then re-run:"
    )
    lines.append("    zeb update")
    lines.append("  (or use `zeb update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    zeb.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from zeb_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Zeb...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m zeb_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: zeb gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``zeb gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from zeb_cli import gateway_windows
        from zeb_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``zeb update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Zeb Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Zeb Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `zeb update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Zeb via pip (for PyPI installs)."""
    from zeb_cli import __version__
    from zeb_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "zeb-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "zeb-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "zeb-agent"]
    ...
    print("✓ Update complete! Restart zeb to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Zeb Agent...")
    print()

    # On Windows, abort early if another zeb.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_zeb_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `zeb serve` backend) will keep .pyd
    ...
    # the zeb.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://zeb-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Zeb windows/gateways and re-run: zeb update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $ZEB_HOME, this is cheap
        ...
        try:
            from zeb_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in zeb_cli/config.py bricked
        # every user who ran ``zeb update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``zeb
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``zeb update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_zeb_home added to zeb_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``zeb`` launch finishes the
        ...
        # build.  ``zeb desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``zeb desktop`` shouldn't be forced into a full
        # Electron build by ``zeb update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from zeb_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "zeb_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `zeb desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from zeb_constants import display_zeb_home as _dhh
        ...
        # current — so copy it straight over ~/.zeb/cache/model_catalog.json
        ...
        # modules in this process.  Reload zeb_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zeb_home() added since the last release.
        try:
            import importlib
            import zeb_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ZEB_HOME so
        # it is not affected by sync_skills()'s module-level ZEB_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZEB_HOME env var points at the default or a named profile.
        try:
            from zeb_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from zeb_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'zeb config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: zeb config migrate")
            else:
                print()
                print("Skipped. Run 'zeb config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from zeb_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `zeb curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``zeb update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from zeb_cli.tools_config import install_cua_driver
        ...
        # When running as ``zeb update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_zeb_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... zeb-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... zeb-gateway*)
            ...
            # Discover all zeb-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. zeb-gateway.service or zeb-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `zeb update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: zeb gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: zeb gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: zeb -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Zeb gateway unit files are still installed.
        # When both zeb.service (from a pre-rename install) and the
        # current zeb-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zeb update` surfaces the issue until the user migrates.
        try:
            from zeb_cli.gateway import (
                has_legacy_zeb_units,
                _find_legacy_zeb_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zeb_units():
                print()
                print("⚠ Legacy Zeb gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zeb_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zeb.service) fight the current")
                print("  zeb-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zeb gateway migrate-legacy")
                print()
        ...
        print("  zeb model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zeb -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from zeb_constants import display_zeb_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `zeb profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zeb_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → zeb -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  zeb profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zeb -p {name} chat")
                else:
        ...
        from zeb_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zeb_constants import get_zeb_home as _hh
        ...
        from zeb_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `zeb profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → zeb -p {name}  ({wrapper})")
        print()
        ...
        from zeb_cli.profiles import profile_exists, validate_alias_name
        ...
        from zeb_cli.profiles import rename_profile
        ...
        from zeb_cli.profiles import export_profile
        ...
        from zeb_cli.profiles import import_profile
        ...
        import tempfile
        from zeb_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="zeb_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  zeb -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      zeb -p {plan.manifest.name} chat")
        ...
        from zeb_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `zeb profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"zeb -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from zeb_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("zeb_requires"):
            print(f"Requires:     Zeb {data['zeb_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from zeb_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zeb_requires:
        print(f"  Requires: Zeb {mf.zeb_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``zeb dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``zeb dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No zeb dashboard processes running.")
        return 0

    print(f"{len(pids)} zeb dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``zeb dashboard register`` for OAuth.
    ...
    try:
        from zeb_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `zeb dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    zeb dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "ZEB_DASHBOARD_OAUTH_CLIENT_ID into ~/.zeb/.env for you.\n"
            "  Docs: https://zeb-agent.nousresearch.com/docs/"
    ...
    try:
        from zeb_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No zeb dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `zeb update`.
    ...
    # launches the dashboard (`worker dashboard` → ZEB_HOME points into
    ...
    try:
        from zeb_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ZEB_DESKTOP") != "1"
    ):
        ...
        # ZEB_HOME.  We must resolve the root explicitly instead of just
        # dropping ZEB_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ZEB_HOME=/opt/data`), so an unset
        # ZEB_HOME falls back to $HOME/.zeb = /opt/data/.zeb — an
        ...
        # update-button guard also misfires).  get_default_zeb_root()
        # returns the root for both layouts: ~/.zeb for a standard install
        ...
        try:
            from zeb_constants import get_default_zeb_root
            env["ZEB_HOME"] = str(get_default_zeb_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ZEB_HOME) rather than block the reroute.
            env.pop("ZEB_HOME", None)
    ...
    # the same logs directory as every other Zeb surface.
    try:
        from zeb_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ZEB_SERVE_HEADLESS"] = "1"
    elif "ZEB_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZEB_WEB_DIST"])
            if "ZEB_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zeb_cli" / "web_dist"
        )
    ...
    try:
        from zeb_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard_register(args):
    ...
    from zeb_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from zeb_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zeb_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zeb_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Zeb log files."""
    from zeb_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Zeb command console."""
    from zeb_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zeb_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``zeb -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``zeb_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``zeb -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``zeb`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ZEB_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from zeb_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from zeb_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ZEB_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ZEB_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ZEB_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `zeb --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_zeb_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_zeb_home()}/memories/\n")
    else:
        from zeb_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Zeb Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zeb_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zeb_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zeb_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zeb_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zeb_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zeb_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zeb_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from zeb_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from zeb_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for zeb CLI."""
    # Cosmetic: make the process show up as 'zeb' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from zeb_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``zeb.exe.old.*`` quarantine files left by previous
    # ``zeb update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_zeb_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``zeb update``
    ...
    # ``zeb skills install update``) merely defers recovery one launch;
    # under-matching (missing ``zeb -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in zeb_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from zeb_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from zeb_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://zeb-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zeb model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.zeb/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://zeb-agent.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from zeb_cli import secrets_cli as _secrets_cli
    from zeb_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from zeb_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in zeb_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in zeb_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in zeb_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in zeb_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `zeb whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in zeb_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from zeb_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in zeb_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in zeb_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in zeb_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in zeb_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in zeb_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in zeb_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from zeb_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from zeb_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from zeb_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in zeb_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in zeb_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in zeb_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in zeb_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in zeb_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in zeb_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zeb/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo zeb uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from zeb_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in zeb_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in zeb_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in zeb_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in zeb_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in zeb_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from zeb_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in zeb_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``zeb --help``, ``zeb version``, ``zeb logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zeb_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zeb_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Zeb shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from zeb_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from zeb_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in zeb_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in zeb_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `zeb computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `zeb tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `zeb computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Zeb app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from zeb_cli.tools_config import _cua_driver_cmd
            # Honor ZEB_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from zeb_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: zeb computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: zeb computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: zeb computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: zeb computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: zeb computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: zeb computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in zeb_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <zeb home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from zeb_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from zeb_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from zeb_cli.session_export_html import (
            ...
            # Markdown / QMD export
            from zeb_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_zeb_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_zeb_home() / "sessions"
            ...
            # `zeb sessions prune`: no time window and no filters at all
            ...
            # Launch zeb --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in zeb_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in zeb_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in zeb_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in zeb_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in zeb_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in zeb_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in zeb_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in zeb_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Zeb-Setup.exe success screen tells users to
    # run `zeb desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in zeb_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in zeb_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in zeb_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``zeb -c Pokemon Agent Dev`` → ``zeb -c 'Pokemon Agent Dev'``
    ...
    from zeb_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zeb -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (zeb hooks list, cron
