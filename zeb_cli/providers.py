"""
Single source of truth for provider identity in Zeb Agent.
...
2. **Zeb overlays** — transport type, auth patterns, aggregator flags,
...
# -- Zeb overlay ----------------------------------------------------------
# Zeb-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZebOverlay:
    """Zeb-specific provider metadata layered on top of models.dev."""
...
ZEB_OVERLAYS: Dict[str, ZebOverlay] = {
    "moa": ZebOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ZebOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": ZebOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ZebOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ZebOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ZebOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZEB_QWEN_BASE_URL",
    ),
    "lmstudio": ZebOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ZebOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZebOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZebOverlay(
    ...
    "alibaba-coding-plan": ZebOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zeb", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Zeb overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Zeb overlay
    ...
    overlay = ZEB_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zeb extra
    ...
    if overlay is not None:
        # Zeb-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ZEB_OVERLAYS
    if provider == "bedrock":
