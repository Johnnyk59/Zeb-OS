...
on Japanese locales, etc.).  Zeb's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``zeb_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Zeb is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ZEB_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ZEB_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Zeb-managed Git install directories so
    ...
    # already-running shells don't see the change — which means zeb
    ...
    # install without Zeb-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Zeb's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Zeb's
    ``zeb config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Zeb (User env var in Windows Settings, or
    export in a PowerShell profile) and Zeb picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Zeb-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\zeb\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``zeb`` from right after install) retain their old PATH.

    Any subprocess Zeb spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Zeb-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "zeb", "git", "cmd"),
        os.path.join(local_appdata, "zeb", "git", "bin"),
        os.path.join(local_appdata, "zeb", "git", "usr", "bin"),
        # Zeb venv Scripts directory — host of the zeb.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes zeb, but harmless to include.
        os.path.join(local_appdata, "zeb", "zeb-agent", "venv", "Scripts"),
