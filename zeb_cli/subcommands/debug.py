...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Zeb Agent. Use 'zeb debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zeb debug share              Upload debug report (asks for confirmation)
    zeb debug share --yes        Skip confirmation (for scripts/CI)
    zeb debug share --lines 500  Include more log lines
    zeb debug share --expire 30  Keep paste for 30 days
    zeb debug share --local      Print report locally (no upload)
    zeb debug share --no-redact  Disable upload-time secret redaction
    zeb debug share --nous       Upload to Nous-internal storage (private)
    zeb debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'zeb debug share'",
    )
