"""`zeb checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.zeb/checkpoints/``.  Actions:

    zeb checkpoints               # same as `status`
    zeb checkpoints status        # total size, project count, breakdown
    zeb checkpoints list          # per-project checkpoint counts + workdir
    zeb checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    zeb checkpoints clear [-f]    # nuke the entire base (asks first)
    zeb checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    zeb checkpoints
    zeb checkpoints prune --retention-days 3 --max-size-mb 200
    zeb checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zeb checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zeb checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zeb checkpoints` → status
