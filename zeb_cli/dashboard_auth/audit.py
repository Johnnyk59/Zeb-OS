...
Profile-aware location: ``$ZEB_HOME/logs/dashboard-auth.log``.
...
from ``zeb_constants`` or other zeb_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ZEB_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``zeb_constants.get_zeb_home`` semantics: env var wins,
    else ``~/.zeb``. A local copy avoids an import cycle with the
    middleware which lives below ``zeb_cli``.
    """
    home = os.environ.get("ZEB_HOME") or str(Path.home() / ".zeb")
