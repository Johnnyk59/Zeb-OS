...
Three cookies in play:
  ...
  - zeb_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-zeb_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "zeb_session_at"
SESSION_RT_COOKIE = "zeb_session_rt"
PKCE_COOKIE = "zeb_session_pkce"
...
SSO_ATTEMPT_COOKIE = "zeb_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /zeb`` we want ``Path=/zeb`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (zeb #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/zeb``)
