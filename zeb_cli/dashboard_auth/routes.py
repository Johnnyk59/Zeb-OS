...
from zeb_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ZEB_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /zeb`` (Mission Control deploys) — we
    ...
    from zeb_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``zeb_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    if getattr(p, "supports_password", False):
        raise HTTPException(
            status_code=400,
            detail=f"Provider {provider!r} is password-only; use /auth/password-login instead.",
        )
    ...
    pkce = ls.cookie_payload.get("zeb_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from zeb_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
