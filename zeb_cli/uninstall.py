"""
Zeb Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.zeb/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Zeb PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zeb-agent or zeb PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Zeb Agent" comment and following line
                if '# Zeb Agent' in line or '# zeb-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('zeb' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zeb
                if 'zeb' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zeb wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "zeb",
        Path("/usr/local/bin/zeb"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zeb_cli reference)
                content = wrapper.read_text()
                if 'zeb_cli' in content or 'zeb-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(zeb_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``zeb`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Zeb home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (zeb_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zeb gateway run`` processes
    ...
    try:
        from zeb_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zeb_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZEB_HOME`` and ``ZEB_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\zeb\git\cmd``, ``%LOCALAPPDATA%\zeb\git\bin``,
#      ``%LOCALAPPDATA%\zeb\git\usr\bin``, ``%LOCALAPPDATA%\zeb\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\zeb\git\`` and Node to
#      ``%LOCALAPPDATA%\zeb\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``zeb dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\zeb\gateway-service\`` and sometimes
...
def _zeb_path_markers(zeb_home: Path) -> list[str]:
    """Path-entry substrings that identify Zeb-owned User-PATH entries."""
    root = str(zeb_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare zeb-agent install dir.
    markers = [root + "\\zeb-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZEB_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "zeb".  We don't want to catch
    # unrelated entries like "czeb-foo" or "ephermeral", so we look for
    # backslash-zeb as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(zeb_home: Path) -> list[str]:
    """Strip Zeb-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _zeb_path_markers(zeb_home)
            kept: list[str] = []
...
def remove_zeb_env_vars_windows() -> list[str]:
    """Delete ZEB_HOME and ZEB_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZEB_HOME", "ZEB_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(zeb_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\zeb\\``.  Only called on full uninstall; they're
...
def _is_default_zeb_home(zeb_home: Path) -> bool:
    """Return True when ``zeb_home`` points at the default (non-profile) root."""
    try:
        from zeb_constants import get_default_zeb_root
        return zeb_home.resolve() == get_default_zeb_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZEB_HOME directory.

    We shell out to ``zeb -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZEB_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zeb_cli.main` so we don't depend on a `zeb`
    #    wrapper that may be half-removed mid-uninstall.
    zeb_invocation = [_sys.executable, "-m", "zeb_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zeb_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZEB_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``zeb uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ZEB_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from zeb_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(zeb_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Zeb Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Zeb Chat GUI installation was found.")
        print(f"  Checked: {zeb_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Zeb agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(zeb_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Zeb agent at {zeb_home / 'zeb-agent'}")
        print(f"  • Your config, sessions, and secrets under {zeb_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Zeb agent is still installed. Run 'zeb' to use the CLI,")
    print("or 'zeb uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zeb/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zeb/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ZEB_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zeb_home(zeb_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.zeb data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Zeb Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zeb_home / 'config.yaml'}")
    print(f"  Secrets: {zeb_home / '.env'}")
    print(f"  Data:    {zeb_home / 'cron/'}, {zeb_home / 'sessions/'}, {zeb_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ZEB_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Zeb data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Zeb code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, zeb_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Zeb PATH entries from shell configs / Windows User PATH")
    print("  • Zeb wrapper scripts and Zeb-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Zeb config/data: {zeb_home}")
        if _is_default_zeb_home(zeb_home):
        ...
        print(f"  • Keep Zeb config/data: {zeb_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``zeb`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ZEB_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in zeb_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\zeb\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(zeb_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Zeb-owned PATH entries in User environment")

        log_info("Removing ZEB_HOME / ZEB_GIT_BASH_PATH User env vars...")
        removed_env = remove_zeb_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Zeb-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing zeb command...")
    ...
    #     (only when they still point into this Zeb home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Zeb-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(zeb_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Zeb-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(zeb_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ZEB_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zeb_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(zeb_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zeb/, just remove the zeb-agent subdir
            if zeb_home in project_root.parents or project_root.parent == zeb_home:
    ...
    #     under ZEB_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(zeb_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(zeb_home)
    ...
    # 5. Optionally remove ~/.zeb/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZEB_HOME dirs live under
        ...
        try:
            if zeb_home.exists():
                shutil.rmtree(zeb_home)
                log_success(f"Removed {zeb_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zeb_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zeb_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://zeb-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://zeb-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Zeb Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m zeb_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import zeb_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zeb_constants`` + ``zeb_cli.colors``
    (and lazily ``zeb_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m zeb_cli.uninstall")
    parser.add_argument(
        "--mode",
