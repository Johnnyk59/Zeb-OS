"""Profile distributions — shareable, packaged Zeb profiles via git.

A distribution is a Zeb profile published as a git repository (or
...
* ``zeb profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``zeb skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``zeb profile``, not a parallel tree):

    zeb profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "zeb-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            zeb_requires=str(data.get("zeb_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zeb_requires:
            out["zeb_requires"] = self.zeb_requires
        if self.author:
...
def check_zeb_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Zeb {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``zeb profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Zeb profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Zeb distribution."
        )

    # Version check up-front so we fail fast
    check_zeb_requires(manifest.zeb_requires, zeb_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.zeb).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="zeb_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `zeb profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`zeb profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="zeb_dist_update_") as tmp:
        plan = plan_install(
