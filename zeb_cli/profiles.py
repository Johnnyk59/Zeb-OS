"""
Profile management for multiple isolated Zeb instances.

Each profile is a fully independent ZEB_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zeb/profiles/<name>/`` by default.

The "default" profile is ``~/.zeb`` itself — backward compatible,
zero migration needed.

Usage::

    zeb profile create coder          # fresh profile + bundled skills
    zeb profile create coder --clone  # also copy config, .env, SOUL.md, skills
    zeb profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zeb -p coder chat                 # or via flag
    zeb profile use coder             # set as sticky default
    zeb profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zeb_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `zeb profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `zeb
...
# `zeb skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Zeb infrastructure directories that only the default profile
         (``~/.zeb``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_zeb_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.zeb) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zeb-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ZEB_HOME equals the
...
# into the export. The set below identifies the *known Zeb profile
# artifacts* at the root of ZEB_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``zeb_constants``.
...
_RESERVED_NAMES = frozenset({
    "zeb", "default", "test", "tmp", "root", "sudo",
})

# Zeb subcommands that cannot be used as profile names/aliases
_ZEB_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zeb root, NOT to the current ZEB_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZEB_HOME points outside
    ``~/.zeb``, profiles live under ``ZEB_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zeb_home() / "profiles"


def _get_default_zeb_home() -> Path:
    """Return the default (pre-profile) ZEB_HOME path.

    In standard deployments this is ``~/.zeb``.
    In Docker/custom deployments where ZEB_HOME is outside ``~/.zeb``
    (e.g. ``/opt/data``), returns ZEB_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_zeb_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Zeb installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZEB_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_zeb_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, zeb subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ZEB_SUBCOMMANDS:
        return f"'{canon}' conflicts with a zeb subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            zeb_exe = shutil.which("zeb") or "zeb"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(zeb_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Zeb. If we
    ...
    try:
        ...
        from zeb_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `zeb doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``zeb -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``zeb profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``zeb -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``zeb profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ZEB_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Zeb config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zeb_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, zeb_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ZEB_HOME.
    ...
    The returned ``zeb_home`` is the path to pass to
    ``set_zeb_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_zeb_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``zeb update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zeb)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zeb/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `zeb -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Zeb profile.\n"
    ...
    if not soul_path.exists():
        try:
            from zeb_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `zeb update`'s
    ...
    # Cloned configs can be older than the running Zeb (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `zeb profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `zeb -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZEB_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``zeb profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZEB_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_zeb_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Zeb *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Zeb backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ZEB_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `zeb -p <canon> profile
    ...
    zeb_markers = ("zeb_cli.main", "zeb-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Zeb process: either an entrypoint marker in argv, or
            # a resolved executable named `zeb`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_zeb = (
                any(marker in joined for marker in zeb_markers)
                or exe_name == "zeb"
                or exe_name.startswith("zeb")
            )
            if not is_zeb:
                continue
            ...
            # ...or by ZEB_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ZEB_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zeb).\n"
            "To remove everything, use: zeb uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_zeb_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ZEB_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from zeb_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZEB_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZEB_HOME")
    try:
        os.environ["ZEB_HOME"] = str(profile_dir)
        from zeb_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZEB_HOME"] = old_home
        elif "ZEB_HOME" in os.environ:
            del os.environ["ZEB_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zeb/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: zeb profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZEB_HOME.

    Returns ``"default"`` if ZEB_HOME is not set or points to ``~/.zeb``.
    Returns the profile name if ZEB_HOME points into ``~/.zeb/profiles/<name>``.
    Returns ``"custom"`` if ZEB_HOME is set to an unrecognized path.
    """
    from zeb_constants import get_zeb_home
    zeb_home = get_zeb_home()
    resolved = zeb_home.resolve()

    default_resolved = _get_default_zeb_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ZEB_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Zeb file the
      user may have lying alongside ZEB_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zeb itself — its parent is ~/ and its
        # directory name is ".zeb", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zeb profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.zeb itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zeb). "
            "Specify a different name: zeb profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zeb_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zeb_{old_name}"
    legacy_old_host = f"zeb.{old_name}"
    new_host = f"zeb_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zeb_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("zeb_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZEB_HOME path string.

    Called early in the CLI entry point, before any zeb modules
    are imported, to set the ZEB_HOME environment variable.
    """
