...
Zeb owns its own uv binary at ``$ZEB_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ZEB_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Zeb keeps *its* uv binary.

    ``$ZEB_HOME/bin/uv`` on POSIX, ``$ZEB_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``zeb update`` runs
    the call site from the *old*, already-imported ``zeb_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``zeb update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$ZEB_HOME/bin/`` instead of ``~/.local/bin/``.
    """
