...
Registry and ABC for every inference provider Zeb knows about.
...
`$ZEB_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `zeb_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `zeb_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `zeb_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `zeb_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `zeb_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$ZEB_HOME/plugins/model-providers/` for a private plugin).
