...
as ``kind: backend``) or ``~/.zeb/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``zeb tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ZEB_HOME/cache/videos/``, creating parents as needed."""
    from zeb_constants import get_zeb_home

    path = get_zeb_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ZEB_HOME/cache/videos/``.
