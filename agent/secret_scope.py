...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ZEB_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Zeb runtime / deployment
    "ZEB_HOME", "ZEB_PROFILE", "ZEB_GATEWAY_LOCK_DIR",
    "ZEB_MAX_ITERATIONS", "ZEB_MAX_TOKENS", "ZEB_API_TIMEOUT",
    "ZEB_REDACT_SECRETS", "ZEB_NOUS_TIMEOUT_SECONDS",
    "_ZEB_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ZEB_KANBAN_DB", "ZEB_KANBAN_WORKSPACES_ROOT", "ZEB_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ZEB_KANBAN_",
    "ZEB_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(zeb_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(zeb_home) / ".env")
