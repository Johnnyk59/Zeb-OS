"""Coding-context awareness — base Zeb, every interactive surface.

When the user runs Zeb inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Zeb shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("zeb.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "zeb", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _ZEB_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``ZEB_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Zeb should operate in its coding posture right now."""
