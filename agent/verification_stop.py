...
def _session_is_messaging_surface() -> bool:
    ...
    ``ZEB_SESSION_PLATFORM``; the CLI and TUI set ``ZEB_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``ZEB_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("ZEB_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``ZEB_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("ZEB_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `zeb-verify-` filename prefix, run it "
