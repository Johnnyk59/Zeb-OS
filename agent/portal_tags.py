...
Every Zeb request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Zeb Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=zeb-agent",
        "client=zeb-client-v<__version__>",
    ]

The version is sourced live from ``zeb_cli.__version__`` so it auto-aligns
...
``zeb_cli.__version__`` is the canonical source of truth.
"""
...
def _zeb_version() -> str:
    """Return the current Zeb release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``zeb_cli`` cannot be imported (should
...
def zeb_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=zeb-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=zeb-client-v{_zeb_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=zeb-agent", zeb_client_tag()]
