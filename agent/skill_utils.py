...
from zeb_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ZEB_KANBAN_TASK`` / ``ZEB_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ZEB_KANBAN_TASK") or os.getenv("ZEB_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Zeb Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``zeb_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZEB_PLATFORM`` or
            ``ZEB_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``zeb`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zeb/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``zeb`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ZEB_HOME, not cwd
        if not p.is_absolute():
            p = (zeb_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zeb/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.zeb/skills/`` (including via symlinks) or configured
    ...
    # ~/.zeb/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Zeb can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zeb = metadata.get("zeb") or {}
    if not isinstance(zeb, dict):
        zeb = {}
    return {
        "fallback_for_toolsets": zeb.get("fallback_for_toolsets", []),
        "requires_toolsets": zeb.get("requires_toolsets", []),
        "fallback_for_tools": zeb.get("fallback_for_tools", []),
        "requires_tools": zeb.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Zeb metadata, VCS, virtualenv/dependency, cache, and skill
