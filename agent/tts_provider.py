...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Zeb with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.zeb/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Zeb
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zeb tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``zeb setup`` for
