...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ZEB_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Zeb expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zeb_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zeb_home()`` themselves.
        """
        if "zeb_home" not in kwargs:
            from zeb_constants import get_zeb_home
            kwargs["zeb_home"] = str(get_zeb_home())
        for provider in self._providers:
            try:
