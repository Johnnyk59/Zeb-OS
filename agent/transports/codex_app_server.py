...
projection into Zeb' display, approval bridging, transcript projection into
...
"codex_app_server"`. Zeb' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Zeb secret — gateway
        ...
        spawn_env = zeb_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ZEB_KANBAN_TASK"):
            kanban_db = spawn_env.get("ZEB_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ZEB_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ZEB_HOME", os.path.expanduser("~/.zeb")),
                        "kanban",
                    ),
                )
            )
