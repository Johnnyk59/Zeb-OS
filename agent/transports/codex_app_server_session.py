...
Owns one Codex thread per Zeb session. Drives `turn/start`, consumes
...
# Zeb' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ZEB_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Zeb/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Zeb session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ZEB_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ZEB_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="zeb",
            client_title="Zeb Agent",
            client_version=_get_zeb_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Zeb' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Zeb' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own zeb-tools callback we
            # auto-accept — the user already approved Zeb' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Zeb approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Zeb returns 'once', 'session', 'always', or 'deny'.
...
def _get_zeb_version() -> str:
    """Best-effort Zeb version string for codex's userAgent line."""
    try:
