...
upstream prefix cache warm.  See ``zeb-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Zeb never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ZEB_HOME persona while keeping
    ...
    # Pointer to the zeb-agent skill + docs for user questions about Zeb itself.
    stable_parts.append(ZEB_AGENT_HELP_GUIDANCE)
    ...
    # ZEB_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Zeb, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Zeb profile the agent is running
    # under so it doesn't conflate ~/.zeb/skills/ (default profile) with
    # ~/.zeb/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Zeb profile: default. Other profiles (if any) live "
            "under ~/.zeb/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Zeb profile: {active_profile}. This session reads "
            f"and writes ~/.zeb/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.zeb/skills/, ~/.zeb/plugins/, "
            f"~/.zeb/cron/, ~/.zeb/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Zeb never rebuilds or reinjects parts of it
