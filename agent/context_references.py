...
_SENSITIVE_ZEB_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    zeb_home = get_zeb_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(zeb_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(zeb_home / rel for rel in _SENSITIVE_ZEB_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Zeb path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.zeb/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Zeb path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
