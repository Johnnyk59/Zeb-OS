...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Zeb never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Zeb continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<zeb_home>/cache/op_cache.json``
so back-to-back short-lived ``zeb`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a ZEB_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Zeb to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_zeb_dotenv()`` after the .env files have loaded.
