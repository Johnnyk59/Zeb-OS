...
cache problem must never block Zeb startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Zeb home used for cache paths.

    ``home_path`` is whatever ``load_zeb_dotenv()`` already resolved;
    falling back to ``$ZEB_HOME`` / ``~/.zeb`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("ZEB_HOME", Path.home() / ".zeb"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<zeb_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ZEB_HOME is ever made traversable.
            try:
