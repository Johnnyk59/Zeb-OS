...
Storage
-------
Bundles live in ``~/.zeb/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ZEB_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``zeb bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ZEB_HOME.

    Honors ``ZEB_BUNDLES_DIR`` for tests; falls back to
    ``<ZEB_HOME>/skill-bundles``.
    """
    override = os.environ.get("ZEB_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_zeb_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `zeb bundles` CLI subcommand.
# ---------------------------------------------------------------------------
