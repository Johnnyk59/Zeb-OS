...
Plugins built on Zeb Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Zeb
...
:class:`~zeb_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Zeb' main config
...
handles every provider, fallback chain, and per-task override Zeb
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zeb_cli.plugins.PluginContext`
