...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Zeb starts and finds ``~/.openclaw/``.

    Points users at ``zeb claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zeb claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Zeb, run "
        "`zeb claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zeb claw cleanup` (renames it to ~/.openclaw.pre-migration — "
