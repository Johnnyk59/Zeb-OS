...
def _zeb_home_path() -> Path:
    """Resolve the active ZEB_HOME (profile-aware) without circular imports."""
    try:
        from zeb_constants import get_zeb_home  # local import to avoid cycles
        return get_zeb_home()
    except Exception:
        return Path(os.path.expanduser("~/.zeb"))


def _zeb_root_path() -> Path:
    """Resolve the Zeb root dir (always the parent of any profile, never per-profile)."""
    try:
        from zeb_constants import get_default_zeb_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    zeb_home = _zeb_home_path()
    zeb_root = _zeb_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(zeb_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(zeb_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved ZEB_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("ZEB_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    zeb_dirs = []
    for base in (_zeb_home_path(), _zeb_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in zeb_dirs:
                zeb_dirs.append(real)
        except Exception:
            continue

    for base_real in zeb_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Zeb path.

    Three categories are blocked:

      * Internal Zeb cache files under ``ZEB_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ZEB_HOME and the global Zeb
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.zeb/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ZEB_HOME (profile-aware) AND the global
    # Zeb root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ZEB_HOME points at
    ...
    for hd in zeb_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Zeb cache file "
    ...
    for hd in zeb_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Zeb credential store "
    ...
    for hd in zeb_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Zeb MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Zeb MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Zeb read (see
...
#
# Zeb profiles are separate ZEB_HOME dirs under
...
#
# Reference: May 2026 incident where a zeb-security profile session
# edited skills under both ``~/.zeb/profiles/zeb-security/skills/``
# AND ``~/.zeb/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ZEB_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ZEB_HOME.
    ...
    """
    try:
        home_real = _zeb_home_path().resolve()
        root_real = _zeb_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Zeb scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Zeb entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Zeb profile {info['target_profile']!r}, but the "
...
#
#   <ZEB_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.zeb/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.zeb/…`` segment and warns
# regardless of which Zeb profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.zeb/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.zeb`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.zeb/…`` and returns the
    index where the inner Zeb-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .zeb / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".zeb":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Zeb state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.zeb``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.zeb`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Zeb resolver to
    succeed, so it works correctly even when called from contexts where
    ZEB_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Zeb process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ZEB_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.zeb/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.zeb/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ZEB_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Zeb state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Zeb "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ZEB_HOME. Use the "
