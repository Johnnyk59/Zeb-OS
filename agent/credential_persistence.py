...
``auth.json``.  They intentionally have no dependency on ``zeb_cli.auth`` so
...
# Sources Zeb owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Zeb-owned OAuth/device-code state)
