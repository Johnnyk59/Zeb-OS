...
MoA-enabled; the normal Zeb agent loop still owns tool calling and turn
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Zeb boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Zeb agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Zeb agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Zeb agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
