...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `zeb backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zeb_home (str): The active ZEB_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.zeb``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zeb").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zeb memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zeb_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zeb memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zeb_home`` is the active ZEB_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ZEB_HOME.

        ``zeb backup`` only walks ZEB_HOME, so any provider state kept
        ...
        of the archive, and ``zeb import`` restores them to their original
