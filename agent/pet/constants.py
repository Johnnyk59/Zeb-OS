...
row taxonomy is inferred from the atlas shape so Zeb can render both legacy
...
# rows (1536x1872), while older Zeb/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Zeb' activity state names. They are not always identical to the
...
# Legacy Zeb/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Zeb activity names -> accepted row-name aliases in descending
