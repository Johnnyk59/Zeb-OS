...
from zeb_constants import get_zeb_home, get_skills_dir, is_wsl
...
_ZEB_MD_NAMES = (".zeb.md", "ZEB.md")


def _find_zeb_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zeb.md`` or ``ZEB.md``.
    ...
    # could pick up a .zeb.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ZEB_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Zeb Agent, an intelligent AI assistant created by Nous Research. "
...
ZEB_AGENT_HELP_GUIDANCE = (
    "You run on Zeb Agent (by Nous Research). When the user needs help with "
    "Zeb itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://zeb-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `zeb-agent` skill with skill_view(name='zeb-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zeb/kanban.db`. Your task id is in "
    "`$ZEB_KANBAN_TASK`; your workspace is `$ZEB_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZEB_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ZEB_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ZEB_KANBAN_BRANCH:-wt/$ZEB_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ZEB_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`zeb profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zeb kanban <verb>` for board operations. Use "
...
#
# The zeb-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to zeb-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `zeb computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Zeb "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Zeb terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Zeb WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Zeb itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Zeb runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Zeb itself is running. The host OS, home, and cwd "
                f"of the Zeb process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Zeb "
    ...
    # Zeb desktop GUI — any agent running under the desktop app should know
    # it. ZEB_DESKTOP marks the backend powering the chat; ZEB_DESKTOP_TERMINAL
    # marks a zeb launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("ZEB_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("ZEB_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Zeb desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Zeb
    ...
    extra = (os.getenv("ZEB_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zeb_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zeb/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ZEB_PLATFORM")
        or get_session_env("ZEB_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Zeb Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `zeb-agent` skill "
            "first. It has the actual commands (e.g. `zeb config set …`, `zeb tools`, "
            "`zeb setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: zeb setup, zeb setup tools, zeb setup terminal, zeb status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ZEB_HOME and return its content, or None.
    ...
    """
    try:
        from zeb_cli.config import ensure_zeb_home
        ensure_zeb_home()
    except Exception as e:
        logger.debug("Could not ensure ZEB_HOME before loading SOUL.md: %s", e)

    soul_path = get_zeb_home() / "SOUL.md"
...
def _load_zeb_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zeb.md / ZEB.md — walk to git root."""
    zeb_md_path = _find_zeb_md(cwd_path)
    if not zeb_md_path:
        return ""
    try:
        content = zeb_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zeb_md_path.name
        try:
            rel = str(zeb_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".zeb.md", context_length=context_length,
            read_path=str(zeb_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", zeb_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .zeb.md / ZEB.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZEB_HOME is independent and always included when present.
    ...
    project_context = (
        _load_zeb_md(cwd_path, context_length)
    ...
    # SOUL.md from ZEB_HOME only — skip when already loaded as identity
    if not skip_soul:
