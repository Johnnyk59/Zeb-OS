# Zeb Agent - Development Guide

Instructions for AI coding assistants and developers working on the zeb-agent codebase.

**Never give up on the right solution.**

## What Zeb Is

Zeb is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Zeb ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`zeb tools`, `zeb setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ZEB_HOME`. Mocks hide integration bugs.
...
- **New `ZEB_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `zeb tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.zeb/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `zeb <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `zeb webhook`, `zeb cron`, `zeb tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.zeb/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.zeb/zeb-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ZEB_CORE_TOOLS list
├── cli.py                # ZebCLI class — interactive CLI orchestrator (~11k LOC)
├── zeb_state.py       # SessionDB — SQLite session store (FTS5 search)
├── zeb_constants.py   # get_zeb_home(), display_zeb_home() — profile-aware paths
├── zeb_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── zeb_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── zeb-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `zeb --tui`
...
**User config:** `~/.zeb/config.yaml` (settings), `~/.zeb/.env` (API keys only).
**Logs:** `~/.zeb/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_zeb_home()`.
Browse with `zeb logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Zeb: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`zeb_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `ZebCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.zeb/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`zeb_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/zeb` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `zeb_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `ZebCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `zeb --tui` or `ZEB_TUI=1`.
...
npm run dev       # watch mode (rebuilds zeb-ink + tsx --watch)
npm start         # production
npm run build     # full build (zeb-ink + tsc)
...
### TUI in the Dashboard (`zeb dashboard` → `/chat`)

The dashboard embeds the real `zeb --tui` — **not** a rewrite.  See `zeb_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `zeb_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `zeb --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `zeb --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@zeb/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `zeb serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `ZEB_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `zeb` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `zeb --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `zeb-desktop-app-work` skill, not `zeb-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Zeb built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Zeb core.
Use the plugin route instead: create `~/.zeb/plugins/<name>/plugin.yaml`
and `~/.zeb/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Zeb tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_ZEB_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ZEB_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_zeb_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ZEB_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_zeb_home()` for the base directory — never `Path.home() / ".zeb"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `zeb_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `zeb_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `zeb tools`, `zeb setup`, most CLI subcommands | `zeb_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`zeb_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
zeb_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.zeb/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Zeb gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `zeb_cli/skin_engine.py`:
...
Users create `~/.zeb/skins/<name>.yaml`:
...
Zeb has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.zeb/plugins/` and pip-installed entry points.

### General plugins (`zeb_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.zeb/plugins/`, `./.zeb/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `zeb` at startup so
  `zeb <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(zeb_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `zeb <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `zeb --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `zeb_cli/main.py`, etc.).
...
into `~/.zeb/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `zeb memory setup` / `post_setup()` without
...
`~/.zeb/plugins/` (or via pip entry points). They register through
...
2. User: `$ZEB_HOME/plugins/model-providers/<name>/`
...
[`zeb-example-plugins`](https://github.com/NousResearch/zeb-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `zeb skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.zeb.tags`, `metadata.zeb.category`,
`metadata.zeb.related_skills`, `metadata.zeb.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.zeb.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Zeb tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Zeb Agent" is the secondary collaborator. If the
   contributor's commit shows "Zeb Agent" as author (because they
   used Zeb to draft the skill), replace it with their actual name
...
lives in the `zeb-agent-dev` skill at
...
`"messaging"`); `_ZEB_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `zeb tools` (the curses UI) or the
...
Zeb returns a delegation id immediately and the result re-enters the
...
go to `~/.zeb/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `zeb_cli/curator.py` wires `zeb curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.zeb/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `zeb cron <verb>`
...
- File lock at `~/.zeb/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `zeb kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `zeb_cli/kanban.py` wires `zeb kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`zeb-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ZEB_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Zeb-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `ZEB_BACKGROUND_NOTIFICATIONS` env var):
...
Zeb supports **profiles** — multiple fully isolated instances, each with its own
`ZEB_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `zeb_cli/main.py` sets
`ZEB_HOME` before any module imports. All `get_zeb_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_zeb_home()` for all ZEB_HOME paths.** Import from `zeb_constants`.
   NEVER hardcode `~/.zeb` or `Path.home() / ".zeb"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".zeb" / "config.yaml"
   ```

2. **Use `display_zeb_home()` for user-facing messages.** Import from `zeb_constants`.
   This returns `~/.zeb` for default or `~/.zeb/profiles/<name>` for profiles.
   ```python
   # GOOD
   from zeb_constants import display_zeb_home
   print(f"Config saved to {display_zeb_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.zeb/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_zeb_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_zeb_home()`,
   not `Path.home() / ".zeb"`.

4. **Tests that mock `Path.home()` must also set `ZEB_HOME`** — since code now uses
   `get_zeb_home()` (reads env var), not `Path.home() / ".zeb"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ZEB_HOME": str(tmp_path / ".zeb")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ZEB_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".zeb" / "profiles"`, NOT `get_zeb_home() / "profiles"`.
   This is intentional — it lets `zeb -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.zeb` paths
Use `get_zeb_home()` from `zeb_constants` for code paths. Use `display_zeb_home()`
for user-facing print/log messages. Hardcoding `~/.zeb` breaks profiles — each profile
has its own `ZEB_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `zeb_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `zeb_cli/curses_ui.py` — see
`zeb_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `ZEB_HOME`.

### Tests must not write to `~/.zeb/`
The `_isolate_zeb_home` autouse fixture in `tests/conftest.py` redirects `ZEB_HOME` to a temp dir. Never hardcode `~/.zeb/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_zeb_home()` resolve within the temp dir.
Use the pattern from `tests/zeb_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEB_HOME", str(home))
    return home
```
...
| HOME / `~/.zeb/` | Your real config+auth.json                  | Temp dir per test                         |
