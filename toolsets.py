...
_ZEB_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on ZEB_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ZEB_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `zeb tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ZEB_KANBAN_TASK env "
    ...
    # Coding posture (base Zeb — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in zeb_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Zeb toolsets (CLI + messaging platforms)
    #
    ...
    # the `zeb send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "zeb-cli": {
        ...
        "tools": _ZEB_CORE_TOOLS,
        "includes": []
    },

    "zeb-cron": {
        # Mirrors zeb-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zeb tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as zeb-cli; gated by `zeb tools`",
        "tools": _ZEB_CORE_TOOLS,
        "includes": []
    },
    ...
    "zeb-email": {
        "description": "Email bot toolset - interact with Zeb via email (IMAP/SMTP)",
        "tools": _ZEB_CORE_TOOLS,
        "includes": []
    },
    ...
    "zeb-sms": {
        "description": "SMS bot toolset - interact with Zeb via SMS (Twilio)",
        "tools": _ZEB_CORE_TOOLS,
        "includes": []
    },
    ...
    "zeb-gateway": {
        ...
        "tools": [],
        "includes": ["zeb-telegram", "zeb-discord", "zeb-whatsapp", "zeb-slack", "zeb-signal", "zeb-bluebubbles", "zeb-homeassistant", "zeb-email", "zeb-sms", "zeb-mattermost", "zeb-matrix", "zeb-dingtalk", "zeb-feishu", "zeb-wecom", "zeb-wecom-callback", "zeb-weixin", "zeb-qqbot", "zeb-webhook", "zeb-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``zeb-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ZEB_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``zeb-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (zeb-<name>).
        # Gives them _ZEB_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("zeb-"):
            platform_name = name[len("zeb-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ZEB_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
