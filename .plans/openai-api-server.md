# OpenAI-Compatible API Server for Zeb Agent
...
SSE streaming. By exposing this endpoint, zeb-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  zeb-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zeb/.env`
2. Run `zeb gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zeb-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zeb-agent as a model) |
...
Authorization: Bearer zeb-api-key-here
...
But zeb-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zeb-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zeb-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zeb-agent's core prompt
...
Once implemented, zeb-agent works as a drop-in backend for:
