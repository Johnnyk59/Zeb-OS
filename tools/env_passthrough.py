...
def _is_zeb_provider_credential(name: str) -> bool:
    """True if ``name`` is a Zeb-managed provider credential (API key,
    token, or similar) per ``_ZEB_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Zeb API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Zeb credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Zeb-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_zeb_internal_secret(name):
        return True
    return name in _ZEB_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Zeb-managed provider credentials (from
    ``_ZEB_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Zeb-managed
    ...
    Non-Zeb third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_zeb_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Zeb provider "
                "credential %r (blocked by _ZEB_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Zeb-managed provider credentials must not be passed
                ...
                if _is_zeb_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Zeb "
                        "provider credential %r from config.yaml (blocked "
                        "by _ZEB_PROVIDER_ENV_BLOCKLIST). Operator "
