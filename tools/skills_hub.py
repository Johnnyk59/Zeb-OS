#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Zeb Skills Hub.
...
Used by zeb_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _zeb_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zeb_meta = metadata.get("zeb", {})
            if isinstance(zeb_meta, dict):
                tags = zeb_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.zeb/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/zeb-agent"
...
# ---------------------------------------------------------------------------
# Zeb centralized index source
# ---------------------------------------------------------------------------

ZEB_INDEX_URL = "https://zeb-agent.nousresearch.com/docs/api/skills-index.json"
ZEB_INDEX_TTL = 6 * 3600  # 6 hours


def _zeb_index_cache_file() -> Path:
    return _index_cache_dir() / "zeb-index.json"


def _load_zeb_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZEB_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    zeb_index_cache_file = _zeb_index_cache_file()
    if zeb_index_cache_file.exists():
        try:
            age = time.time() - zeb_index_cache_file.stat().st_mtime
            if age < ZEB_INDEX_TTL:
                return json.loads(zeb_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Zeb index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Zeb index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Zeb index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        zeb_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        zeb_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZebIndexSource(SkillSource):
    """Skill source backed by the centralized Zeb Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zeb-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zeb-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZebIndexSource(auth=auth), # Centralized index (search + resolved install paths)
