...
* Never raises. Any I/O error (disk full, permission denied, missing
  ZEB_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_zeb_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: ZEB_HOME env var, then ~/.zeb
            home = os.environ.get("ZEB_HOME") or os.path.expanduser("~/.zeb")
