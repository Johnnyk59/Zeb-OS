...
`zeb skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.zeb.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.zeb.blueprint`` is a mapping containing
    ...
    zeb = meta.get("zeb") if isinstance(meta, dict) else None
    blueprint = zeb.get("blueprint") if isinstance(zeb, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.zeb.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.zeb.blueprint`` block) they can hand
    to ``zeb skills publish`` to share. ``body`` is the plain-language
