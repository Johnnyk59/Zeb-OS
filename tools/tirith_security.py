...
it is automatically downloaded from GitHub releases to $ZEB_HOME/bin/tirith.
...
def _get_zeb_home() -> str:
    """Return the Zeb home directory, respecting ZEB_HOME env var."""
    return str(get_zeb_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zeb_home(), ".tirith-install-failed")
...
def _zeb_bin_dir() -> str:
    """Return $ZEB_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zeb_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZEB_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zeb_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZEB_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZEB_HOME/bin/tirith
    ...
    zeb_bin = os.path.join(_zeb_bin_dir(), "tirith")
    if os.path.isfile(zeb_bin) and os.access(zeb_bin, os.X_OK):
        _resolved_path = zeb_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zeb_bin
