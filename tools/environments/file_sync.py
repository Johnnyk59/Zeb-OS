...
_FORCE_SYNC_ENV = "ZEB_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.zeb") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.zeb to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ZEB_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, zeb_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.zeb/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="zeb-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.zeb/skills/a.md`` →
        ``~/.zeb/skills/a.md``, a new remote file at
        ``/root/.zeb/skills/b.md`` maps to ``~/.zeb/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
