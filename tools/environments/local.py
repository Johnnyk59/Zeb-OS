...
# Zeb-internal env vars that should NOT leak into terminal subprocesses.
_ZEB_PROVIDER_ENV_FORCE_PREFIX = "_ZEB_FORCE_"

# Zeb-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Zeb inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Zeb-managed inference credential — Claude subscription auth is not a
    # working Zeb provider path.  Stripping it broke agent-spawned
...
_ZEB_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Zeb venv path instead of the project's own
# ``.venv`` — silently clobbering the Zeb environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Zeb venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_zeb_internal_secret(key: str) -> bool:
    """Return True for Zeb-internal secrets injected under *dynamic* names.

    ``_ZEB_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Zeb-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`zeb_subprocess_env` helper all call it, so the
...
def _inject_context_zeb_home(env: dict) -> None:
    """Bridge the context-local Zeb home override into subprocess env."""
    try:
        from zeb_constants import get_zeb_home_override

        value = get_zeb_home_override()
        if value:
            env["ZEB_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ZEB_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Zeb-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ZEB_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_zeb_internal_secret(key):
            continue
        if key not in _ZEB_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ZEB_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ZEB_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_zeb_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_zeb_internal_secret(key):
            continue
        elif key not in _ZEB_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_zeb_home(sanitized)

    from zeb_constants import apply_subprocess_home_env
...
# legitimate child Zeb spawns needs them, and they are the highest-value
...
# narrow subset of _ZEB_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in zeb_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_zeb_internal_secret, but _ID has no secret suffix, so it must be
...
def zeb_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_ZEB_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Zeb spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_ZEB_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Zeb-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_zeb_internal_secret`.
    for key in list(env):
        if key.startswith(_ZEB_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_zeb_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _ZEB_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_zeb_home(env)
    from zeb_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose ZEB_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ZEB_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\zeb\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\zeb\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _zeb_portable_git = os.path.join(_local_appdata, "zeb", "git") if _local_appdata else ""
    if _zeb_portable_git:
        for candidate in (
            os.path.join(_zeb_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_zeb_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Zeb Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ZEB_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``zeb`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_ZEB_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_zeb_bin_dir() -> str | None:
    """Return the directory holding the ``zeb`` console-script, or None.
    ...
    zeb install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``zeb``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``zeb`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``zeb`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("zeb")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``zeb`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _ZEB_BIN_DIR
    if _ZEB_BIN_DIR is not _SENTINEL:
        return _ZEB_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("zeb")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "zeb" or base.startswith("zeb."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "zeb.exe" if _IS_WINDOWS else "zeb"
    ...
    _ZEB_BIN_DIR = candidate
    return candidate


def _prepend_zeb_bin_dir(existing_path: str) -> str:
    """Prepend the zeb install dir to ``existing_path`` if it's missing.
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Zeb
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ZEB_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ZEB_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_zeb_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_zeb_internal_secret(k):
            continue
        elif k not in _ZEB_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the zeb install dir is reachable so plugins can shell out
        # to bare ``zeb`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_zeb_bin_dir(new_path)

    _inject_context_zeb_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Zeb trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ZEB_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ZEB_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_zeb_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "zeb_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._zeb_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_zeb_pgid", None)
                    if pgid is None:
                        raise
