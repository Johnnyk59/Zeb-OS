...
One ``CDPSupervisor`` runs per Zeb ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Zeb logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "zeb-dialog-bridge.invalid"
...
(() => {
  if (window.__zebDialogBridgeInstalled) return;
  window.__zebDialogBridgeInstalled = true;
  const ENDPOINT = "http://zeb-dialog-bridge.invalid/";
