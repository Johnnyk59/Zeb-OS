...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ZEB_YOLO_MODE", ""))
...
# os.environ["ZEB_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ZEB_INTERACTIVE.
_zeb_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "zeb_interactive",
    default=None,
)


def set_zeb_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["ZEB_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ZEB_INTERACTIVE`` env var for legacy callers.
    """
    return _zeb_interactive_ctx.set("1" if interactive else "")


def reset_zeb_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_zeb_interactive_context`."""
    _zeb_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ZEB_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _zeb_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ZEB_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ZEB_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ZEB_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ZEB_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ZEB_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ZEB_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ZEB_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ZEB_CRON_SESSION"):
        return False
    if env_var_enabled("ZEB_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ZEB_HOME, or by the resolved absolute
# active profile home path such as /home/zeb/.zeb/config.yaml. The
# resolved-absolute form is folded into the ~/.zeb/ patterns at detection
...
# go stale when ZEB_HOME is set after this module is imported, e.g. under the
...
# ~/.zeb/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ZEB_ENV_PATH; matches the ZEB_HOME override form as
# well as ~/.zeb/.
_ZEB_CONFIG_PATH = (
    r'(?:~\/\.zeb/|'
    r'(?:\$home|\$\{home\})/\.zeb/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `zeb` and `gateway` (e.g. `zeb -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bzeb\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart zeb gateway (kills running agents)"),
    (r'\bzeb\s+update\b', "zeb update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `zeb gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart zeb-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart zeb-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(zeb|gateway|cli\.py)\b', "kill zeb/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill zeb` but not
    # `kill -9 $(pgrep -f zeb)` because the substitution is opaque
    ...
    # the `zeb gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.zeb.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(zeb|ai\.zeb)\b', "stop/restart zeb launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Zeb file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.zeb/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Zeb-managed security file (~/.zeb/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ZEB_CONFIG_PATH}|{_ZEB_ENV_PATH})', "in-place edit of Zeb config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ZEB_CONFIG_PATH}|{_ZEB_ENV_PATH})', "in-place edit of Zeb config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ZEB_CONFIG_PATH}|{_ZEB_ENV_PATH})', "in-place edit of Zeb config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.zeb/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ZEB_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Zeb home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\zeb), so folding the user home
    # first would eat the prefix the Zeb-home fold needs.
    command = _rewrite_resolved_zeb_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ZEB_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.zeb``); the matched
...
def _rewrite_resolved_zeb_home(command: str) -> str:
    """Rewrite the resolved absolute Zeb home prefix to ``~/.zeb/``.

    Resolves the active ``ZEB_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.zeb/`` so the static ``_ZEB_CONFIG_PATH`` / ``_ZEB_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/zeb/.zeb/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ZEB_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from zeb_constants import get_zeb_home
        home = get_zeb_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.zeb")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ZEB_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ZEB_SPINNER_PAUSE" in os.environ:
            del os.environ["ZEB_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Zeb approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``ZEB_YOLO_MODE`` (frozen at import time
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ZEB_INTERACTIVE or ZEB_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ZEB_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ZEB_EXEC_ASK")
        ...
        if env_var_enabled("ZEB_CRON_SESSION"):
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from zeb_cli.config import load_config as _load_cfg
