...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``zeb tools`` repaint, tool-registration scans,
    ...
    2. ``~/.zeb/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_zeb_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.zeb/.env`` first, then ``os.environ``.

    Wraps :func:`zeb_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from zeb_cli.config import get_env_value as _zeb_get_env_value

        value = _zeb_get_env_value(name)
...
def zeb_xai_user_agent() -> str:
    """Return a stable Zeb-specific User-Agent for xAI HTTP calls."""
    try:
        from zeb_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Zeb-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Zeb config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Zeb home, then mark it seen."""
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Zeb-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``zeb_cli.config.get_env_value`` so keys
    stored in ``~/.zeb/.env`` (the standard Zeb location) are honored —
