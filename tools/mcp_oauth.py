...
This module provides the glue:
    - ``ZebTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Zeb Agent"
"""
...
from zeb_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__zeb_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ZEB_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZEB_HOME/mcp-tokens/``
    """
    try:
        from zeb_constants import get_zeb_home
        base = Path(get_zeb_home())
    except ImportError:
        base = Path(os.environ.get("ZEB_HOME", str(Path.home() / ".zeb")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``zeb mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `zeb mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# ZebTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZebTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZEB_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZEB_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ZEB_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Zeb records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Zeb.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://zeb-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `zeb mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ZebTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Zeb Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`zeb mcp login {server_name}` interactively first to complete "
