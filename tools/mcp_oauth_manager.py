...
`mcp_tool.py`, and `zeb_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ZebMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_zeb_provider_class() -> Optional[type]:
    ...
    class ZebMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._zeb_server_name = server_name
            ...
            self._zeb_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ZebTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ZebTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ZebTokenStorage
                        if isinstance(storage, ZebTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._zeb_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ZebTokenStorage
            if not isinstance(storage, ZebTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``zeb mcp reauth``.
            ...
            back to ``zeb mcp reauth``.
            """
            try:
                if self._zeb_preregistered:
                    return
    ...
    return ZebMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ZEB_PROVIDER_CLS: Optional[type] = _make_zeb_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ZebMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `zeb mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``zeb mcp remove <name>`` and (indirectly) by
        ``zeb mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
