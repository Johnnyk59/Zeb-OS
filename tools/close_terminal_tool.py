#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Zeb desktop GUI.
...
``read_terminal`` it is gated on ``ZEB_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — ZEB_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("ZEB_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Zeb desktop GUI (the tabs mirroring terminal(background=true) runs). "
