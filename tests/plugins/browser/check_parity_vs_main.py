...
Run from the PR worktree:
    ...
    cd ~/.zeb/zeb-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.zeb/zeb-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.zeb/zeb-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a zeb-agent checkout"
)
...
# Isolated ZEB_HOME for the config write.
