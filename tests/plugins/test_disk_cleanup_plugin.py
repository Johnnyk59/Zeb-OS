...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate ZEB_HOME for each test.

    The global hermetic fixture already redirects ZEB_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    ZEB_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "zeb_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "zeb_plugins" not in sys.modules:
        ns = types.ModuleType("zeb_plugins")
        ns.__path__ = []
        sys.modules["zeb_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "zeb_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["zeb_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_zeb_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_zeb_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_zeb_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/zeb-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "zeb-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "zeb-agent":
                raise AssertionError("quick() descended into protected zeb-agent/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, zeb_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from zeb_cli import plugins as pmod
