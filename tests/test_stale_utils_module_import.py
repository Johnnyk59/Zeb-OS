...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.zeb/zeb-agent/utils.py)
...
2. ``zeb update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``zeb_cli/main.py`` (the ``zeb update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``zeb_constants`` after the
