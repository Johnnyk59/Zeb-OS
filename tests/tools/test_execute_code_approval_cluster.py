...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ZEB_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ZEB_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_zeb_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ZEB_HOME": "/h", "ZEB_PROFILE": "p",
        "ZEB_CONFIG": "/c.yaml", "ZEB_ENV": "/e",
        # other ZEB_* → dropped (broad prefix removed)
        "ZEB_BASE_URL": "https://x", "ZEB_INTERACTIVE": "1",
        "ZEB_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ZEB_HOME", "ZEB_PROFILE", "ZEB_CONFIG", "ZEB_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZEB_BASE_URL", "ZEB_INTERACTIVE", "ZEB_KANBAN_DB",
...
def test_env_scrub_logs_dropped_zeb_vars(caplog):
    """Dropping a non-allowlisted, non-secret ZEB_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ZEB_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ZEB_HOME": "/h",          # allowlisted → kept, not logged
        "ZEB_BASE_URL": "https://x",   # dropped → logged
        "ZEB_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ZEB_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ZEB_HOME" in out and "PATH" in out
    assert "ZEB_BASE_URL" not in out and "ZEB_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ZEB_BASE_URL" in msgs and "ZEB_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ZEB_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ZEB_* vars."""
    import logging
