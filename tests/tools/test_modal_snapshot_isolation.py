...
def _restore_tool_modules():
    original_zeb_home = os.environ.get("ZEB_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zeb_home is None:
            os.environ.pop("ZEB_HOME", None)
        else:
            os.environ["ZEB_HOME"] = original_zeb_home
        _reset_modules(("tools", "zeb_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zeb_cli = types.ModuleType("zeb_cli")
    zeb_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zeb_cli"] = zeb_cli
    zeb_home = tmp_path / "zeb-home"
    os.environ["ZEB_HOME"] = str(zeb_home)
    sys.modules["zeb_cli.config"] = types.SimpleNamespace(
        get_zeb_home=lambda: zeb_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zeb-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zeb_home / "modal_snapshots.json",
