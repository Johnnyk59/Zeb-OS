...
def _clean_state():
    ...
    saved = {}
    for k in ("ZEB_INTERACTIVE", "ZEB_GATEWAY_SESSION", "ZEB_EXEC_ASK", "ZEB_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZEB_INTERACTIVE"] = "1"
        session_key = os.getenv("ZEB_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZEB_INTERACTIVE or ZEB_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
