...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zeb_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(zeb_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(zeb_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zeb_bin(self):
        """After _INSTALL_FAILED, manual install in ZEB_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zeb_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zeb_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zeb_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zeb_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zeb_bin
            assert _tirith_mod._resolved_path == zeb_bin
            mock_clear.assert_called_once()
...
class TestZebHomeIsolation:
    def test_zeb_bin_dir_respects_zeb_home(self):
        """_zeb_bin_dir must use ZEB_HOME, not hardcoded ~/.zeb."""
        from tools.tirith_security import _zeb_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZEB_HOME": tmpdir}):
            result = _zeb_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zeb_home(self):
        """_failure_marker_path must use ZEB_HOME, not hardcoded ~/.zeb."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZEB_HOME": "/custom/zeb"}):
            result = _failure_marker_path()
        assert result == "/custom/zeb/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZEB_HOME; verify it's active."""
        zeb_home = os.getenv("ZEB_HOME")
        assert zeb_home is not None, "ZEB_HOME should be set by conftest"
        assert "zeb_test" in zeb_home, "Should point to test temp dir"

    def test_get_zeb_home_fallback(self):
        """Without ZEB_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_zeb_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZEB_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".zeb")
            result = _get_zeb_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._zeb_bin_dir",
