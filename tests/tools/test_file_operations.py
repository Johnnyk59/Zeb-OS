...
class TestIsWriteDenied:
    ...
    def test_zeb_control_files_requested_writable(self, path):
        from zeb_constants import get_zeb_home

        assert _is_write_denied(str(get_zeb_home() / path)) is False
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.zeb/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__ZEB_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__ZEB_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "ZEB_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__ZEB_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__ZEB_FENCE_a9f7b3__\n"
        )
