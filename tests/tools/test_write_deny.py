...
class TestWriteDenyExactPaths:
    ...
    def test_zeb_env(self):
        # ``.env`` under the active ZEB_HOME (profile-aware, not just
        # ``~/.zeb``) must be write-denied. The hermetic test conftest
        # points ZEB_HOME at a tempdir — resolve via get_zeb_home()
        # to match the denylist.
        from zeb_constants import get_zeb_home
        path = str(get_zeb_home() / ".env")
        assert _is_write_denied(path) is True

    def test_zeb_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.zeb/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zeb_root"
        ...
        # Sanity check: ZEB_HOME does point to the profile dir, not the root.
        from zeb_constants import get_zeb_home, get_default_zeb_root
        assert get_zeb_home() == profile_home
        assert get_default_zeb_root() == root
...
class TestWriteAllowed:
    ...
    def test_zeb_control_files_requested_writable(self):
        from zeb_constants import get_zeb_home
