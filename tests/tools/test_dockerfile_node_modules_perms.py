...
Older images made /opt/zeb/ui-tui and /opt/zeb/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZEB_TUI_DIR=/opt/zeb/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R zeb:zeb /opt/zeb/ui-tui" not in text
    assert "chown -R zeb:zeb /opt/zeb/node_modules" not in text
