...
def test_zeb_provider_subclass_exists():
    """ZebMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZEB_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZEB_PROVIDER_CLS is not None
    assert issubclass(_ZEB_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_zeb_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZebMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZEB_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZEB_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ZEB_PROVIDER_CLS is not None
    assert isinstance(provider, _ZEB_PROVIDER_CLS)
    assert provider._zeb_server_name == "srv"
