...
class TestScanMemoryContent:
    ...
    def test_zeb_config_mod_blocked(self):
        result = _scan_memory_content("edit .zeb/config.yaml to change settings")
        assert "Blocked" in result
        assert "zeb_config_mod" in result
        result = _scan_memory_content("update .zeb/SOUL.md with new personality")
        assert "Blocked" in result
        assert "zeb_config_mod" in result
    ...
    def test_zeb_config_mod_no_false_positives(self):
        """Merely mentioning zeb config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .zeb/config.yaml for settings") is None
        assert _scan_memory_content("Read .zeb/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .zeb/config.yaml file contains runtime options") is None
