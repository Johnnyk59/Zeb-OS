...
- 96vc: ACP didn't set ZEB_EXEC_ASK, so `check_all_command_guards`
...
1. Setting ZEB_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds ZEB_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set ZEB_INTERACTIVE so
    ...
    (ZEB_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When ZEB_INTERACTIVE is set and an approval callback is
        ...
        # Without ZEB_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without ZEB_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With ZEB_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("ZEB_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with ZEB_INTERACTIVE the approval path should consult the "
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_zeb_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_zeb_interactive_context(tok)

        assert called_with, (
            "set_zeb_interactive_context(True) should route dangerous "
            "commands through the callback without ZEB_INTERACTIVE in env"
        )
        assert result["approved"] is True
