"""Tests for ACP Registry metadata shipped with Zeb."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "zeb-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Zeb Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/zeb-agent"
    assert data["website"].startswith("https://zeb-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"zeb-agent[acp]=={data['version']}"
    assert uvx["args"] == ["zeb-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"zeb-agent[acp]=={_pyproject_version()}"
    )
