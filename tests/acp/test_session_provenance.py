...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZebSessionId"] == "root1"
    assert prov["rootZebSessionId"] == "root1"
    assert prov["parentZebSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zeb_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZebSessionId"] == "old"
    assert prov["rootZebSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZebSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootZebSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZebSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_zeb_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousZebSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"zeb"}
    assert "sessionProvenance" in meta["zeb"]
    assert meta["zeb"]["sessionProvenance"]["currentZebSessionId"] == "root1"
