...
Uses a throwaway ZEB_HOME so it never touches ~/.zeb. No real names.
"""
...
def _fresh_home():
    """Point ZEB_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_zeb_home() at import time)."""
...
# a different get_zeb_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ZEB_HOME (throwaway): {HOME}")
