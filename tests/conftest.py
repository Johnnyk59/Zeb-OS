"""Shared fixtures for the zeb-agent test suite.
...
2. **Isolated ZEB_HOME.** ZEB_HOME points to a per-test tempdir so
   code reading ``~/.zeb/*`` via ``get_zeb_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".zeb"`` instead of the canonical
   ``get_zeb_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZEB_SESSION_* inheritance** — the agent's current gateway
...
# ZEB_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZEB_BEHAVIORAL_VARS = frozenset({
    "ZEB_YOLO_MODE",
    "ZEB_INTERACTIVE",
    "ZEB_QUIET",
    "ZEB_TOOL_PROGRESS",
    "ZEB_TOOL_PROGRESS_MODE",
    "ZEB_MAX_ITERATIONS",
    "ZEB_SESSION_PLATFORM",
    "ZEB_SESSION_CHAT_ID",
    "ZEB_SESSION_CHAT_NAME",
    "ZEB_SESSION_THREAD_ID",
    "ZEB_SESSION_SOURCE",
    "ZEB_SESSION_KEY",
    "ZEB_GATEWAY_SESSION",
    "ZEB_CRON_SESSION",
    ...
    "ZEB_TIMEZONE",
    "ZEB_REDACT_SECRETS",
    "ZEB_BACKGROUND_NOTIFICATIONS",
    "ZEB_EXEC_ASK",
    "ZEB_HOME_MODE",
    "ZEB_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zeb/kanban.db instead of the per-test ZEB_HOME.
    "ZEB_KANBAN_DB",
    "ZEB_KANBAN_BOARD",
    "ZEB_KANBAN_HOME",
    "ZEB_KANBAN_WORKSPACES_ROOT",
    "ZEB_KANBAN_LOGS_ROOT",
    "ZEB_KANBAN_TASK",
    "ZEB_KANBAN_WORKSPACE",
    "ZEB_KANBAN_RUN_ID",
    "ZEB_KANBAN_CLAIM_LOCK",
    "ZEB_KANBAN_DISPATCH_IN_GATEWAY",
    "ZEB_TENANT",
    ...
    "ZEB_DASHBOARD_OAUTH_CLIENT_ID",
    "ZEB_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZEB_HOME to per-test tempdirs so code that
    reads ``~/.zeb/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZEB_* vars that could change test semantics.
    for name in _ZEB_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ZEB_HOME to a per-test tempdir. Code that reads
    #    ``~/.zeb/*`` via ``get_zeb_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zeb/*`` via
    #    ``Path.home() / ".zeb"`` instead of ``get_zeb_home()``
    #    is a bug to fix at the callsite.
    fake_zeb_home = tmp_path / "zeb_test"
    fake_zeb_home.mkdir()
    (fake_zeb_home / "sessions").mkdir()
    (fake_zeb_home / "cron").mkdir()
    (fake_zeb_home / "memories").mkdir()
    (fake_zeb_home / "skills").mkdir()
    monkeypatch.setenv("ZEB_HOME", str(fake_zeb_home))
    ...
    #    ~/.zeb/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zeb_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_zeb_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal zeb config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``zeb-gateway`` process
...
# correlated with a ``tests/zeb_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> zeb-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart zeb-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zeb/python patterns are also blocked.
    """
    ...
    def _matches_zeb_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZEB_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zeb_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch zeb-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zeb_cli.main".
                if (
                    "zeb" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live zeb-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting zeb/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `zeb update` (or the
        # equivalent `python -m zeb_cli.main update`).  These commands
        ...
        if "update" in low and (
            # zeb update / zeb update --gateway / setsid bash -c ... zeb update
            ("zeb" in low and "update" in low.split())
            or
            # python -m zeb_cli.main update --gateway
            ("zeb_cli" in low and "update" in low.split())
            or
            # venv/bin/zeb update  (absolute path variant used in tests)
            (".venv/bin/zeb" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`zeb update` against the real checkout, fetching "
