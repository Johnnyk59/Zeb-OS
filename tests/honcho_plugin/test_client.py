...
from zeb_cli.profiles import _get_default_zeb_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "zeb"
        assert config.workspace_id == "zeb"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.zeb/honcho.json
        monkeypatch.setenv("ZEB_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"zeb": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="zeb-agent"
        ):
            result = config.resolve_session_name("/home/user/zeb-agent/subdir")
        assert result == "zeb-agent"
...
class TestResolveConfigPath:
    def test_prefers_zeb_home_when_exists(self, tmp_path):
        zeb_home = tmp_path / "zeb"
        zeb_home.mkdir()
        local_cfg = zeb_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ZEB_HOME points at ~/.zeb/profiles/<name>, so
        # _get_default_zeb_home() must resolve back to ~/.zeb — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zeb"
        ...
        assert _get_default_zeb_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_zeb_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("ZEB_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".zeb".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zeb"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "zeb_coder"
        assert profile_host_key("default") == "zeb"

    def test_default_returns_zeb(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ZEB_HONCHO_HOST", None)
            os.environ.pop("ZEB_HOME", None)
            assert resolve_active_host() == "zeb"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ZEB_HONCHO_HOST": "zeb.coder"}):
            assert resolve_active_host() == "zeb.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEB_HONCHO_HOST", None)
            with patch("zeb_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "zeb_coder"

    def test_default_profile_returns_zeb(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEB_HONCHO_HOST", None)
            with patch("zeb_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "zeb"

    def test_custom_profile_returns_zeb(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEB_HONCHO_HOST", None)
            with patch("zeb_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "zeb"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEB_HONCHO_HOST", None)
            # Temporarily remove zeb_cli.profiles to simulate import failure
            saved = sys.modules.get("zeb_cli.profiles")
            sys.modules["zeb_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "zeb"
            finally:
                if saved is not None:
                    sys.modules["zeb_cli.profiles"] = saved
                else:
                    sys.modules.pop("zeb_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zeb_coder")
        assert config.host == "zeb_coder"
        assert config.workspace_id == "zeb"  # shared workspace
        assert config.ai_peer == "zeb_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zeb")
        assert config.host == "zeb"
        assert config.workspace_id == "zeb"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="zeb_coder", config_path=config_file,
        )
        assert config.host == "zeb_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zeb_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="zeb_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "zeb_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zeb.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="zeb_dreamer",
        ...
        assert config.host == "zeb_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "zeb_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zeb": {"enabled": True, "aiPeer": "zeb"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zeb": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_zeb_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zeb",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zeb_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zeb_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_zeb_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zeb",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zeb_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
