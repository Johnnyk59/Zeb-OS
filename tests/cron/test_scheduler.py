...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("zeb_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._zeb_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("zeb_cli.env_loader.load_zeb_dotenv"), \
             patch("zeb_cli.env_loader.reset_secret_source_cache"), \
             patch("zeb_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("cron.scheduler._zeb_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("zeb_cli.env_loader.load_zeb_dotenv"),
            patch("zeb_cli.env_loader.reset_secret_source_cache"),
            patch("zeb_state.SessionDB", return_value=fake_db),
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``zeb tools`` platform config for ``cron``
        ...
        # The cron default is _ZEB_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``zeb tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        # Even if the user has ``zeb tools`` configured to enable web+file
        # for cron, the per-job override wins.
        extra = [patch("zeb_cli.tools_config._get_platform_tools", return_value={"web", "file"})]
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("ZEB_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("ZEB_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("ZEB_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("ZEB_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("ZEB_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("ZEB_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("ZEB_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("ZEB_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("ZEB_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this ZEB_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_zeb_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._zeb_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("zeb_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("zeb_cli.env_loader.load_zeb_dotenv", _record_load), \
             patch("zeb_state.SessionDB", return_value=fake_db), \
             patch(
                 "zeb_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("ZEB_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("ZEB_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("ZEB_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("cron.scheduler._zeb_home", tmp_path), \
             ...
             patch("zeb_cli.env_loader.load_zeb_dotenv"), \
             patch("zeb_cli.env_loader.reset_secret_source_cache"), \
             patch("zeb_cli.runtime_provider.resolve_runtime_provider",
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ZEB_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_ZEB_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_ZEB_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_ZEB_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ZEB_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_ZEB_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_ZEB_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``ZEB_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses ZEB_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("ZEB_MODEL", "env-model")
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved ZEB_MODEL.
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``zeb_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._zeb_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_zeb_home", return_value=tmp_path), \
             patch("zeb_cli.env_loader.load_zeb_dotenv"), \
             patch("zeb_cli.env_loader.reset_secret_source_cache"), \
             patch("zeb_state.SessionDB", return_value=fake_db), \
             patch(
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """ZEB_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("ZEB_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``zeb cron create --deliver=<platform>`` silently
