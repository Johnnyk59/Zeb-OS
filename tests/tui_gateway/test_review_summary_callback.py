...
def server():
    with patch.dict(
        "sys.modules",
        {
            "zeb_constants": MagicMock(
                get_zeb_home=MagicMock(return_value="/tmp/zeb_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'zeb-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'zeb-release' patched"
    }
