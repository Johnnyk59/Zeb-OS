"""Tests for zeb_constants module."""
...
from zeb_constants import (
    ...
    find_zeb_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_zeb_root,
    get_zeb_dir,
    get_zeb_home,
    heal_zeb_managed_node,
    zeb_managed_node_tree_present,
    iter_zeb_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultZebRoot:
    """Tests for get_default_zeb_root() — Docker/custom deployment awareness."""

    def test_no_zeb_home_returns_native(self, tmp_path, monkeypatch):
        """When ZEB_HOME is not set, returns ~/.zeb."""
        monkeypatch.delenv("ZEB_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_zeb_root() == tmp_path / ".zeb"

    def test_zeb_home_is_native(self, tmp_path, monkeypatch):
        """When ZEB_HOME = ~/.zeb, returns ~/.zeb."""
        native = tmp_path / ".zeb"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEB_HOME", str(native))
        assert get_default_zeb_root() == native

    def test_zeb_home_is_profile(self, tmp_path, monkeypatch):
        """When ZEB_HOME is a profile under ~/.zeb, returns ~/.zeb."""
        native = tmp_path / ".zeb"
    ...
    def test_zeb_home_is_docker(self, tmp_path, monkeypatch):
        """When ZEB_HOME points outside ~/.zeb (Docker), returns ZEB_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEB_HOME", str(docker_home))
        assert get_default_zeb_root() == docker_home

    def test_zeb_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ZEB_HOME outside ~/.zeb is treated as the root."""
        custom = tmp_path / "my-zeb-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEB_HOME", str(custom))
        assert get_default_zeb_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ZEB_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEB_HOME", str(profile))
        assert get_default_zeb_root() == docker_root

    def test_no_zeb_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\zeb, not ~/.zeb."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ZEB_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(zeb_constants.sys, "platform", "win32")

        assert get_default_zeb_root() == local_appdata / "zeb"

    def test_no_zeb_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/zeb without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ZEB_HOME", raising=False)
        ...
        assert get_default_zeb_root() == home / "AppData" / "Local" / "zeb"


class TestGetZebHome:
    """Tests for get_zeb_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ZEB_HOME is unset on Windows, use %LOCALAPPDATA%\\zeb."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ZEB_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(zeb_constants.sys, "platform", "win32")
        monkeypatch.setattr(zeb_constants, "_profile_fallback_warned", False)

        assert get_zeb_home() == local_appdata / "zeb"


class TestZebManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_zeb_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(zeb_constants.sys, "platform", "win32")
        monkeypatch.setenv("ZEB_HOME", str(home))
        monkeypatch.setattr(zeb_constants, "node_tool_runnable", lambda path: True)

        assert find_zeb_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZEB_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(zeb_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(zeb_constants, "heal_zeb_managed_node", lambda: False)
        monkeypatch.setattr(
            zeb_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert zeb_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_zeb_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "zeb"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_zeb_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Zeb-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(zeb_constants, "heal_zeb_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_zeb_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(zeb_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(zeb_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".zeb"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import zeb_cli._subprocess_compat as subprocess_compat
...
class TestGetZebDir:
    """Tests for ``get_zeb_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_zeb_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_zeb_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_zeb_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_zeb_dir("cache/audio", "audio_cache")
