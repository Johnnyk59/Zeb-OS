...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_zeb_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <ZEB_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``zeb pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <ZEB_HOME>/profiles/yangyang/pairing/."""
        from zeb_constants import get_zeb_home
        monkeypatch.setattr("zeb_constants.get_zeb_home", lambda: tmp_path)
