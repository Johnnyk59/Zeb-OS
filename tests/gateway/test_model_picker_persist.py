...
closure the PR changed, against a real temp ``ZEB_HOME``.
"""
...
def _fake_switch_result():
    ...
    from zeb_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "zeb_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_zeb_home() / config.yaml`` — point it here.
    monkeypatch.setattr("zeb_constants.get_zeb_home", lambda: zeb_home)
    monkeypatch.setattr("zeb_cli.config.get_zeb_home", lambda: zeb_home)
    return cfg_path
