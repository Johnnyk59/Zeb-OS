...
""",
        ...
        from zeb_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        zeb_home = tmp_path / ".zeb"
        zeb_home.mkdir()
        (zeb_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ZEB_HOME", str(zeb_home))
        from zeb_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from zeb_cli.plugins import PluginContext, PluginManifest, PluginManager
