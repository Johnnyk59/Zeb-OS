...
path via ``zeb chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ZEB_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ZEB_HOME at a temp dir and turn moa.save_traces on."""
    zeb_home = tmp_path / ".zeb"
    zeb_home.mkdir()
    monkeypatch.setenv("ZEB_HOME", str(zeb_home))

    # save_moa_turn reads config via zeb_cli.config.load_config; stub it to
