...
def isolated_home(monkeypatch):
    """Temp ZEB_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zeb_test_31179_")
    zeb_home = os.path.join(test_home, ".zeb")
    os.makedirs(zeb_home)
...
def _fresh_modules():
    """Drop cached zeb modules so each test reloads against current env."""
