...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_zeb_env(self):
        """Zeb' own .env inside ZEB_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: zeb internal .env is in ~/.zeb/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.zeb/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.zeb/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Zeb cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zeb_home = tmp_path / ".zeb"
        cache = zeb_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zeb_home_path", return_value=zeb_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Zeb cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zeb_home = tmp_path / ".zeb"
        hub = zeb_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zeb_home(self, tmp_path):
        """The env basename guard does not depend on ZEB_HOME resolution."""
        zeb_home = tmp_path / ".zeb"
        zeb_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zeb_home = tmp_path / ".zeb"
        cache = zeb_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
