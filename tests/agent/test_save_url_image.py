...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZEB_HOME under tmp_path."""
    monkeypatch.setenv("ZEB_HOME", str(tmp_path / ".zeb"))
    (tmp_path / ".zeb").mkdir()

    # Force the constants/image cache helpers to re-read ZEB_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zeb_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zeb_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZEB_HOME — gateway cleanup
