...
def test_zeb_client_tag_includes_current_version():
    """The client tag must reflect zeb_cli.__version__ verbatim."""
    from zeb_cli import __version__
    from agent.portal_tags import zeb_client_tag

    assert zeb_client_tag() == f"client=zeb-client-v{__version__}"


def test_zeb_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import zeb_client_tag

    tag = zeb_client_tag()
    assert tag.startswith("client=zeb-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import zeb_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=zeb-agent" in tags
    assert zeb_client_tag() in tags
    assert len(tags) == 2
