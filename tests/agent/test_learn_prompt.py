...
class TestBuildLearnPrompt:
    ...
    def test_teaches_the_full_hardline_standards(self):
        ...
        # author is always the literal Zeb, never the host/OS identity (#52368).
        assert "author: always the literal value `zeb`" in std
        assert "never fill it from the host" in std
        # #2 Zeb-tool framing names the wrapped tools, not shell utilities.
