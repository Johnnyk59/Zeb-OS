...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZEB_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zeb_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".zeb"))
    prev_home = os.environ.get("ZEB_HOME")
    os.environ["ZEB_HOME"] = os.path.join(test_home, ".zeb")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zeb_"):
            del sys.modules[mod]
