...
engine plugins (e.g. zeb-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``zeb plugins`` picker."""
    from zeb_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "zeb_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "zeb_cli.plugins.get_plugin_context_engine",
