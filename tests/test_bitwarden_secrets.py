...
binary download are exercised manually by `zeb secrets bitwarden
...
def zeb_home(tmp_path, monkeypatch):
    """Point Zeb at an isolated home directory."""
    home = tmp_path / ".zeb"
    home.mkdir()
    monkeypatch.setenv("ZEB_HOME", str(home))
    # Some modules cache get_zeb_home; clear if needed.
    import zeb_constants
    if hasattr(zeb_constants, "_ZEB_HOME_CACHE"):
        zeb_constants._ZEB_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(zeb_home, monkeypatch):
...
def test_install_bws_happy_path(zeb_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(zeb_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(zeb_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from zeb_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under zeb_home/cache/."""
    home = tmp_path / ".zeb"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
