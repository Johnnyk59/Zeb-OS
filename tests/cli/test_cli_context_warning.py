...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZEB_HOME so tests don't touch real config."""
    home = tmp_path / ".zeb"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ZebCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZebCLI.__new__(ZebCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zeb-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Zeb' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Zeb' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Zeb' minimum context length."""
