...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    zeb_state = ModuleType("zeb_state")
    zeb_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "zeb_state", zeb_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Zeb"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Zeb"
