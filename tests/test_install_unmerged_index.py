...
The ``zeb update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m zeb_cli.main gateway run`` — image name
    ``pythonw``, not ``zeb.exe`` — so the ``taskkill /IM zeb.exe`` guard
    ...
    # The zeb.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM zeb.exe' in text
