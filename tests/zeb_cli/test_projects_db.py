"""Tests for the per-profile Projects store (zeb_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Zeb Agent", folders=["/tmp/zeb"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "zeb-agent"
    assert proj.name == "Zeb Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/zeb"
    assert [f.path for f in proj.folders] == ["/tmp/zeb"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "zeb-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Zeb Agent")
    pdb.create_project(conn, name="Zeb Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["zeb-agent", "zeb-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ZEB_HOME.
...
def test_db_path_under_zeb_home():
    # Resolves under ZEB_HOME (set by the autouse isolation fixture).
