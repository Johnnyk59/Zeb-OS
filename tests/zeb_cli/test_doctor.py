...
import zeb_cli.doctor as doctor
import zeb_cli.gateway as gateway_cli
from zeb_cli import doctor as doctor_mod
from zeb_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `zeb doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ZEB_HOME", zeb_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ZEB_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ZEB_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_zeb_home(self, tmp_path, provider=""):
        """Create a minimal ZEB_HOME with config.yaml."""
        home = tmp_path / ".zeb"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_zeb_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "ZEB_HOME", home)
        ...
        try:
            from zeb_cli import auth as _auth_mod
...
def test_run_doctor_accepts_zeb_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from zeb_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("zeb_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "zeb_cli.doctor", raising=False)
        from zeb_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ZEB_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"ZEB_MAX_ITERATIONS={ghost}\n")
        (zeb_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ZEB_HOME", zeb_home)
        monkeypatch.setattr(doctor_mod, "get_zeb_home", lambda: zeb_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ZEB_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ZEB_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ZEB_MAX_ITERATIONS" in out
        env_after = (zeb_home / ".env").read_text(encoding="utf-8")
        assert "ZEB_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`zeb doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Zeb install.
