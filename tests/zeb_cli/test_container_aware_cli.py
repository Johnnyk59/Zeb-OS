...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZEB_HOME with .container-mode file."""
    zeb_home = tmp_path / ".zeb"
    zeb_home.mkdir()
    monkeypatch.setenv("ZEB_HOME", str(zeb_home))
    monkeypatch.delenv("ZEB_DEV", raising=False)

    container_mode = zeb_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zeb-agent\n"
        "exec_user=zeb\n"
        "zeb_bin=/data/current-package/bin/zeb\n"
    )
    return zeb_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zeb_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zeb-agent"
    assert info["exec_user"] == "zeb"
    assert info["zeb_bin"] == "/data/current-package/bin/zeb"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zeb_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zeb_dev(container_env, monkeypatch):
    """Returns None when ZEB_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZEB_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zeb_dev_zero(container_env, monkeypatch):
    """ZEB_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZEB_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zeb_home = Path(tmpdir) / ".zeb"
        zeb_home.mkdir()
        (zeb_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zeb_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_zeb_home": lambda: zeb_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEB_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zeb-custom\n"
        "exec_user=myuser\n"
        "zeb_bin=/opt/zeb/bin/zeb\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zeb-custom"
    assert info["exec_user"] == "myuser"
    assert info["zeb_bin"] == "/opt/zeb/bin/zeb"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zeb_cli.main import _exec_in_container
