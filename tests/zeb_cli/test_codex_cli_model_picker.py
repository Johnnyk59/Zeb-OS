...
Covers:
 - Normal path (tokens already in Zeb auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Zeb
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `zeb auth openai-codex`. The old
...
@pytest.fixture()
def zeb_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Zeb auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(zeb_auth_only_env):
    """openai-codex appears when tokens are already in Zeb auth store."""
...
def test_codex_picker_uses_live_codex_catalog(zeb_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Zeb
    auth store."""
    zeb_home = tmp_path / ".zeb"
    zeb_home.mkdir()
