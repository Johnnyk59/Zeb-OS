...
Before the fix, ``zeb whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``zeb gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    zeb = home / ".zeb"
    zeb.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ZEB_HOME", str(zeb))
...
def _env_value(zeb_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from zeb_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("zeb_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``zeb whatsapp`` with an existing paired session and
