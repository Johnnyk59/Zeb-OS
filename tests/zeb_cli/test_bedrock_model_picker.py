...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ZEB_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ZEB_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZEB_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zeb_cli.providers import ZEB_OVERLAYS
        assert "bedrock" in ZEB_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from zeb_cli.providers import ZEB_OVERLAYS
        assert ZEB_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from zeb_cli.providers import ZEB_OVERLAYS
        assert ZEB_OVERLAYS["bedrock"].auth_type == "aws_sdk"
