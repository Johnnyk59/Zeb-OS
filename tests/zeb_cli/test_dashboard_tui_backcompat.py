"""Regression test: `zeb dashboard --tui` must not hard-crash.

Older Zeb desktop app shells (<= 0.15.x) spawn the backend as::

    zeb dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Zeb couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real zeb_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "zeb_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
