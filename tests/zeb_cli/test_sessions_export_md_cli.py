...
def test_sessions_export_md_rejects_stdout_target(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zeb", "sessions", "export", "--format", "md", "--session-id", "s1", "-"],
    )
...
def test_sessions_export_jsonl_requires_output_path(monkeypatch, capsys):
    ...
    monkeypatch.setattr(zeb_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(sys, "argv", ["zeb", "sessions", "export"])
...
def test_sessions_export_md_bulk_requires_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zeb", "sessions", "export", "--format", "md", str(tmp_path)],
    )
...
def test_sessions_export_jsonl_honors_filters(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zeb", "sessions", "export", "--source", "telegram", str(out)],
    )
