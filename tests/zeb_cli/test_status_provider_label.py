"""`zeb status` provider label honors config.yaml model.base_url (#3296)."""

from unittest.mock import patch

from zeb_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("zeb_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("zeb_cli.status.resolve_provider", return_value="openrouter"), \
         patch("zeb_cli.status.load_config", return_value=config), \
         patch("zeb_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("zeb_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("zeb_cli.status.resolve_provider", return_value="anthropic"):
