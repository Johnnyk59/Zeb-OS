...
hand-rolled parsers — ``zeb_cli.config.load_env``,
``zeb_cli.main._has_any_provider_configured``, and
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_zeb_home()/.env directly.
    import importlib

    import tools.skills_tool as skills_tool

    importlib.reload(skills_tool)
    with patch.object(skills_tool, "get_zeb_home", return_value=tmp_path):
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import zeb_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from ZEB_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_zeb_dotenv may have populated at import/reload time, forcing the
