...
# ── deprecated `zeb login` fails gracefully, not with argparse error ────
#
# `zeb login` is a removed command; its handler (`login_command` in
# `zeb_cli/auth.py`) prints a deprecation notice pointing at `zeb auth` /
# `zeb model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `zeb --help`.
