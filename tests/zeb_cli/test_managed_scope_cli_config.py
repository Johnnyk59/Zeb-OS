...
zeb_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `zeb config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_zeb_home = get_zeb_home()`` at import time (module
    singleton), so monkeypatching ZEB_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ZEB_HOME.)
    """
    import cli
