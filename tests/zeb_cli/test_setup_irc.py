"""Tests for IRC gateway configuration via `zeb setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Zeb install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Zeb install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#zeb")
            monkeypatch.setenv("IRC_NICKNAME", "zeb-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `zeb setup gateway` experience with IRC present."""
