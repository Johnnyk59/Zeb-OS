...
the SAME provider universe as ``zeb model`` (the CLI/TUI picker).
...
def test_every_zeb_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `zeb model` universe."""
    ...
    assert not missing, (
        "providers shown in `zeb model` but not configurable in the desktop "
