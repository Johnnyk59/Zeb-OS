"""Tests for zeb_cli configuration management."""
...
class TestGetZebHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_zeb_home()
            assert home == Path.home() / ".zeb"

    def test_env_override(self):
        with patch.dict(os.environ, {"ZEB_HOME": "/custom/path"}):
            home = get_zeb_home()
            assert home == Path("/custom/path")


class TestEnsureZebHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path)}):
            ensure_zeb_home()
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from zeb_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from zeb_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".zeb" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"ZEB_HOME": str(profile_home)}):
            ensure_zeb_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``zeb logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from zeb_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="zeb_cli.config"):
                config = load_config()
            ...
            # stands out at zeb startup before logging is configured)
            captured = capsys.readouterr()
            assert "zeb config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path)}):
            ...
            assert "zeb config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from zeb_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ZEB_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `zeb setup`) lets a
        ...
        via config.yaml; ZEB_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from zeb_cli.config import OPTIONAL_ENV_VARS
        assert "ZEB_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `zeb setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ZEB_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Zeb
    process on next startup via the dotenv → ``os.environ`` chain in
    ``zeb_cli/env_loader.py``.
    ...
    def _zeb_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ZEB_SPOTIFY_CLIENT_ID",
            "ZEB_QWEN_BASE_URL",
            "ZEB_MAX_ITERATIONS",
        ],
    )
    def test_zeb_integration_keys_still_writable(self, allowed_key):
        """``ZEB_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ZEB_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ZEB_*``) keep working. The denylist
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ZEB_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: zeb\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "zeb"
