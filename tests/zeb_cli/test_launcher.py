"""Tests for the top-level `./zeb` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zeb` should use `zeb_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zeb"
    called = []

    fake_main_module = types.ModuleType("zeb_cli.main")

    def fake_main():
        called.append("zeb_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zeb_cli.main", fake_main_module)
