...
def zeb_home(tmp_path, monkeypatch):
    ...
    # /etc/zeb on the dev/CI box can't influence the test.
    monkeypatch.setenv("ZEB_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(zeb_home, monkeypatch):
    from zeb_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(zeb_home, monkeypatch):
    from zeb_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(zeb_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_zeb_dotenv(zeb_home=str(home))
