"""Unit tests for zeb_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Zeb.
"""
...
# A representative set of real toolset names. `zeb` is deliberately absent —
# that is the corruption #38798 reported (`zeb-cli` rewritten to `zeb`).
_KNOWN = {
    "zeb-cli",
    "zeb-telegram",
    "zeb-discord",
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # zeb-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "zeb-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "zeb-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "zeb-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["zeb"], "discord": ["zeb"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_zeb_cli_valid_and_zeb_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `zeb-cli` is the real toolset and `zeb` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("zeb-cli") is True
    assert validate_toolset("zeb") is False
    warnings = validate_platform_toolsets({"cli": ["zeb"]}, validate_toolset)
    assert any("did you mean 'zeb-cli'?" in w for w in warnings)
