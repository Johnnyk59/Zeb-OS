"""Tests for `zeb memory setup [provider]` routing.
...
``zeb memory setup honcho``) without the interactive picker — which matters
because the per-provider ``zeb <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "zeb memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("zeb_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("zeb_cli.tools_config.subprocess.run", fake_run):
