...
``zeb_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``zeb --help``, ``zeb version``,
``zeb logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``zeb --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from zeb_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["zeb", "--tui", "chat"], "chat"),
        (["zeb", "-w", "logs"], "logs"),
        (["zeb", "chat", "hello world"], "chat"),
        (["zeb", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["zeb", "-m", "gpt5", "chat"], "chat"),
        (["zeb", "--model", "gpt5", "chat", "hi"], "chat"),
        (["zeb", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["zeb", "-z", "hello world"], None),
        (["zeb", "-z", "hello", "chat"], "chat"),
        (["zeb", "--model=gpt5", "chat"], "chat"),     # inline form
        (["zeb", "--", "chat"], "chat"),               # -- terminator
        (["zeb", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["zeb", "some-plugin-cmd"], "some-plugin-cmd"),
        (["zeb", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["zeb"],                          # bare → chat
        ["zeb", "--help"],                # top-level help
        ["zeb", "-h"],
        ["zeb", "version"],               # known built-in
        ["zeb", "logs"],
        ["zeb", "gateway", "run"],
        ["zeb", "--tui"],
        ["zeb", "-w", "--tui"],
        ["zeb", "chat", "hi"],
        ["zeb", "help"],                  # accepted built-in-ish
        ["zeb", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["zeb", "meet", "join"],          # potential google_meet plugin
        ["zeb", "honcho", "status"],      # potential memory plugin
        ["zeb", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``zeb help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"zeb_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
