"""Regression tests for #34107 — Docker UID/GID handling in ensure_zeb_home.

When Zeb runs in Docker with ``ZEB_UID=1000`` / ``ZEB_GID=911``,
the entrypoint chowns the top-level ``ZEB_HOME`` once at startup. But
...
The fix is a ``_chown_to_zeb_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zeb_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZebUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZEB_UID", "1000")
        monkeypatch.setenv("ZEB_GID", "911")
        from zeb_cli.config import _resolve_zeb_uid_gid
        uid, gid = _resolve_zeb_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ZEB_UID", raising=False)
        monkeypatch.delenv("ZEB_GID", raising=False)
        from zeb_cli.config import _resolve_zeb_uid_gid
        uid, gid = _resolve_zeb_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ZEB_UID", "not-a-number")
        monkeypatch.setenv("ZEB_GID", "911")
        from zeb_cli.config import _resolve_zeb_uid_gid
        uid, gid = _resolve_zeb_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ZEB_UID", "")
        monkeypatch.setenv("ZEB_GID", "")
        from zeb_cli.config import _resolve_zeb_uid_gid
        uid, gid = _resolve_zeb_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToZebUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zeb_uid(d)  # must not raise
