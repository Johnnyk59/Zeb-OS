...
Before the s6 migration, ``docker run nousresearch/zeb-agent gateway
...
crash and a supervised dashboard alongside (when ``ZEB_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``ZEB_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``ZEB_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``zeb`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'zeb.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'zeb.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → zeb gateway run) can take
    ...
    # (main-wrapper.sh → python → zeb gateway run) can take well over
    ...
    # top), NOT the static main-zeb service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-zeb
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``zeb gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "ZEB_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The ZEB_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``zeb gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``zeb gateway
    ...
    # (main-wrapper.sh → python → zeb gateway run) can take well
    ...
    # started either — so we'd see the CMD's `zeb gateway run`
    ...
    # `zeb gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*zeb.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `zeb gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-zeb`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``ZEB_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${ZEB_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Zeb Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Zeb Gateway
    ...
    assert "⚕" in combined or "Zeb Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Zeb Gateway Starting" in file_contents, (
