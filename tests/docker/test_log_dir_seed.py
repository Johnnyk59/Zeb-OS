"""Runtime smoke test for Docker $ZEB_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the zeb user after container boot.
...
registered later runs its log service as the dropped zeb user and
...
def test_logs_gateways_seeded_and_zeb_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by zeb after boot."""
    ...
    assert "logs=zeb" in r.stdout, (
        f"logs/ not owned by zeb: {r.stdout}"
    )
    assert "gateways=zeb" in r.stdout, (
        f"logs/gateways/ not owned by zeb: {r.stdout}"
    )
