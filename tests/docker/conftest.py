...
Override the image with ``ZEB_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZEB_TEST_IMAGE", "zeb-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZEB_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZEB_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged zeb user
# ---------------------------------------------------------------------------
#
# Background: every Zeb runtime path inside the container drops to UID
# 10000 (the ``zeb`` user) via ``s6-setuidgid zeb``. ``docker exec``
...
# fails (PermissionError swallowed) for zeb, so a test that ran as root
...
# helpers below run every probe under ``-u zeb`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: zeb).
