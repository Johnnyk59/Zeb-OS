"""Tests for zeb_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Zeb entry point
(zeb, zeb-agent, zeb-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Zeb entry point imports zeb_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zeb_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zeb_bootstrap", None)
    import zeb_bootstrap  # noqa: WPS433
    return zeb_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Zeb CLI
...
class TestEntryPointsImportBootstrap:
    """Every Zeb entry point must import zeb_bootstrap as its
    ...
    # Entry points that invoke Zeb as a process.  Each one must
    # import zeb_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zeb_cli/main.py",   # zeb CLI (console_script)
        "run_agent.py",          # zeb-agent (console_script)
        "acp_adapter/entry.py",  # zeb-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zeb_bootstrap' and that
        ...
        half-finished ``zeb update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zeb_bootstrap`` as a top-level module) leaves zeb
        ...
        """
        # Resolve relative to the zeb-agent repo root.  Tests live
        # at tests/test_zeb_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # zeb start even when zeb_bootstrap hasn't been
        ...
        assert first_import_name == "zeb_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zeb_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zeb_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Zeb's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ZEB_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ZEB_PYTHON_SRC_ROOT"] = env
            elif "ZEB_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ZEB_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/zeb")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZEB_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ZEB_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/zeb", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Zeb root.  The guard must relocate Zeb to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/zeb"])
        assert result[0] == "/opt/zeb"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Zeb root.
        assert result.index("/opt/zeb") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/zeb", "/opt/zeb", ""])
        assert result.count("/opt/zeb") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ZEB_PYTHON_SRC_ROOT"] = "/env/zeb"
            hb.harden_import_path()
            assert sys.path[0] == "/env/zeb"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZEB_PYTHON_SRC_ROOT", None)
