# ============================================================================
# Zeb Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://zeb-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ZebHome = $(if ($env:ZEB_HOME) { $env:ZEB_HOME } else { "$env:LOCALAPPDATA\zeb" }),
    [string]$InstallDir = $(if ($env:ZEB_HOME) { "$env:ZEB_HOME\zeb-agent" } else { "$env:LOCALAPPDATA\zeb\zeb-agent" }),
    ...
    # builds apps/desktop into a launchable Zeb.exe.
    #
    # Why opt-in:
    #   * Zeb-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Zeb.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Zeb.exe
    ...
    #     `zeb desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/zeb-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/zeb-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Zeb Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ZebHome)) {
        New-Item -ItemType Directory -Force -Path $ZebHome | Out-Null
    }
    $envFile = Join-Path $ZebHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ZebHome "node"
...
function Install-Uv {
    # Zeb owns its own uv at $ZebHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zeb_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `zeb update` stay in sync.
    $managedUv = Join-Path $ZebHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ZebHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ZebHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ZebHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ZebHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ZEB_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Zeb-Setup.exe
...
function Install-Git {
    ...
    which Zeb uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\zeb\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Zeb needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Zeb copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\zeb\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZEB_GIT_BASH_PATH`` (User scope) so Zeb can find it in a fresh
    ...
    # Download PortableGit into $ZebHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ZebHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Zeb features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Zeb needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZEB_GIT_BASH_PATH`` (User env scope) so Zeb can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZebHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZebHome\git\usr\bin\bash.exe
    $candidates += "$ZebHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZebHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZEB_GIT_BASH_PATH", $candidate, "User")
            $env:ZEB_GIT_BASH_PATH = $candidate
            Write-Info "Set ZEB_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Zeb may not find Git Bash."
    Write-Info "If needed, set ZEB_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$ZebHome\node;$env:Path"
        Write-Success "Node.js $version found (Zeb-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Zeb-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ZebHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ZebHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zeb-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ZebHome\node") { Remove-Item -Recurse -Force "$ZebHome\node" }
                Move-Item $extractedDir.FullName "$ZebHome\node"
                ...
                $version = & "$ZebHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ZebHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\zeb-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `zeb
                    # update` path (#4735).
                    ...
                    $stashName = "zeb-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``zeb update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Zeb behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running zeb processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/zeb-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/zeb-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/zeb-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\zeb-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\zeb-agent-extract"
    ...
    # next `zeb update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Zeb-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running zeb process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running zeb processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Zeb_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (zeb.exe) plus its child tree.
            & taskkill /F /T /IM zeb.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM zeb.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m zeb_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not zeb.exe.
            ...
            # user (see zeb_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `zeb update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `zeb.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install zeb-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and zeb.exe broken with
    ...
    # here, not 30 seconds later when the user runs `zeb`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register zeb.exe in dist-info/RECORD but fail to
        ...
        # `zeb` command while zeb-agent.exe / zeb-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m zeb_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `zeb dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\zeb_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `zeb dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `zeb dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: zeb_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up zeb command..."
    
    if ($NoVenv) {
        $zebBin = "$InstallDir"
    } else {
        $zebBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so zeb is globally available
    # On Windows, the zeb.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$zebBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zebBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zebBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZEB_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zeb instead
    # of the Unix default ~/.zeb
    $currentZebHome = [Environment]::GetEnvironmentVariable("ZEB_HOME", "User")
    if (-not $currentZebHome -or $currentZebHome -ne $ZebHome) {
        [Environment]::SetEnvironmentVariable("ZEB_HOME", $ZebHome, "User")
        Write-Success "Set ZEB_HOME=$ZebHome"
    }
    $env:ZEB_HOME = $ZebHome
    
    # Update current session
    $env:Path = "$zebBin;$env:Path"
    
    Write-Success "zeb command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.zeb-bootstrap-complete which tells the Zeb
    ...
    $markerPath = Join-Path $InstallDir ".zeb-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ZEB_HOME directory structure ($ZebHome, default %LOCALAPPDATA%\zeb)
    New-Item -ItemType Directory -Force -Path "$ZebHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZebHome\skills" | Out-Null
    ...
    # Create config.yaml
    $configPath = "$ZebHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Zeb's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ZebHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in zeb_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ZebHome"
    
    # Seed bundled skills into $ZebHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ZebHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $ZebHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZebHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ZebHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Zeb-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'zeb setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\zeb-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\zeb-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\zeb-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Zeb` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Zeb.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_zeb_desktop command
    # resolves apps/desktop/release/win-unpacked/Zeb.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Zeb-managed
    ...
    # The --dir mode produces an unpacked Zeb.exe in
    ...
    # are NOT trying to work around). The Zeb icon + product name are
    # stamped onto Zeb.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\zeb-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Zeb.exe",
        "$desktopDir\release\win-arm64-unpacked\Zeb.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Zeb.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Zeb icon + identity are stamped onto Zeb.exe by the
    ...
    #    Zeb.exe. We deliberately do NOT point them at `zeb desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Zeb.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Zeb.lnk')
        )
        ...
        # with the Zeb icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.zeb/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'zeb setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zeb_cli.main setup
    } else {
        python -m zeb_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $zebCmd = "$InstallDir\venv\Scripts\zeb.exe"
    if (-not (Test-Path $zebCmd)) {
        $zebCmd = "zeb"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZebHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zeb whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: zeb gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZebHome\logs\gateway.log"
            Start-Process -FilePath $zebCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZebHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zeb gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zeb gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZebHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZebHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZebHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZebHome\zeb-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zeb              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zeb setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zeb config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zeb config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zeb gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zeb update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Zeb repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Zeb-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Zeb to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://zeb-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
