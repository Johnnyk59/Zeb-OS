#!/usr/bin/env python3
"""Live test harness for Zeb Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any zeb imports.
ORIGINAL_HOME = os.environ.get("ZEB_HOME")
ORIGINAL_AUTH = Path.home() / ".zeb" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.zeb/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.zeb/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zeb_ts_live_"))
    zeb_home = home_dir / ".zeb"
    zeb_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zeb_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".zeb" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zeb_home / ".env")
        ...
        load_zeb_dotenv(zeb_home=str(Path.home() / ".zeb"))
    ...
    (zeb_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zeb_home
...
def reset_module_state():
    """Drop cached modules so the new ZEB_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zeb_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ZEB_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZEB_HOME"] = ORIGINAL_HOME
    else:
