#!/usr/bin/env python3
"""Drive the Zeb TUI under ZEB_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.zeb/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ZEB_TUI_RESUME set so it
bypasses the zeb_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ZEB_PERF_LOG     (default ~/.zeb/perf.log)
  ZEB_PERF_NODE    (default node from $PATH)
  ZEB_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ZEB_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("ZEB_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("ZEB_PERF_LOG", str(get_zeb_home() / "perf.log")))
DEFAULT_STATE_DB = get_zeb_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — ZEB_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["ZEB_DEV_PERF"] = "1"
    env["ZEB_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ZEB_DEV_PERF_LOG"] = str(log)
    env["ZEB_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ZEB_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ZEB_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "zeb-ink" / "src"
