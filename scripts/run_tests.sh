#!/usr/bin/env bash
# Canonical test runner for zeb-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.zeb/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.zeb/zeb-agent/venv"; do
...
if [ -f "$HOME/.zeb/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.zeb"
...
exec env -i \
  ...
  ${ZEB_RUN_SLOW_PET_TESTS:+ZEB_RUN_SLOW_PET_TESTS="$ZEB_RUN_SLOW_PET_TESTS"} \
