...
# Python-driven tool session (e.g. Zeb terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/zeb-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/zeb-agent.git"
ZEB_HOME="${ZEB_HOME:-$HOME/.zeb}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ZEB_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ZEB_INSTALL_DIR"
...
#   code at /usr/local/lib/zeb-agent, command at /usr/local/bin/zeb,
#   data still at /root/.zeb (ZEB_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ZEB_HOME/.no-bundled-skills so future"
            echo "                   'zeb update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Zeb.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.zeb/zeb-agent"
            echo "                   default (root, Linux): /usr/local/lib/zeb-agent"
            echo "  --zeb-home PATH  Data directory (default: ~/.zeb, or \$ZEB_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Zeb installs the code under"
            echo "  /usr/local/lib/zeb-agent and links the command into"
            echo "  /usr/local/bin/zeb (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ZEB_HOME"
            echo "  (default /root/.zeb).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ZEB_HOME/zeb-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs zeb setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `zeb update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Zeb-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Zeb Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install zeb command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `zeb` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ZEB_HOME/zeb-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ZEB_HOME/zeb-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/zeb-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ZEB_HOME/zeb-agent — then preserve it)
#
# Always no-op when the user set --dir or $ZEB_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ZEB_HOME.
    if is_termux; then
        INSTALL_DIR="$ZEB_HOME/zeb-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ZEB_HOME/zeb-agent/.git" ]; then
            INSTALL_DIR="$ZEB_HOME/zeb-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/zeb-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/zeb-agent"
        ...
        # /usr/local/bin/zeb wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/zeb"
        log_info "  Data:    $ZEB_HOME (unchanged)"
...
# Point a Zeb-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ZEB_HOME/node/bin — which is NOT on
...
# Zeb's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Zeb-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ZEB_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ZEB_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ZEB_HOME/node/etc/npmrc"
}

get_zeb_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/zeb" ]; then
        echo "$link_dir/zeb"
    else
        echo "zeb"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://zeb-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Zeb owns its own uv at $ZEB_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zeb_cli/managed_uv.py) looks in the same
    # place, so install.sh and `zeb update` stay in sync.
    local _managed_uv="$ZEB_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ZEB_HOME/bin ..."
    mkdir -p "$ZEB_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/zeb-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/zeb-uv-installer.$$.sh")"
    ...
    # directly into $ZEB_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ZEB_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Zeb-
...
check_node() {
    ...
    # Repair pre-existing Zeb-managed installs where `npm install -g` lands
    ...
    # Prefer a Zeb-managed Node from a previous run over a too-old system one.
    if [ -x "$ZEB_HOME/node/bin/node" ] && node_satisfies_build "$("$ZEB_HOME/node/bin/node" --version)"; then
        export PATH="$ZEB_HOME/node/bin:$PATH"
        log_success "Node.js $("$ZEB_HOME/node/bin/node" --version) found (Zeb-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Zeb-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.zeb/node/..."
    ...
    # Place into ~/.zeb/node/ and symlink binaries into the same bin dir
    # the zeb command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ZEB_HOME/node"
    mkdir -p "$ZEB_HOME"
    mv "$extracted_dir" "$ZEB_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ZEB_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ZEB_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ZEB_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ZEB_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.zeb/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Zeb install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Zeb Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `zeb update` path
                # (#4735).
                ...
                local stash_name
                stash_name="zeb-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``zeb update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Zeb behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up zeb command..."

    if [ "$USE_VENV" = true ]; then
        ZEB_BIN="$INSTALL_DIR/venv/bin/zeb"
    else
        ZEB_BIN="$(which zeb 2>/dev/null || echo "")"
        if [ -z "$ZEB_BIN" ]; then
            log_warn "zeb not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ZEB_BIN" ]; then
        log_warn "zeb entry point not found at $ZEB_BIN"
    ...
    # Create a user-facing shim for the zeb command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ZEB_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ZEB_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/zeb"
    cat > "$command_link_dir/zeb" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ZEB_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/zeb"
    log_success "Installed zeb launcher → $command_link_display_dir/zeb"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "zeb command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v zeb' \
                >/dev/null 2>&1; then
        ...
        log_info "zeb not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Zeb Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Zeb Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Zeb Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so zeb works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.zeb directory structure (config at top level, code in subdir)
    mkdir -p "$ZEB_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.zeb/.env (top level, easy to find)
    if [ ! -f "$ZEB_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ZEB_HOME/.env"
            log_success "Created ~/.zeb/.env from template"
        else
            touch "$ZEB_HOME/.env"
            log_success "Created ~/.zeb/.env"
        fi
    else
        log_info "~/.zeb/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ZEB_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.zeb/config.yaml (top level, easy to find)
    if [ ! -f "$ZEB_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ZEB_HOME/config.yaml"
            log_success "Created ~/.zeb/config.yaml from template"
        fi
    else
        log_info "~/.zeb/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in zeb_cli/default_soul.py — the
    ...
    if [ ! -f "$ZEB_HOME/SOUL.md" ]; then
        cat > "$ZEB_HOME/SOUL.md" << 'SOUL_EOF'
You are Zeb Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.zeb/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.zeb/"

    # Seed bundled skills into ~/.zeb/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `zeb update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'zeb update'." \
            > "$ZEB_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ZEB_HOME/.no-bundled-skills"
        log_info "  Future 'zeb update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.zeb/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.zeb/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ZEB_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ZEB_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.zeb/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Zeb Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Zeb will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Zeb Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (zeb --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `zeb update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'zeb setup' after install."
        return 0
    fi
    ...
    # Run zeb setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m zeb_cli.main setup < /dev/tty
    else
        python -m zeb_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ZEB_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Zeb to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ZEB_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'zeb whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ZEB_CMD="$(get_zeb_command_path)"
                $ZEB_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'zeb whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'zeb gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ZEB_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ZEB_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: zeb gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: zeb gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ZEB_CMD gateway > "$ZEB_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.zeb/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: zeb gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: zeb gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ZEB_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ZEB_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ZEB_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}zeb${NC}              Start chatting"
    echo -e "   ${GREEN}zeb setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}zeb config${NC}       View/edit configuration"
    echo -e "   ${GREEN}zeb config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}zeb gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}zeb update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'zeb' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'zeb' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'zeb' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ZEB_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ZEB_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ZEB_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ZEB_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ZEB_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Zeb for pip install"
    ...
    ZEB_CMD="$(command -v zeb 2>/dev/null || echo "")"
    if [ -n "$ZEB_CMD" ]; then
        log_info "Running zeb setup..."
        "$ZEB_CMD" setup
    else
        log_warn "zeb command not found on PATH"
        log_info "Try: python -m zeb_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Zeb` rename
...
# _purge_electron_build_cache() used by `zeb desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Zeb-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `zeb desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Zeb" ]; then
            app="$desktop_dir/release/linux-unpacked/Zeb"
        elif [ -x "$desktop_dir/release/linux-unpacked/zeb" ]; then
            app="$desktop_dir/release/linux-unpacked/zeb"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Zeb.app" \
            "$desktop_dir/release/mac/Zeb.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Zeb is damaged and can't be opened").
    ...
    # checkout stays clean for the next `zeb update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Zeb-managed Node
            # provisioned during prerequisites/node-deps (at $ZEB_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ZEB_HOME.
    # $ZEB_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'zeb update' on this host install.
