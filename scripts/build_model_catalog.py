#!/usr/bin/env python3
"""Build the Zeb Model Catalog — a centralized JSON manifest of curated models.
...
Zeb CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Zeb release.
...
# Ensure ZEB_HOME is set for imports that touch it at module level.
os.environ.setdefault("ZEB_HOME", os.path.join(os.path.expanduser("~"), ".zeb"))

from zeb_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "zeb-agent repo",
            "docs": "https://zeb-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
