# Zeb Agent Has Had "Routines" Since March
...
Zeb equivalent — works today:
```bash
zeb webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Zeb implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Zeb Agent |
|---|---|---|
...
## Things Zeb Does That Routines Can't
...
Zeb has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Zeb Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
zeb gateway setup    # enable webhooks
zeb webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [zeb-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://zeb-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [zeb-agent.nousresearch.com](https://zeb-agent.nousresearch.com)

GitHub: [github.com/NousResearch/zeb-agent](https://github.com/NousResearch/zeb-agent)

---

*Zeb Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
