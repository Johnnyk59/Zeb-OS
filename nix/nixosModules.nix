# nix/nixosModules.nix — NixOS module for zeb-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: zeb runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZEB_HOME/.env
# and read by zeb at startup — no container recreation needed for env changes.
#
# Tool resolution: the zeb wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    zeb-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-zeb-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "zeb-config-attrs";
      description = "Zeb YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "zeb-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ZEB_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "zeb-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ZEB_DOC_EOF'\n${value}\nZEB_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "zeb-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/zeb";
    ...
    # zeb user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "zeb-container-entrypoint" ''
      set -eu

      ZEB_UID="''${ZEB_UID:?ZEB_UID must be set}"
      ZEB_GID="''${ZEB_GID:?ZEB_GID must be set}"

      # ── Group: ensure a group with GID=$ZEB_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ZEB_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="zeb"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ZEB_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ZEB_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ZEB_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ZEB_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="zeb"
        TARGET_HOME="/home/zeb"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ZEB_UID" -g "$ZEB_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ZEB_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ZEB_UID:$ZEB_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ZEB_HOME is owned by the target user.
      ...
      if [ -n "''${ZEB_HOME:-}" ] && [ -d "$ZEB_HOME" ]; then
        find "$ZEB_HOME" \! -user "$ZEB_UID" -exec chown "$ZEB_UID:$ZEB_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/zeb-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/zeb-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zeb ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zeb
        chmod 0440 /etc/sudoers.d/zeb
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ZEB_UID" --regid="$ZEB_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ZEB_HOME/.env.
    ...
    # Default: /var/lib/zeb/workspace → /data/workspace.
  ...
  in {
    options.services.zeb-agent = with lib; {
      enable = mkEnableOption "Zeb Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = zeb-agent;
        description = "The zeb-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/zeb";
        description = "State directory. Contains .zeb/ subdir (ZEB_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Zeb config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ZEB_HOME/.env at activation time.
          Zeb reads this file on every startup via load_zeb_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ZEB_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ZEB_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zeb gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the zeb user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zeb plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Zeb discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zeb_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zeb (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in zeb-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the zeb CLI to environment.systemPackages and export
          ZEB_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.zeb symlink to the service
            stateDir. These users are automatically added to the zeb group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.zeb-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the zeb CLI to system PATH and export ZEB_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.zeb/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ZEB_HOME = "${cfg.stateDir}/.zeb";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.zeb-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the zeb user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.zeb-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed zeb
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure zeb is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.zeb        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zeb/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zeb/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zeb/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zeb/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zeb/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."zeb-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.zeb
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.zeb ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.zeb ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.zeb -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.zeb/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.zeb/$_subdir"
            chmod 2770 "${cfg.stateDir}/.zeb/$_subdir"
            find "${cfg.stateDir}/.zeb/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # zeb-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.zeb/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.zeb/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zeb/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.zeb/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.zeb/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zeb/.managed
          chmod 0644 ${cfg.stateDir}/.zeb/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.zeb/.container-mode <<'ZEB_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    zeb_bin=${containerDataDir}/current-package/bin/zeb
    ZEB_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zeb/.container-mode
            chmod 0644 ${cfg.stateDir}/.zeb/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.zeb/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zeb";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.zeb" ]; then
                  rm -f "${symlinkPath}"
                  echo "zeb-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.zeb -> stateDir/.zeb for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zeb";
                target = "${cfg.stateDir}/.zeb";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "zeb-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zeb/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.zeb/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zeb/auth.json
              fi
            ''}
          ''}
          ...
          # Zeb reads $ZEB_HOME/.env at startup via load_zeb_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.zeb/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ZEB_NIX_ENV_EOF'
    ${envFileContent}
    ZEB_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.zeb/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.zeb/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zeb/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.zeb-agent = {
          description = "Zeb Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ZEB_HOME/.env by the activation script. load_zeb_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/zeb"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the zeb group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.zeb-agent = {
          description = "Zeb Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ZEB_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ZEB_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ZEB_UID="$ZEB_UID" \
                --env ZEB_GID="$ZEB_GID" \
                --env ZEB_HOME=${containerDataDir}/.zeb \
                --env ZEB_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/zeb gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
