...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      zeb-agent = self'.packages.default;
      zebVenv = zeb-agent.zebVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "zeb-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${zebVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zeb-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "zeb-build-package" { } ''
          echo "PASS: package built at ${zeb-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "zeb-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "zeb-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${zeb-agent}/bin/zeb || (echo "FAIL: zeb binary missing"; exit 1)
          test -x ${zeb-agent}/bin/zeb-agent || (echo "FAIL: zeb-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${zeb-agent}/bin/zeb version 2>&1 | grep -qi "zeb" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zeb-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zeb zeb-agent zeb-acp; do
            test -x ${zeb-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zeb-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zeb --help ==="
          ${zeb-agent}/bin/zeb --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${zeb-agent}/bin/zeb --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zeb-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${zeb-agent}/share/zeb-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${zeb-agent}/share/zeb-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZEB_BUNDLED_SKILLS" ${zeb-agent}/bin/zeb || \
            (echo "FAIL: ZEB_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZEB_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "zeb-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${zeb-agent}/share/zeb-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${zeb-agent}/share/zeb-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZEB_BUNDLED_PLUGINS" ${zeb-agent}/bin/zeb || \
            (echo "FAIL: ZEB_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZEB_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "zeb-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${zeb-agent}/share/zeb-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${zeb-agent}/share/zeb-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${zeb-agent}/share/zeb-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ZEB_BUNDLED_LOCALES" ${zeb-agent}/bin/zeb || \
            (echo "FAIL: ZEB_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ZEB_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ZEB_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ZEB_BUNDLED_LOCALES=${zeb-agent}/share/zeb-agent/locales \
            ${zebVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ZEB_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${zebVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${zebVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ZEB_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "zeb-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${zeb-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${zeb-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZEB_TUI_DIR" ${zeb-agent}/bin/zeb || \
            (echo "FAIL: ZEB_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZEB_TUI_DIR set in wrapper"
        ...
        # Verify ZEB_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        zeb-node = pkgs.runCommand "zeb-node-version" { } ''
          set -e
          echo "=== Checking ZEB_NODE in wrapper ==="
          grep -q "ZEB_NODE" ${zeb-agent}/bin/zeb || \
            (echo "FAIL: ZEB_NODE not set in wrapper"; exit 1)
          echo "PASS: ZEB_NODE present in wrapper"

          ZEB_NODE=$(sed -n "s/^export ZEB_NODE='\(.*\)'/\1/p" ${zeb-agent}/bin/zeb)
          test -x "$ZEB_NODE" || (echo "FAIL: ZEB_NODE=$ZEB_NODE not executable"; exit 1)
          echo "PASS: ZEB_NODE executable at $ZEB_NODE"

          NODE_MAJOR=$("$ZEB_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ZEB_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZEB_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zeb-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZEB_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZEB_MANAGED guards ==="
          check_blocked "config set" ${zeb-agent}/bin/zeb config set model foo
          check_blocked "config edit" ${zeb-agent}/bin/zeb config edit
        ...
        extra-python-packages = let
          ...
          zebWithExtra = zeb-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zeb-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zebWithExtra}/bin/zeb || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zebWithExtra}/bin/zeb || \
          ...
          if grep -q "PYTHONPATH" ${zeb-agent}/bin/zeb; then
        ...
        extra-dependency-groups = let
          zebWithGroups = zeb-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "zeb-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${zebWithGroups}"
          echo "venv: ${zebWithGroups.zebVenv}"
        ...
        messaging-variant = pkgs.runCommand "zeb-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.zebVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "zeb-config-roundtrip" {
          ...
          merge_and_load() {
            local zeb_home="$1"
            export ZEB_HOME="$zeb_home"
            ${configMergeScript} ${nixSettings} "$zeb_home/config.yaml"
            ${zebVenv}/bin/python3 -c '
import json, sys
