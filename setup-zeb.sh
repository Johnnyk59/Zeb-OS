...
# 5. Symlinks the 'zeb' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Zeb Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zeb-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink zeb into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zeb command..."

ZEB_BIN="$SCRIPT_DIR/venv/bin/zeb"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZEB_BIN" "$COMMAND_LINK_DIR/zeb"
echo -e "${GREEN}✓${NC} Symlinked zeb → $COMMAND_LINK_DISPLAY_DIR/zeb"
...
# ============================================================================
# Seed bundled skills into ~/.zeb/skills/
# ============================================================================

ZEB_SKILLS_DIR="${ZEB_HOME:-$HOME/.zeb}/skills"
mkdir -p "$ZEB_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zeb/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZEB_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zeb status        # Check configuration"
if is_termux; then
    echo "  zeb gateway       # Run gateway in foreground"
else
    echo "  zeb gateway install # Install gateway service (messaging + cron)"
fi
echo "  zeb cron list     # View scheduled jobs"
echo "  zeb doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zeb_cli.main setup
fi
