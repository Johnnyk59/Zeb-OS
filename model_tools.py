...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_zeb_home() under the active profile.
...
#   - cli.py, zeb_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("ZEB_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ZEB_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ZEB_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like zeb-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("zeb-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (zeb-*) include _ZEB_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("zeb-")
    ...
    # window). Core Zeb tools (toolsets._ZEB_CORE_TOOLS) are NEVER
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to zeb-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from zeb_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from zeb_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from zeb_cli.middleware import run_tool_execution_middleware
