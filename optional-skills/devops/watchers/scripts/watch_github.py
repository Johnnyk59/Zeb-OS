...
Usage (via cron with --no-agent):
    ...
    zeb cron create zeb-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZEB_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name zeb-issues --repo NousResearch/zeb-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Zeb .env file
(``${ZEB_HOME:-~/.zeb}/.env``) to avoid the 60 req/hr
