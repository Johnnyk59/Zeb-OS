...
## 5. GODMODE FAST — Zeb 4 405B (Zero Refusal)

**Model:** `nousresearch/zeb-4-405b`
**Strategy:** Zeb models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Zeb Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.zeb/config.yaml`:
