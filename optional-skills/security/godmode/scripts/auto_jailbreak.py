...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZEB_HOME", os.path.expanduser("~/.zeb")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZEB_HOME", Path.home() / ".zeb")) / "skills" / "red-teaming" / "godmode"
...
ZEB_HOME = Path(os.getenv("ZEB_HOME", Path.home() / ".zeb"))
CONFIG_PATH = ZEB_HOME / "config.yaml"
PREFILL_PATH = ZEB_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Zeb models — already uncensored, just needs clean prompt
    "zeb": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zeb" in model_lower or "nous" in model_lower:
        return "zeb"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zeb config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zeb/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zeb for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zeb for changes to take effect.")
