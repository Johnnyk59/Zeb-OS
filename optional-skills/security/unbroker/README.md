...
The design is **Zeb-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
zeb skills install official/security/unbroker
```

Then start a new Zeb session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Zeb home directory (`0600`, optionally
