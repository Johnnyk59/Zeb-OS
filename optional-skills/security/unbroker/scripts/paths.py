...
All per-subject data lives under PDD_DATA_DIR (default: $ZEB_HOME/unbroker),
which is the same trust boundary Zeb uses for .env and OAuth tokens.
"""
...
def zeb_home() -> Path:
    return Path(os.environ.get("ZEB_HOME") or (Path.home() / ".zeb"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else zeb_home() / "unbroker"
