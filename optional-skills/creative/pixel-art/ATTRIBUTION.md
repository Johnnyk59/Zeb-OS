...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Zeb uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Zeb has `image_generate`
...
- License: MIT (inherits from zeb-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Zeb Agent contributors
