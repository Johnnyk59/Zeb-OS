...
the actual `setup.sh` that creates the project workspace, configures Zeb
...
2. **Create profiles** — `zeb profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.zeb/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `zeb kanban create` assigned to the director
...
```bash
zeb profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.zeb/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.zeb/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Zeb
...
Each profile gets a `SOUL.md` at `~/.zeb/profiles/<name>/SOUL.md` that
...
```bash
zeb kanban create "Direct production of <video title>" \
    --assignee director \
...
the Zeb `.env` (`${ZEB_HOME:-$HOME/.zeb}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _zeb_env="${ZEB_HOME:-$HOME/.zeb}/.env"
    if grep -q "^${var}=" "$_zeb_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_zeb_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_zeb_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY zeb ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY zeb OPENROUTER_API_KEY || exit 1
# ...
```
