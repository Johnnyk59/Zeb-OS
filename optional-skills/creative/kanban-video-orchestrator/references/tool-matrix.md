...
Maps each role archetype to the Zeb skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zeb-agent
...
## Zeb skills relevant to video production

### Visual / rendering skills (`zeb-agent/skills/creative/`)
...
### Design / pre-production skills (`zeb-agent/skills/creative/`)
...
### Audio / media skills (`zeb-agent/skills/creative/` + `skills/media/`)
...
These are **not** Zeb skills but external CLIs / APIs that profiles invoke.
...
## Built-in Zeb tools for media review

These are native Zeb tools — not invoked via terminal but through their own
...
key is present in `${ZEB_HOME:-~/.zeb}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Zeb profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ZEB_HOME:-~/.zeb}/.env` → environment variable.
...
When a new Zeb-public video skill ships:
