...
zeb kanban watch --tenant <project-slug>

# Snapshot of the board
zeb kanban list --tenant <project-slug>
zeb kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zeb kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zeb dashboard

# Inspect a specific task (includes comments + events)
zeb kanban show <task-id>

# Follow a single task's event stream
zeb kanban tail <task-id>
```

Verify available subcommands with `zeb kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zeb kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zeb kanban block <id>` then `zeb kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zeb kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zeb kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zeb profile list` (profile exists?), `zeb status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(zeb kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zeb kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zeb kanban block <task-id>
zeb kanban archive <task-id>

# Diagnose what happened
zeb kanban show <task-id>      # task body, comments, recent events
zeb kanban tail <task-id>      # follow the live event stream
zeb kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zeb kanban create` for the director
...
while true; do
    clear
    zeb kanban list --tenant <slug>
    echo "---"
    zeb kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zeb kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zeb profile list` and start
