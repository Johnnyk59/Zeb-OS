#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `zeb kanban list` and
...
def zeb_available() -> bool:
    return shutil.which("zeb") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed zeb CLI."""
    try:
        out = subprocess.run(
            ["zeb", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `zeb kanban list`
    out = subprocess.run(
        ["zeb", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not zeb_available():
        print("ERROR: 'zeb' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
