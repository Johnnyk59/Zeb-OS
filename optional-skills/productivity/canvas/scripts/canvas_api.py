#!/usr/bin/env python3
"""Canvas LMS API CLI for Zeb Agent.
...
def _check_config():
    ...
    if missing:
        zeb_env = os.path.join(
            os.environ.get("ZEB_HOME", os.path.expanduser("~/.zeb")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zeb_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Zeb Agent"
    )
