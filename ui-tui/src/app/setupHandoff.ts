import type { RunExternalProcess } from '@zeb/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zeb ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zeb: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zeb ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
