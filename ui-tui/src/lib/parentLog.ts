...
const logDir = join(process.env.ZEB_HOME?.trim() || join(homedir(), '.zeb'), 'logs')
...
// a real ~/.zeb (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('zeb-tui: parent lifecycle log unavailable\n')
    }
  }
}
