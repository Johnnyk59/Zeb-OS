...
const ENV_KEYS = ['ZEB_AUTO_HEAPDUMP', 'ZEB_HEAPDUMP_DIR', 'ZEB_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'zeb-heapdump-test-'))
    process.env.ZEB_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without ZEB_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without ZEB_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when ZEB_AUTO_HEAPDUMP=1', async () => {
    process.env.ZEB_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.ZEB_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of ZEB_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.ZEB_HEAPDUMP_DIR
    savedMax = process.env.ZEB_HEAPDUMP_MAX_BYTES
    delete process.env.ZEB_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'zeb-heapdump-prune-'))
    process.env.ZEB_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.ZEB_HEAPDUMP_DIR
    } else {
      process.env.ZEB_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.ZEB_HEAPDUMP_MAX_BYTES
    } else {
      process.env.ZEB_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.ZEB_HEAPDUMP_MAX_BYTES = String(2 * 1024)
