...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZEB_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Zeb on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZEB_TUI_TRUECOLOR=1.
