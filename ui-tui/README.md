...
React + Ink terminal UI for Zeb. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZEB_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `zeb setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.zeb/.zeb_history` or under `ZEB_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `zeb setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/zeb-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external zeb setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.zeb/tui-parent.log
    ...
    types/
      zeb-ink.d.ts            type declarations for @zeb/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZebCLI subprocess for slash commands
```