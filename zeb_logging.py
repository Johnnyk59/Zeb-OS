"""Centralized logging setup for Zeb Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.zeb/logs/`` (profile-aware via ``get_zeb_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``zeb_cli.web_server``,
    ``zeb_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Zeb (TUI, gateway, ``hy_memory`` server, MCP
...
from zeb_constants import get_config_path, get_zeb_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zeb_session_injector", False):
    ...
    _session_record_factory._zeb_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``zeb logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``zeb logs --component gateway``.
    "gateway": ("gateway", "zeb_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Zeb logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    zeb_home
        Override for the Zeb home directory.  Falls back to
        ``get_zeb_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zeb_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zeb_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the zeb group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from zeb_cli.config import is_managed
...
# the module header): when several Zeb processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._zeb_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="zeb-log-drain", daemon=True)
    t.start()
    t.join(timeout)
