...
#
# Wired into the image as /etc/cont-init.d/01-zeb-setup by the
...
ZEB_HOME="${ZEB_HOME:-/opt/data}"
INSTALL_DIR="/opt/zeb"

# Drop to zeb via s6-setuidgid, but skip it when already non-root.
as_zeb() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid zeb "$@"; }
...
# starts non-root. The baked install tree under /opt/zeb is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZEB_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZEB_UID/ZEB_GID — or the PUID/PGID aliases — which the
...
# `id -u zeb` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# zeb UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zeb)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-zeb UID).
...
and the baked /opt/zeb install tree is intentionally root-owned and
...
# --- Bootstrap ZEB_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid zeb mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZEB_HOME paths whose parents only
# root can create (e.g. `ZEB_HOME=/home/zeb/.zeb` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZEB_UID/ZEB_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZEB_UID/
# ZEB_GID still win when both are set.  See #15290, salvages #25872.
ZEB_UID="${ZEB_UID:-${PUID:-}}"
ZEB_GID="${ZEB_GID:-${PGID:-}}"

if [ -n "${ZEB_UID:-}" ] && validate_uid_gid "$ZEB_UID" && [ "$ZEB_UID" != "$(id -u zeb)" ]; then
    echo "[stage2] Changing zeb UID to $ZEB_UID"
    usermod -u "$ZEB_UID" zeb
fi
if [ -n "${ZEB_GID:-}" ] && validate_uid_gid "$ZEB_GID" && [ "$ZEB_GID" != "$(id -g zeb)" ]; then
    echo "[stage2] Changing zeb GID to $ZEB_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZEB_GID" zeb 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised zeb user (UID 10000)
...
# zeb process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes zeb. Idempotent across container
...
#   - zeb is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G zeb 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] zeb already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" zeb 2>/dev/null; then
        echo "[stage2] Added zeb to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group zeb failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZEB_UID is remapped or the top-level $ZEB_HOME isn't owned by
# the runtime zeb UID, restore ownership to zeb — but ONLY for the
# directories zeb actually writes to. The full $ZEB_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of zeb-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zeb_uid=$(id -u zeb)
...
chown_zeb_tree() {
    ...
    chown -R zeb:zeb "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$ZEB_HOME" 2>/dev/null)" != "$actual_zeb_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZEB_HOME (targeted) to zeb ($actual_zeb_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZEB_HOME: chown the directory itself (not its contents)
    # so zeb can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ZEB_HOME"; then
        :
    else
        chown zeb:zeb "$ZEB_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ZEB_HOME failed (rootless container?) — continuing"
    fi
    # Zeb-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by zeb (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ZEB_HOME/$sub" ]; then
            chown_zeb_tree "$ZEB_HOME/$sub"
        fi
    done
fi
...
# zeb user. Hosted/container instances keep mutable state under
# $ZEB_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZEB_DISABLE_LAZY_INSTALLS=1. Keeping /opt/zeb root-owned and
...
# $ZEB_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ZEB_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to zeb
...
# Always reset ownership of $ZEB_HOME/profiles to zeb on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> zeb …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as zeb and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ZEB_HOME/profiles" ]; then
    chown_zeb_tree "$ZEB_HOME/profiles"
fi

# Always reset ownership of $ZEB_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged zeb runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ZEB_HOME/cron" ]; then
    chown_zeb_tree "$ZEB_HOME/cron"
fi
...
# zeb pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged zeb gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ZEB_HOME is
...
# is negligible.
if [ -d "$ZEB_HOME/platforms/pairing" ]; then
    chown_zeb_tree "$ZEB_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ZEB_HOME/pairing" ]; then
    chown_zeb_tree "$ZEB_HOME/pairing"
fi

# Reset ownership of zeb-owned top-level state files on every boot.
# The targeted data-volume chown above only covers zeb-owned
# *subdirectories*; loose state files living directly under $ZEB_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> zeb …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged zeb runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZEB_HOME are never
...
# entries of zeb_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZEB_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ZEB_HOME/$f"; then
            :
        else
            chown zeb:zeb "$ZEB_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the zeb runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZEB_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZEB_HOME/config.yaml"; then
        :
    else
        chown zeb:zeb "$ZEB_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ZEB_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as zeb user ---
# Run as zeb via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ZEB_HOME values
...
as_zeb mkdir -p \
    ...
    "$ZEB_HOME/home" \
    "$ZEB_HOME/pairing" \
    "$ZEB_HOME/platforms/pairing" \
    "$ZEB_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/zeb/.install_method (see Dockerfile), NOT written here into
# $ZEB_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZEB_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.zeb:/opt/data) and sometimes shared with a
...
# 'zeb update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZEB_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/zeb stamp, so deleting the data-dir copy is safe).
if [ -f "$ZEB_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZEB_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZEB_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZEB_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ZEB_HOME/$dest"; then
            :
        else
            as_zeb cp "$INSTALL_DIR/$src" "$ZEB_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ZEB_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZEB_HOME/.env"; then
        :
    else
        chown zeb:zeb "$ZEB_HOME/.env" 2>/dev/null || true
        chmod 600 "$ZEB_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ZEB_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `zeb update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZEB_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZEB_HOME/config.yaml" ]; then
    s6-setuidgid zeb "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZEB_HOME/auth.json" ] && [ -n "${ZEB_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ZEB_HOME/auth.json"; then
        :
    else
        printf '%s' "$ZEB_AUTH_JSON_BOOTSTRAP" > "$ZEB_HOME/auth.json"
        chown zeb:zeb "$ZEB_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ZEB_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ZEB_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ZEB_HOME/auth.json" ] && [ -n "${ZEB_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ZEB_HOME/auth.json"; then
        :
    else
        s6-setuidgid zeb "$INSTALL_DIR/.venv/bin/python" \
...
# can set ZEB_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZEB_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZEB_HOME/gateway_state.json" ] && \
        [ "${ZEB_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ZEB_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ZEB_HOME/gateway_state.json"
        chown zeb:zeb "$ZEB_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ZEB_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_zeb "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/zeb/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Zeb spawns for the
...
# main-wrapper.sh propagates it into the supervised ``zeb`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-zeb, dashboard, etc.).
