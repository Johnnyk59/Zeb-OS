...
This document is the formal interface between the **Zeb gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `zeb gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`zeb gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `zeb` CLI.
