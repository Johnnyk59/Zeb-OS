...
The canonical transcript store is SQLite via `SessionDB` (from `zeb_state`). The
...
Counts consecutive restarts via a JSON file (`{ZEB_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `zeb update`, `zeb gateway restart`,
or `/restart`.
