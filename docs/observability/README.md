...
Zeb observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Zeb' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Zeb catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Zeb only builds
