...
Zeb supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.zeb/config.yaml`:
...
Or set the env var: `ZEB_KANBAN_DISPATCH_IN_GATEWAY=false`
