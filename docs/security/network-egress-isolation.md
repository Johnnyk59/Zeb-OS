...
When running Zeb inside Docker, the default `network_mode: host` gives the
...
The Zeb [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ZEB_UID=$(id -u) ZEB_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=zeb,zeb-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://zeb-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Zeb trust model and vulnerability reporting
