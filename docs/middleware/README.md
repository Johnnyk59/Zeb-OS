...
Zeb middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Zeb'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Zeb calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Zeb retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Zeb guardrails,
...
- `telemetry_schema_version`: currently `zeb.observer.v1`
- `middleware_schema_version`: currently `zeb.middleware.v1`
...
Zeb stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Zeb runs
them as a nested chain in registration order. Middleware failures are fail-open:
Zeb logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Zeb applies middleware in this order:
...
For each tool call, Zeb applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Zeb pre-execution path against the effective arguments:
...
```bash
zeb plugins enable <plugin-name>
```

For isolated local testing, use one `ZEB_HOME` for plugin enablement and the
agent run:

```bash
export ZEB_HOME=/tmp/zeb-middleware-test
mkdir -p "$ZEB_HOME"
zeb plugins enable <plugin-name>
zeb chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run zeb plugins enable <plugin-name>
uv run zeb chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["zeb_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/zeb-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Zeb expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Zeb treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Zeb preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Zeb does not convert downstream
