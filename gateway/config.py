...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_zeb_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ZEB_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.zeb/config.yaml (primary user-facing config)
    3. ~/.zeb/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_zeb_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through zeb_cli.config.load_config, so without this a managed
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `zeb config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``zeb config set gateway.streaming.*``
