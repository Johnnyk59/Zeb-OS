...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{ZEB_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``ZEB_HOME`` is a **durable** store — on Zeb
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``zeb gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting ZEB_HOME."""
    base = home if home is not None else get_zeb_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable ZEB_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (ZEB_HOME is a
    durable Fly volume on Zeb Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable ZEB_HOME volume is
