...
Module-level run.py helpers a handler needs (``_zeb_home``,
...
from zeb_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: zeb gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Zeb commands.
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from zeb_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`zeb gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _zeb_home
        ...
        try:
            ...
            atomic_json_write(
                _zeb_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Zeb Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from zeb_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _zeb_home, _load_gateway_config
        import yaml
        ...
        user_provs = None
        custom_provs = None
        config_path = _zeb_home / "config.yaml"
        try:
            ...
            if cfg:
                ...
                try:
                    from zeb_cli.config import get_compatible_custom_providers
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from zeb_cli.context_switch_guard import (
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Zeb default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_zeb_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from zeb_cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _zeb_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zeb_home, _platform_config_key
        import yaml
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zeb_home
        from zeb_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.zeb/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``zeb skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _zeb_home
        from zeb_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `zeb skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.zeb/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _zeb_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zeb_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zeb_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from zeb_cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `zeb bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``zeb debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Zeb Agent to the latest version.

        Spawns ``zeb update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``zeb update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _zeb_home, _resolve_zeb_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from zeb_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Zeb Agent')}"
        ...
        zeb_cmd = _resolve_zeb_bin()
        if not zeb_cmd:
            return t("gateway.update.zeb_cmd_not_found")

        pending_path = _zeb_home / ".update_pending.json"
        output_path = _zeb_home / ".update_output.txt"
        exit_code_path = _zeb_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `zeb update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `zeb update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # zeb_cmd is a list of argv parts we can pass directly
                ...
                zeb_cmd_str = " ".join(shlex.quote(part) for part in zeb_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {zeb_cmd_str} update --gateway"
