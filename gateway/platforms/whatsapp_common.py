...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Zeb Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ZEB_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/zeb), this function
    mirrors the bridge source to a writable ZEB_HOME location and returns that
    ...
    # Try ZEB_HOME location first
    zeb_home = get_zeb_home()
    zeb_home_bridge = zeb_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ZEB_HOME if needed
    if zeb_home_bridge.exists():
        return zeb_home_bridge

    # Mirror the bridge source to ZEB_HOME
    try:
        zeb_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            zeb_home_bridge,
            dirs_exist_ok=False,
