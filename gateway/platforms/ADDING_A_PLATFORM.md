...
There are two ways to add a platform to the Zeb gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zeb/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zeb gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zeb_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Zeb core.
...
And add it to the `zeb-gateway` composite:

```python
"zeb-gateway": {
    "includes": [..., "zeb-your-platform"]
}
```
...
## 12. Status Display (`zeb_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zeb_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zeb_cli/ toolsets.py \
  --include="*.py" -l | sort -u
