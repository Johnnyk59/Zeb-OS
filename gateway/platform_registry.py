...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``zeb setup`` display).
    ...
    # built-ins).  Used by ``zeb gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `zeb`
        # invocation -- including plain `zeb chat`, which never touches any
        ...
        # `zeb setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``zeb setup``/``gateway status``, channel
