...
The PID file lives at ``{ZEB_HOME}/gateway.pid``.  ZEB_HOME defaults to
``~/.zeb`` but can be overridden via the environment variable.  This means
separate ZEB_HOME directories naturally get separate PID files — a property
...
from zeb_constants import get_zeb_home, _get_platform_default_zeb_home
...
def _get_process_zeb_home() -> Path:
    """Return the process-level ZEB_HOME, skipping context-local overrides.
    ...
    ``get_zeb_home()`` honors ``_ZEB_HOME_OVERRIDE`` contextvar used for
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ZEB_HOME."""
    home = _get_process_zeb_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ZEB_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "zeb" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Zeb gateway lifecycle subcommand from a command line.
    ...
    test also matched ``zeb_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\zeb-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Zeb's
    ...
    for token in tokens:
        ...
        if basename in ("zeb-gateway", "zeb-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "zeb_cli.main" in joined
        or "zeb_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("zeb", "zeb.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `zeb gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Zeb-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Zeb gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ZEB_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.zeb`` or ``$ZEB_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``zeb_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ZEB_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"zeb_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ZEB_HOME= on the argv. ZEB_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "zeb_home=" in command_lc and f"zeb_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `zeb status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ZEB_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ZEB_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ZEB_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``zeb.service`` + ``zeb-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different ZEB_HOME. When two profile gateway
    # services share the same default ~/.zeb (ZEB_HOME not set
    ...
    # field is absent in markers written by older Zeb versions; treat
    ...
    replacer_home = record.get("replacer_zeb_home")
    if replacer_home is not None and replacer_home != str(_get_process_zeb_home()):
        return False
    ...
    # ``zeb gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_zeb_home": str(_get_process_zeb_home()),
