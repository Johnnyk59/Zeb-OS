...
from zeb_constants import get_zeb_dir, get_zeb_home
...
PAIRING_DIR = get_zeb_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from zeb_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ZEB_HOME}/pairing`` while newer code/docs may
    write ``{ZEB_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<ZEB_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<ZEB_HOME>/pairing/``
    directory (backward-compat for the ``zeb pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ZEB_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from zeb_constants import get_zeb_home
            self._dir = get_zeb_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `zeb` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> zeb pairing approve ...` as root, "
                    "re-run with `docker exec -u zeb <container> ...` and "
                    "chown the existing file to the zeb user, or restart the "
