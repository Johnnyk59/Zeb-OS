...
Defenses 1 and 2 (the ``_ZEB_GATEWAY`` guard on ``zeb gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.zeb.gateway")``,
...
State lives in ``<ZEB_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_zeb_home() / "gateway" / "restart_loop.json"
