...
ZEB_HOME_DIR="${ZEB_HOME:-$HOME/.zeb}"
ZEB_CFG="${ZEB_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Zeb config has twozero_td MCP entry ──
if [[ ! -f "$ZEB_CFG" ]]; then
    echo -e " ${FAIL} Zeb config not found at ${ZEB_CFG}"
    manual_steps+=("Create ${ZEB_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZEB_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Zeb config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Zeb config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZEB_CFG} manually"); }
    manual_steps+=("Restart Zeb session to pick up config change")
fi
