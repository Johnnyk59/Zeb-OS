#!/usr/bin/env python3
"""Google Workspace API CLI for Zeb Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zeb-facing JSON contract and falls back to the Python client
...
from _zeb_home import get_zeb_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZEB_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zeb Agent")
