...
``zeb gateway restart`` (or ``launchctl kickstart ai.zeb.gateway``
or ``systemctl restart zeb-gateway``).  When the cron fires, the
...
``zeb cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``zeb gateway``, ``launchctl ... zeb-gateway``,
``systemctl ... zeb-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_ZEB_GATEWAY=1``, and
``zeb gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `zeb gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:zeb\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a zeb-gateway label. macOS launchd
    # labels look like `ai.zeb.gateway` / `zeb-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated zeb services (e.g.
    # `launchctl unload ai.zeb.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bzeb[.\-]?gateway)"
    # Branch C: systemctl ops on a zeb-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bzeb[.\-]?gateway)"
    # Branch D: pkill / kill targeting the zeb gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<ZEB_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.zeb/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_zeb_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `zeb gateway restart` from a shell outside "
