...
interface ZebConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZebConsoleModal({ open, onClose }: ZebConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("zeb> ");
  const inputPromptRef = useRef("zeb> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "zeb> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Zeb Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="zeb-console-title"
    >
      <div
        className={cn(
          themedBody,
