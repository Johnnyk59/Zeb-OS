/**
 * Zeb Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ZEB_PLUGIN_SDK__`` and
 * ``window.__ZEB_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ZEB_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``ZebPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@zeb/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__ZEB_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ZEB_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ZEB_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ZebPluginSDK {
...
declare global {
  interface Window {
    __ZEB_PLUGIN_SDK__?: ZebPluginSDK;
    __ZEB_PLUGINS__?: PluginRegistry;
  }
}
