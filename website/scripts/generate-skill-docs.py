...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/zeb-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zeb_meta = (fm.get("metadata") or {}).get("zeb") or {}
    tags = zeb_meta.get("tags") or []
    related = zeb_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zeb skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Zeb Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Zeb ships with a large built-in skill library copied into `~/.zeb/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Zeb also syncs bundled skills on `zeb update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zeb/skills/` tree, it is still shipped with Zeb; restore it with `zeb skills reset <name> --restore`.",
        "",
