...
The ACP adapter wraps Zeb' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zeb acp / zeb-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.zeb/.env
  -> configure stderr logging
  -> construct ZebACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'zeb-agent[acp]==<version>' zeb-acp`, pointed at the `zeb-agent` PyPI release.
...
- `allow_once` -> Zeb `once`
- `allow_always` -> Zeb `always`
- reject options -> Zeb `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Zeb tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zeb-acp"])
...
Instead it reuses Zeb' runtime resolver:

- `acp_adapter/auth.py`
- `zeb_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Zeb provider/credentials. It also always advertises a terminal setup auth method (`zeb-setup`, args `--setup`) so first-run registry clients can open Zeb' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.zeb/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `zeb-acp` toolset definition
- `zeb_cli/main.py` — `zeb acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zeb-acp` script
