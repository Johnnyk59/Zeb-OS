...
description: "How Zeb stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zeb_cli/cron.py` | CLI `zeb cron` subcommands |
...
Jobs are stored in `~/.zeb/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zeb cron` commands are run or during active CLI sessions.
...
```python
# ~/.zeb/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.zeb/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `zeb cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `zeb cron` CLI provides direct job management:

```bash
zeb cron list                    # Show all jobs
zeb cron create                  # Interactive job creation (alias: add)
zeb cron edit <job_id>           # Edit job configuration
zeb cron pause <job_id>          # Pause a running job
zeb cron resume <job_id>         # Resume a paused job
zeb cron run <job_id>            # Trigger immediate execution
zeb cron remove <job_id>         # Delete a job
```
