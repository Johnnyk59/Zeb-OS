...
sidebar_position: 1
title: "Architecture"
description: "Zeb Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Zeb Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zeb_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZebCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zeb_state.py           # SQLite session/state database with FTS5
├── zeb_constants.py       # ZEB_HOME, profile-aware paths
...
│
├── zeb_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `zeb` subcommands (large file)
...
│   ├── skills_config.py      # zeb skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # zeb tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # zeb gateway start/stop
│
...
```text
User input → ZebCLI.process_input()
...
Three discovery sources: `~/.zeb/plugins/` (user), `.zeb/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `zeb plugins` or `config.yaml`.
...
Exposes Zeb as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`zeb -p <name>`) gets its own ZEB_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
