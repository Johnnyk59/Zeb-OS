...
Zeb Agent uses a SQLite database (`~/.zeb/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `zeb_state.py`


## Architecture Overview

```
~/.zeb/state.db (SQLite, WAL mode)
...
Multiple zeb processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.zeb/state.db
...
Default path: `~/.zeb/state.db`

This is derived from `zeb_constants.get_zeb_home()` which resolves to
`~/.zeb/` by default, or the value of `ZEB_HOME` environment variable.
