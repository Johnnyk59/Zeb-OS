...
```bash
zeb teams-pipeline token-health
zeb teams-pipeline token-health --force-refresh
```
...
```bash
zeb teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
zeb teams-pipeline maintain-subscriptions
zeb teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Zeb cron (recommended if you already run the Zeb gateway)

Zeb ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zeb/scripts/`. First create the script:

```bash
mkdir -p ~/.zeb/scripts
cat > ~/.zeb/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec zeb teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.zeb/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
zeb cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/zeb-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Zeb Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=zeb
EnvironmentFile=/etc/zeb/env
ExecStart=/usr/local/bin/zeb teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/zeb-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Zeb Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now zeb-teams-pipeline-maintain.timer
systemctl list-timers zeb-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/zeb teams-pipeline maintain-subscriptions >> /var/log/zeb/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zeb/.env` at the top of a wrapper script that crontab calls.
...
```bash
zeb teams-pipeline subscriptions   # should show expirationDateTime advanced
zeb teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
zeb teams-pipeline list
zeb teams-pipeline list --status failed
zeb teams-pipeline show <job-id>
```

### Replay a stored job

```bash
zeb teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
zeb teams-pipeline fetch --meeting-id <meeting-id>
zeb teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `zeb teams-pipeline maintain-subscriptions --dry-run`
- inspect `zeb teams-pipeline list --status failed`
...
- run `zeb teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zeb teams-pipeline run`
...
- [ ] `zeb teams-pipeline validate` returns an OK snapshot
- [ ] `zeb teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Zeb cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
