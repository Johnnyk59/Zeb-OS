---
sidebar_position: 7
title: "Use SOUL.md with Zeb"
description: "How to use SOUL.md to shape Zeb Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Zeb

`SOUL.md` is the **primary identity** for your Zeb instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Zeb to feel like the same assistant every time you talk to it — or if you want to replace the Zeb persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Zeb should be
- what Zeb should avoid stylistically
- how Zeb should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Zeb is and how Zeb speaks
...
Zeb now uses only the global SOUL file for the current instance:

```text
~/.zeb/SOUL.md
```

If you run Zeb with a custom home directory, it becomes:
...
Zeb automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Zeb does not overwrite it
- if the file exists but is empty, Zeb adds nothing from it to the prompt

## How Zeb uses it

When Zeb starts a session, it reads `SOUL.md` from `ZEB_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Zeb feels.
...
Zeb already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Zeb should sound.

# Avoid
What Zeb should not do.

# Defaults
How Zeb should behave when ambiguity appears.
```
...
Then restart Zeb or start a new session.
...
### I edited SOUL.md but Zeb still sounds the same

Check:
- you edited `~/.zeb/SOUL.md` or `$ZEB_HOME/SOUL.md`
...
### Zeb is ignoring parts of my SOUL.md
