...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `zeb send`."
---

# Pipe Script Output to Messaging Platforms

`zeb send` is a small, scriptable CLI that pushes a message to any
messaging platform Zeb is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | zeb send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `zeb
...
zeb send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
zeb send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
zeb send --list

# Filter by platform
zeb send --list telegram
```
...
Any platform Zeb ships adapters for works as a target:
...
`zeb send` resolves the message body in this order:

1. **Positional argument** — `zeb send --to telegram "hi"`
2. **`--file PATH`** — `zeb send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | zeb send --to telegram`

When stdin is a TTY (no pipe), Zeb does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  zeb send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `zeb send` reuses your Zeb config, the same script works on
any host where Zeb is installed — no need to export bot tokens into
...
`zeb send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  zeb send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | zeb send \
...
```bash
./train.py --epochs 200 && \
  zeb send --to telegram "training done" || \
  zeb send --to telegram "training failed (exit $?)"
```
...
zeb send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(zeb send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `zeb send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `zeb send` calls
the platform's REST endpoint directly using credentials from
`~/.zeb/.env` and `~/.zeb/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `zeb gateway start` and retry.
...
The listing is built from `~/.zeb/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`zeb gateway
...
| Approach | Multi-platform | Reuses Zeb creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `zeb send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`zeb send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `zeb send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `zeb send` shares with cron delivery.
