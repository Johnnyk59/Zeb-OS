---
sidebar_position: 8
title: "Use Voice Mode with Zeb"
description: "A practical guide to setting up and using Zeb voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zeb
...
- you want Zeb sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zeb.
...
## Step 1: make sure normal Zeb works first
...
Zeb supports both local and cloud speech stacks.
...
### If you use `zeb setup`

If you choose NeuTTS in the setup wizard, Zeb checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zeb transcribes and responds
...
- asking Zeb to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zeb loop.

## Tuning CLI behavior

### Silence threshold

If Zeb starts/stops too aggressively, tune:
...
Zeb stays a normal chat bot, but can speak replies.
...
- you want Zeb to function like a portable research or ops assistant
...
Zeb joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zeb detects speech boundaries
- transcripts are posted in the associated text channel
- Zeb responds in text and audio
...
1. get text Zeb working
2. install `zeb-agent[voice]`
