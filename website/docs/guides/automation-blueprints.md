...
Copy-paste blueprints for common automation patterns. Each blueprint uses Zeb's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`zeb webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `zeb webhook subscribe`) |
...
```bash
zeb cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/zeb-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/zeb-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
zeb webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
zeb cron create "0 9 * * 1" \
  "Scan the NousResearch/zeb-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/zeb-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (zeb_cli/commands.py, zeb_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (zeb_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
zeb cron create "0 6 * * *" \
  "Run a dependency security audit on the zeb-agent project.

1. cd ~/.zeb/zeb-agent && source .venv/bin/activate
...
```bash
zeb webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
zeb webhook subscribe alert-triage \
...
```python title="~/.zeb/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Zeb-Monitor/1.0"})
...
zeb cron create "every 30m" \
  ...
  --script ~/.zeb/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
zeb cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Zeb Agent development." \
...
```bash
zeb webhook subscribe github-issues \
  --events "issues" \
...
```bash
zeb webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
zeb webhook subscribe stripe-payments \
...
```bash
zeb cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the zeb-agent codebase.
