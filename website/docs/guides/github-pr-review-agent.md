...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Zeb Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Zeb instantly when PRs are opened or updated.
:::
...
- **Zeb Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  zeb gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.zeb/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Zeb can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/zeb-agent --state open --limit 3
```
...
Still in the chat, ask Zeb to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/zeb-agent
```
...
A skill gives Zeb consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.zeb/skills/code-review
```

Create `~/.zeb/skills/code-review/SKILL.md`:
...
Verify it loaded — start `zeb` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Zeb your team's standards:
...
2. Teach Zeb your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
zeb gateway status    # Is the gateway running?
zeb cron list         # Is the job enabled?
```
