...
This tutorial walks you through setting up a Telegram bot powered by Zeb Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Zeb Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.zeb/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Zeb itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Zeb Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zeb_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Zeb Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u zeb-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zeb gateway install` to update the plist.
:::
...
```bash
zeb gateway stop && zeb gateway start
```
...
zeb pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.zeb/.env`:
...
Control how much detail the bot shows when using tools. In `~/.zeb/config.yaml`:
...
Customize how the bot communicates by editing `~/.zeb/SOUL.md`:

For a full guide, see [Use SOUL.md with Zeb](/guides/use-soul-with-zeb).
...
```bash
# From the CLI
zeb cron list          # View all scheduled jobs
zeb cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zeb-gateway` (Linux) or `~/.zeb/logs/gateway.log` (macOS) |
| Cron job output | `~/.zeb/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.zeb/cron/jobs.json` |
| Pairing data | `~/.zeb/pairing/` |
| Session history | `~/.zeb/sessions/` |
