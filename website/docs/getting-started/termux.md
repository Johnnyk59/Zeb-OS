...
description: "Run Zeb Agent directly on an Android phone with Termux"
---

# Zeb on Android with Termux
...
Zeb Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zeb from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Zeb now ships a Termux-aware installer path:
...
- links `zeb` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zeb` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zeb" "$PREFIX/bin/zeb"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zeb` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zeb/.env`.
...
### `zeb doctor` says ripgrep or Node is missing
