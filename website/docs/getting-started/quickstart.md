...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Zeb Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Zeb setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Zeb Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Zeb Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Zeb for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Zeb working on my machine | `zeb setup` | Run a real chat and verify it responds |
| I already know my provider | `zeb model` | Save the config, then start chatting |
| I want a bot or always-on setup | `zeb gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `zeb model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `zeb model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Zeb cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Zeb Agent
### With the Zeb Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Zeb Desktop installer](https://zeb-agent.nousresearch.com/) from our website and run it.

### Without Zeb Desktop:
For a command-line only install without Zeb Desktop, run:
...
The single most important setup step. Use `zeb model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `zeb setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `zeb update`. Re-enable anything later with `zeb tools`, seed skills with `zeb skills opt-in --sync`, or tune settings with `zeb setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `zeb model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `zeb model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `zeb model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `zeb_cli/models.py` may change between releases) | `zeb model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `zeb model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `zeb model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `zeb model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `zeb model` (requires `copilot` CLI + `copilot login`) |
...
Zeb Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `zeb model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Zeb separates secrets from normal config:

- **Secrets and tokens** → `~/.zeb/.env`
- **Non-secret settings** → `~/.zeb/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
zeb config set model anthropic/claude-opus-4.6
zeb config set terminal.backend docker
zeb config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
zeb            # classic CLI
zeb --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Zeb ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `zeb` vs `zeb --tui`.
:::
...
```bash
zeb --continue    # Resume the most recent session
zeb -c            # Short form
```
...
```bash
zeb gateway setup    # Interactive platform configuration
```
...
- `zeb tools` — tune tool access per platform
- `zeb skills` — browse and install reusable workflows
...
```bash
zeb config set terminal.backend docker    # Docker isolation
zeb config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Zeb install directory (the curl installer placed it at
# ~/.zeb/zeb-agent on Linux/macOS or %LOCALAPPDATA%\zeb\zeb-agent on Windows):
cd ~/.zeb/zeb-agent
...
Skills are on-demand instruction documents that teach Zeb how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Zeb ships with a catalog of bundled skills already installed in `~/.zeb/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
zeb skills browse                      # list everything available
zeb skills search kubernetes           # find skills by keyword
zeb skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `zeb skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Zeb ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.zeb/zeb-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Zeb opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `zeb model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `zeb gateway setup` and check `zeb gateway status` |
| `zeb --continue` can't find old session | Switched profiles or session never saved | Check `zeb sessions list` and confirm you're in the right profile |
...
| `zeb doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `zeb tools` | Configure which tools are enabled per platform |
| `zeb setup` | Full setup wizard (configures everything at once) |
| `zeb doctor` | Diagnose issues |
| `zeb update` | Update to latest version |
| `zeb gateway` | Start the messaging gateway |
| `zeb --continue` | Resume last session |
