...
This page covers all commands related to [Zeb profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `zeb profile` without a subcommand shows help.
...
```bash
zeb profile use <name>
```

Sets `<name>` as the active profile. All subsequent `zeb` commands (without `-p`) will use this profile.
...
zeb profile use default
```

## `zeb profile create`

```bash
zeb profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `zeb profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `zeb update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.zeb`), use `zeb skills opt-out` / `zeb skills opt-in`. |
...
zeb profile create work2 --clone-from work

# Clone everything from a specific profile
zeb profile create work2-backup --clone-from work --clone-all
```

## `zeb profile describe`

```bash
zeb profile describe [<name>] [options]
```
...
zeb profile describe researcher

# Set it explicitly
zeb profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
zeb profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
zeb profile describe --all --auto
```

## `zeb profile delete`

```bash
zeb profile delete <name> [options]
```
...
```bash
zeb profile delete mybot
zeb profile delete mybot --yes
```
...
```bash
zeb profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Zeb home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ zeb profile show work
Profile: work
Path:    ~/.zeb/profiles/work
...
```bash
zeb profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Zeb installation.
...
```bash
zeb profile alias work
# Creates/updates ~/.local/bin/work

zeb profile alias work --name mywork
# Creates ~/.local/bin/mywork

zeb profile alias work --remove
...
```bash
zeb profile rename <old-name> <new-name>
```
...
```bash
zeb profile rename mybot assistant
# ~/.zeb/profiles/mybot → ~/.zeb/profiles/assistant
...
```bash
zeb profile export <name> [options]
```
...
```bash
zeb profile export work
# Creates work.tar.gz in the current directory

zeb profile export work -o ./work-2026-03-29.tar.gz
```

## `zeb profile import`

```bash
zeb profile import <archive> [options]
```
...
```bash
zeb profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

zeb profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`zeb profile export` / `import` are still the right commands for
...
```bash
zeb profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `zeb -p telemetry`). |
...
zeb profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
zeb profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
zeb profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
zeb profile install ./telemetry/
```

### `zeb profile update`

```bash
zeb profile update <name> [--force-config] [--yes]
```
...
```bash
zeb profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Zeb version, author, env var requirements, the source URL/path, and
...
`zeb profile list` also shows the distribution name and version in a
`Distribution` column, and `zeb profile show <name>` / `delete <name>`
...
zeb profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
zeb profile install https://github.com/your-org/internal-assistant.git
```
...
`zeb_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Zeb version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Zeb can clone from.
3. Tell recipients to run `zeb profile install <your-repo-url>`.
...
```bash
zeb -p <name> <command> [options]
zeb --profile <name> <command> [options]
```

Global flag to run any Zeb command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
zeb -p work chat -q "Check the server status"
zeb --profile dev gateway start
zeb -p personal skills list
zeb -p work config edit
```

## `zeb completion`

```bash
zeb completion <shell>
```
...
- `zeb profile <TAB>` — subcommands (list, use, create, etc.)
- `zeb profile use <TAB>` — profile names
- `zeb -p <TAB>` — profile names
