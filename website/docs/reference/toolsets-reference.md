...
description: "Reference for Zeb core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `zeb-cli` is the default for interactive CLI sessions)
...
```bash
zeb chat --toolsets web,file,terminal
zeb chat --toolsets debugging        # composite — expands to file + terminal + web
zeb chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - zeb-cli          # default for CLI
  # - zeb-telegram   # override for Telegram gateway
```

### Interactive management

```bash
zeb tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Zeb tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `zeb-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `zeb-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `zeb-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `zeb tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `zeb-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `zeb-cli`:

| Toolset | Differences from `zeb-cli` |
|---------|-------------------------------|
| `zeb-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `zeb-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `zeb-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `zeb-cron` | Same as `zeb-cli`. |
| `zeb-telegram` | Same as `zeb-cli`. |
| `zeb-discord` | Adds `discord` and `discord_admin` on top of `zeb-cli`. |
| `zeb-slack` | Same as `zeb-cli`. |
| `zeb-whatsapp` | Same as `zeb-cli`. |
| `zeb-signal` | Same as `zeb-cli`. |
| `zeb-matrix` | Same as `zeb-cli`. |
| `zeb-mattermost` | Same as `zeb-cli`. |
| `zeb-email` | Same as `zeb-cli`. |
| `zeb-sms` | Same as `zeb-cli`. |
| `zeb-bluebubbles` | Same as `zeb-cli`. |
| `zeb-dingtalk` | Same as `zeb-cli`. |
| `zeb-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `zeb-qqbot` | Same as `zeb-cli`. |
| `zeb-wecom` | Same as `zeb-cli`. |
| `zeb-wecom-callback` | Same as `zeb-cli`. |
| `zeb-weixin` | Same as `zeb-cli`. |
| `zeb-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `zeb-cli`. |
| `zeb-homeassistant` | Same as `zeb-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `zeb-webhook` | Same as `zeb-cli`. |
| `zeb-gateway` | Internal gateway orchestrator toolset — union of every `zeb-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `ZEB_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `zeb tools`

The `zeb tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
