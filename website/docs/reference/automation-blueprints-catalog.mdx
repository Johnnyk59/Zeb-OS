...
of fields, and Zeb schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Zeb asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.zeb.blueprint` block in its
