...
If you only run one Zeb agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Zeb agents that should all
...
(`ai.zeb.gateway-<name>.plist`) or systemd user service
(`zeb-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
zeb profile create coder
zeb profile create personal-bot
zeb profile create research
...
```bash
zeb config set gateway.multiplex_profiles true
zeb gateway restart
```

Equivalently, in the default profile's `~/.zeb/config.yaml`:
...
You do **not** run `zeb gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `zeb gateway start` / `run` is a
...
home). `zeb status` reports the multiplexer and the profiles it serves;
`zeb status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/zeb-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: zeb-gateways {start|stop|restart|status|list}"
}
...
```bash
zeb-gateways start      # start every configured profile
zeb-gateways stop       # stop every configured profile
zeb-gateways restart    # restart all
zeb-gateways status     # status across all
zeb-gateways list       # delegates to `zeb gateway list`
```

:::tip
The `default` profile is targeted with `zeb gateway <action>` (no `-p`),
not `zeb -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `zeb -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.zeb.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/zeb-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.zeb.gateway.plist` /
`zeb-gateway.service`.
...
```bash
# Default profile
tail -f ~/.zeb/logs/gateway.log
tail -f ~/.zeb/logs/gateway.error.log

# Named profile
tail -f ~/.zeb/profiles/<name>/logs/gateway.log
tail -f ~/.zeb/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.zeb/logs/gateway.log ~/.zeb/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
zeb logs -f                  # follow default profile
zeb -p coder logs -f         # follow one profile
zeb logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
zeb profile list             # profiles + model + gateway state
zeb-gateways status          # full status across every profile
launchctl list | grep zeb    # macOS — PIDs and labels
systemctl --user list-units 'zeb-gateway-*'   # Linux — units
```
...
The default profile uses `~/.zeb/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
zeb config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.zeb/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=zeb --why="gateways running" \
  sleep infinity &
...
`zeb-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.zeb/.env ~/.zeb/profiles/*/.env
```

## Updating the code

`zeb update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `zeb gateway start` after a previous `zeb gateway stop`. The
...
```bash
ps -ef | grep "zeb_cli.*-p <profile>"
cat ~/.zeb/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.zeb.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.zeb.gateway-<profile>.plist

# Linux
systemctl --user restart zeb-gateway-<profile>.service
```

### Health check

```bash
zeb doctor                  # default profile
zeb -p <profile> doctor     # one profile
```
