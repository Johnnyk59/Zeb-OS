...
title: "Git Worktrees"
description: "Run multiple Zeb agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Zeb Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Zeb so each session has a clean, isolated working directory.

## Why Use Worktrees with Zeb?

Zeb treats the **current working directory** as the project root:

- CLI: the directory where you run `zeb` or `zeb chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.zeb/config.yaml`
...
git worktree add ../repo-feature feature/zeb-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zeb-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Zeb there:

```bash
cd ../repo-feature

# Start Zeb in the worktree
zeb
```
...
git worktree add ../repo-experiment-a feature/zeb-a
git worktree add ../repo-experiment-b feature/zeb-b
```
...
- Works on its own branch (`feature/zeb-a` vs `feature/zeb-b`).
...
- Zeb checkpoint data under `~/.zeb/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Zeb experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zeb-checkpoints-docs`, `feature/zeb-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Zeb from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zeb -w` (Automatic Worktree Mode)

Zeb has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zeb/zeb-<hash>`).
...
```bash
zeb -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zeb -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Zeb session its own clean checkout.
