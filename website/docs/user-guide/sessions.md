...
Zeb Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.zeb/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Zeb stores session history so it can resume conversations, but it does not
...
Zeb explicitly injects for that turn.
...
For example, if a user sends an image and asks Zeb to make a meme from it,
Zeb may inspect that image once with vision and run an image-processing
...
`zeb sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zeb` or `zeb chat`) |
...
zeb -c "my project"   # → resumes "my project #3"
```
...
zeb --resume 20250305_091523_a1b2c3d4
zeb -r 20250305_091523_a1b2c3d4

# Resume by title
zeb --resume "refactoring auth"

# Or with the chat subcommand
zeb chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zeb sessions list`.

### Conversation Recap on Resume

When you resume a session, Zeb displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Zeb session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.zeb/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `zeb -r "<title>"` from the shell) and pick up where the platform left off.
...
Zeb automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `zeb sessions list` or `zeb sessions browse`.
...
```bash
zeb sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Zeb creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zeb -c "my project"`), it automatically picks the most recent session in the lineage.
...
Zeb provides a full set of session management commands via `zeb sessions`:
...
# Filter by platform
zeb sessions list --source telegram

# Show more sessions
zeb sessions list --limit 50
```
...
zeb sessions export backup.jsonl

# Export sessions from a specific platform
zeb sessions export telegram-history.jsonl --source telegram

# Export a single session
zeb sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
zeb sessions export backup.jsonl --redact
```
...
zeb sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
zeb sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
zeb sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
zeb sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.zeb/session-exports`).

```bash
# Export one session to Markdown
zeb sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
zeb sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
zeb sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
zeb sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
zeb sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
zeb sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
zeb sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zeb sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zeb sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zeb sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
zeb sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
zeb sessions prune --newer-than 5h

# Explicit window with absolute timestamps
zeb sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
zeb sessions prune --source telegram
zeb sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
zeb sessions prune --newer-than 5h --title "smoke test"   # title substring
zeb sessions prune --older-than 30 --max-messages 3        # tiny sessions
zeb sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
zeb sessions prune --model gpt-5 --older-than 1w           # by model (substring)
zeb sessions prune --provider openrouter --older-than 60   # by billing provider
zeb sessions prune --branch feature/old-experiment         # by git branch
zeb sessions prune --user 12345678 --chat-type group       # by messaging origin
zeb sessions prune --max-tokens 500 --older-than 7         # by token usage
zeb sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
zeb sessions prune --newer-than 5h --dry-run

# Skip confirmation
zeb sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `zeb sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `zeb sessions prune` keeps the 90-day cutoff. Every
...
`zeb sessions archive` takes the same filters as `prune` but soft-hides
...
zeb sessions archive --newer-than 5h

# Archive by title substring, preview first
zeb sessions archive --title "dry run" --dry-run
zeb sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `zeb sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`zeb sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zeb insights`](/reference/cli-commands#zeb-insights).
...
When Zeb cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Zeb uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Zeb in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.zeb/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.zeb/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.zeb/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.zeb/sessions/sessions.json` is the **gateway routing index** — it maps
...
`zeb sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.zeb/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `zeb sessions list`, the cause is
`state.db` not receiving them — run `zeb sessions repair` and watch for a
...
`*.jsonl` files in `~/.zeb/sessions/`. They are no longer written or
read by Zeb. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Zeb process in the same `ZEB_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.zeb/config.yaml`:
...
zeb sessions delete <session_id>
