...
Connect Zeb to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Zeb — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Zeb runs it in a
...
zeb photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Zeb Agent` project on your account.
...
Runtime credentials are written to `~/.zeb/.env`
...
(device token, dashboard project id) lives in `~/.zeb/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Zeb
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Zeb replies with a pairing code. Approve it with:

```bash
zeb pairing approve photon <CODE>
```

Use `zeb pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.zeb/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.zeb/.env`):
...
By default Zeb responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Zeb` and
`@Zeb agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Zeb will reply.
...
assigned iMessage line Zeb uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run zeb photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `zeb photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `zeb photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `zeb photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Zeb sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Zeb wake words  | JSON list / comma / newline regex patterns for group mentions |
