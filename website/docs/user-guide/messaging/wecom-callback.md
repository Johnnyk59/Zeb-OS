...
Connect Zeb to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Zeb supports two WeCom integration modes:
...
> Run `zeb gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Zeb decrypts the message, queues it for the agent
...
(Use `zeb gateway start` only after `zeb gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Zeb and the token the
...
in `~/.zeb/.env` values around `=` will also break signature checks. After
fixing, restart `zeb gateway run`.
...
Check `zeb gateway run` logs for the bound host/port. If the adapter bound to
