...
description: "Set up Zeb Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Zeb can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Zeb gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Zeb limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Zeb degrades gracefully if absent.
...
2. **Add** → name (e.g. `zeb-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.zeb/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Zeb to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Zeb gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.zeb/.env`.
3. Start the Zeb gateway: `zeb gateway`.
...
In addition to Meta's recipient whitelist, Zeb has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.zeb/.env`:
...
The `zeb whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.zeb/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Zeb uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Zeb acknowledges inbound messages immediately:
...
Zeb TTS produces MP3. Two paths:
...
Zeb warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Zeb. If you need it, please [open an issue](https://github.com/NousResearch/zeb-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Zeb's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Zeb doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.zeb/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `zeb gateway` is up.
- **App Secret not set** — without it, Zeb refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Zeb.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `zeb tools list` and verify the platform is using `zeb-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `zeb_cli/platforms.py` for the platform → default toolset mapping.
...
```bash
zeb config set stt.provider openai
zeb config set stt.use_gateway true
zeb gateway restart
```
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Zeb will accept as authentic.
...
| | Baileys (`zeb whatsapp`) | Cloud API (`zeb whatsapp-cloud`) |
|---|---|---|
...
Most users running Zeb for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
