...
description: "Chat with Zeb from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Zeb from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Zeb](/guides/use-voice-mode-with-zeb).
...
For group chats, hooks, and automation flows, Zeb supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Zeb keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Zeb does not hide failures just because the text resembles a silence token.
...
```bash
zeb gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
zeb gateway              # Run in foreground
zeb gateway setup        # Configure messaging platforms interactively
zeb gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo zeb gateway install --system   # Linux only: install a boot-time system service
zeb gateway start        # Start the default service
zeb gateway stop         # Stop the default service
zeb gateway status       # Check default service status
zeb gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Zeb Agent to the latest version |
...
with the `session_reset` section in `~/.zeb/config.yaml`:
...
Configure per-platform overrides in `~/.zeb/gateway.json`:
...
zeb pairing approve telegram XKGH5N7P

# Other pairing commands:
zeb pairing list          # View pending + approved users
zeb pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Zeb appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.zeb/config.yaml`:
...
Off by default. When enabled, Zeb prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.zeb/config.yaml`:
...
```bash
ZEB_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
zeb gateway install               # Install as user service
zeb gateway start                 # Start the service
zeb gateway stop                  # Stop the service
zeb gateway status                # Check status
journalctl --user -u zeb-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo zeb gateway install --system
sudo zeb gateway start --system
sudo zeb gateway status --system
journalctl -u zeb-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Zeb installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit zeb-gateway` (or `sudo systemctl edit zeb-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `zeb update`. When `zeb update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart zeb-gateway` command (it never blocks on an interactive password prompt).
...
```bash
zeb gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `zeb update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo zeb update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/zeb-gateway`:

```
zeb ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed zeb-gateway*, /usr/bin/systemctl --no-ask-password start zeb-gateway*, /usr/bin/systemctl --no-ask-password restart zeb-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Zeb will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Zeb installations on the same machine (with different `ZEB_HOME` directories), each gets its own systemd service name. The default `~/.zeb` uses `zeb-gateway`; other installations use `zeb-gateway-<hash>`. The `zeb gateway` commands automatically target the correct service for your current `ZEB_HOME`.
:::

### macOS (launchd)

```bash
zeb gateway install               # Install as launchd agent
zeb gateway start                 # Start the service
zeb gateway stop                  # Stop the service
zeb gateway status                # Check status
tail -f ~/.zeb/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.zeb.gateway.plist`. It includes three environment variables:
...
- **ZEB_HOME** — scopes the gateway to your Zeb installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `zeb gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ZEB_HOME` directory gets its own launchd label. The default `~/.zeb` uses `ai.zeb.gateway`; other installations use `ai.zeb.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `zeb-cli` | Full access |
| Telegram | `zeb-telegram` | Full tools including terminal |
| Discord | `zeb-discord` | Full tools including terminal |
| WhatsApp | `zeb-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `zeb-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `zeb-slack` | Full tools including terminal |
| Google Chat | `zeb-google_chat` | Full tools including terminal |
| Signal | `zeb-signal` | Full tools including terminal |
| SMS | `zeb-sms` | Full tools including terminal |
| Email | `zeb-email` | Full tools including terminal |
| Home Assistant | `zeb-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `zeb-mattermost` | Full tools including terminal |
| Matrix | `zeb-matrix` | Full tools including terminal |
| DingTalk | `zeb-dingtalk` | Full tools including terminal |
| Feishu/Lark | `zeb-feishu` | Full tools including terminal |
| WeCom | `zeb-wecom` | Full tools including terminal |
| WeCom Callback | `zeb-wecom-callback` | Full tools including terminal |
| Weixin | `zeb-weixin` | Full tools including terminal |
| BlueBubbles | `zeb-bluebubbles` | Full tools including terminal |
| QQBot | `zeb-qqbot` | Full tools including terminal |
| Yuanbao | `zeb-yuanbao` | Full tools including terminal |
| Microsoft Teams | `zeb-teams` | Full tools including terminal |
| API Server | `zeb-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `zeb-webhook` | Full tools including terminal |
| Raft | `zeb-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.zeb/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
