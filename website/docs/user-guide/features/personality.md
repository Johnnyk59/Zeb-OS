...
description: "Customize Zeb Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Zeb Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZEB_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Zeb is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Zeb now seeds a default `SOUL.md` automatically in:

```text
~/.zeb/SOUL.md
```

More precisely, it uses the current instance's `ZEB_HOME`, so if you run Zeb with a custom home directory, it will use:
...
- Zeb creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Zeb loads `SOUL.md` only from `ZEB_HOME`
- Zeb does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Zeb falls back to a built-in default identity
...
If Zeb loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZEB_HOME`, the personality belongs to the Zeb instance itself.

That also makes it easier to teach users:
- "Edit `~/.zeb/SOUL.md` to change Zeb' default personality."
...
- how Zeb should handle uncertainty, disagreement, or ambiguity
...
## What Zeb injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Zeb falls back to a built-in default identity ("You are Zeb Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Zeb ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Zeb, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Zeb its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.zeb/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.zeb/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-zeb)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Zeb speaks
- `display.skin` and `/skin` affect how Zeb looks in the terminal
