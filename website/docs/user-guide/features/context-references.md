...
Type `@` followed by a reference to inject content directly into your message. Zeb expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Zeb env: `$ZEB_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZEB_HOME/skills/.hub/`
