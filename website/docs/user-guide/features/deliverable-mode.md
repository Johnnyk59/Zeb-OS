...
When Zeb Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.zeb/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.zeb/config.yaml` (switchable per session
via `/personality`).
...
If you use Zeb' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.zeb/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Zeb Agent's deliverable
