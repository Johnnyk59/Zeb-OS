...
description: "Plugins shipped with Zeb Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Zeb ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.zeb/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Zeb Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.zeb/plugins/<name>/`
3. **Project** — `./.zeb/plugins/<name>/` (requires `ZEB_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `zeb_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `zeb memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `zeb plugins list` and the interactive `zeb plugins` UI), but none load until you explicitly enable them:

```bash
zeb plugins enable disk-cleanup
```

Or via `~/.zeb/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Zeb. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
zeb plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `zeb plugins enable <name>`.
...
| `zeb-achievements` | dashboard tab | Steam-style collectible badges generated from your real Zeb session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `zeb memory` and `zeb plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZEB_HOME` or `/tmp/zeb-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ZEB_HOME | always | Never |
...
**State** — everything lives at `$ZEB_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ZEB_HOME` or `/tmp/zeb-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `zeb plugins enable disk-cleanup` (or check the box in `zeb plugins`).

**Disabling again:** `zeb plugins disable disk-cleanup`.
...
**Enabling:** `zeb plugins enable security-guidance` (or check the box in `zeb plugins`).

**Disabling again:** `zeb plugins disable security-guidance`.
...
Traces Zeb turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Zeb' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `zeb logs`.
...
```bash
zeb tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Zeb and the next turn ships a trace.
...
Then put the credentials in `~/.zeb/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Zeb turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ZEB_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Zeb session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `zeb chat` session lives under one Langfuse session.

**Verify:**

```bash
zeb plugins list                 # observability/langfuse should show "enabled"
zeb chat -q "hello"              # check the Langfuse UI for a "Zeb turn" trace
```
...
| `ZEB_LANGFUSE_RELEASE` | — | Release/version tag |
| `ZEB_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ZEB_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ZEB_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Zeb-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Zeb-prefixed wins when both are set.
...
**Disabling:** `zeb plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.zeb/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
zeb plugins enable google_meet
...
**Disabling:** `zeb plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.zeb/cache/google_meet/` until you remove them.

### zeb-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Zeb session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Zeb feature changes.

**How it works:**

- Scans your entire `~/.zeb/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ZEB_HOME/plugins/zeb-achievements/state.json`
...
| Secret | Hidden until Zeb detects the first related signal in your history |

**API** — routes mount under `/api/plugins/zeb-achievements/`:
...
**State files** — live under `$ZEB_HOME/plugins/zeb-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Zeb updates. |
...
**Enabling:** Nothing to enable — `zeb-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `zeb dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/zeb-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.zeb/plugins/zeb-achievements/` that ships no dashboard. The plugin's state files under `$ZEB_HOME/plugins/zeb-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Zeb plugin — see [Build a Zeb Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.zeb/plugins/<name>/`
- Manifest source is reported as `bundled` in `zeb plugins list`
