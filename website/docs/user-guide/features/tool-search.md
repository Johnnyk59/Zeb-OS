...
**Tool Search** is Zeb' opt-in progressive-disclosure layer for that
...
:::info Built-in Zeb tools never defer
The tools that make up Zeb' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_ZEB_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Zeb **unwraps the bridge** and
...
- **No JS sandbox.** Zeb uses the simpler "structured tools" mode
