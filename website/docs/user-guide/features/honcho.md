...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Zeb's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Zeb instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zeb memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `zeb` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Zeb syncs them back at session init.
...
The `zeb honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `zeb memory setup honcho` (or run `zeb memory setup` and pick it from the list); the `zeb honcho` subcommand then appears on the next invocation.

```bash
zeb memory setup honcho    # Configure Honcho directly (works before activation)
zeb honcho status          # Connection status, config, and key settings
zeb honcho setup           # Redirects to `zeb memory setup` (post-activation alias)
zeb honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
zeb honcho peer            # Show or update peer names + dialectic reasoning level
zeb honcho mode            # Show or set recall mode (hybrid/context/tools)
zeb honcho tokens          # Show or set token budget for context and dialectic
zeb honcho identity        # Seed or show the AI peer's Honcho identity
zeb honcho sync            # Sync Honcho config to all existing profiles
zeb honcho peers           # Show peer identities across all profiles
zeb honcho sessions        # List known Honcho session mappings
zeb honcho map             # Map current directory to a Honcho session name
zeb honcho enable          # Enable Honcho for the active profile
zeb honcho disable         # Disable Honcho for the active profile
zeb honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `zeb honcho`

If you previously used the standalone `zeb honcho setup`:
...
No re-login or re-setup needed. Run `zeb memory setup` and select "honcho" — the wizard detects your existing config.
