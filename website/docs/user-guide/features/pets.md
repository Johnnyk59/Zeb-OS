...
Zeb can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ZEB_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Zeb renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `zeb pets list [query] [--limit N]` |
| List installed pets | `zeb pets list --installed` |
| Install a pet | `zeb pets install <slug> [--select] [--force]` |
...
| Preview/animate | `zeb pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `zeb pets off` |
| Remove an installed pet | `zeb pets remove <slug>` |
...
Beyond installing pre-made pets from the gallery, Zeb can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `zeb pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `zeb tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ZEB_PET_IMAGE_PROVIDER` env var (e.g. `ZEB_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Zeb is
...
Run `zeb pets doctor` — it reports:
