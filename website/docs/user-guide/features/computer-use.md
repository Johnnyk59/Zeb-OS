...
Zeb Agent can drive your desktop — clicking, typing, scrolling,
...
```
zeb computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `zeb computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `zeb tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Zeb app). `zeb computer-use doctor` will tell you which permission is missing. |
...
```
zeb -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.zeb/config.yaml`.

## `zeb computer-use doctor` — your first triage stop

`zeb computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ zeb computer-use doctor
...
agent is acting here." Each Zeb run declares its own cua-driver
**session id** (something like `zeb-3a7b9c14d2e8`); the cursor's
...
Zeb intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Zeb-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Zeb skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Zeb
...
Zeb applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.zeb/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Zeb applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Zeb agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Zeb agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Zeb warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Zeb from inside the RDP / console
...
```
ZEB_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ZEB_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Zeb disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Zeb sets
...
When it's on, `zeb computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Zeb at a binary you built from source instead
of the published release. Zeb resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ZEB_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Zeb testing (see notes).
...
### Option B — point Zeb straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ZEB_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ZEB_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ZEB_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Zeb is using your build

- `zeb computer-use status` prints the resolved binary path and
  version.
- `zeb computer-use doctor` confirms the binary is reachable and
...
- **Zeb spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Zeb
  change needed either way.
- **Local builds skip the version check.** Zeb warns when the
...
**First action when anything's off: run `zeb computer-use doctor`.**
...
Run `zeb computer-use install` to fetch the cua-driver binary, or
run `zeb tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `zeb computer-use doctor` will
...
- **Zeb-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Zeb `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Zeb (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Zeb among them)
