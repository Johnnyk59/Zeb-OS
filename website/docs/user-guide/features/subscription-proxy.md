...
chat completions — use your Zeb-managed provider subscription as their
...
| Use case | "Use Zeb as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Zeb stores
the refresh token in `~/.zeb/auth.json` — the same place all Zeb
...
Model:      Zeb-4-70B    # or Zeb-4.3-36B, Zeb-4-405B
```
...
interface in `zeb_cli/proxy/adapters/`.
...
If you see `not logged in`, run `zeb portal`. If you see
...
`zeb_cli/proxy/adapters/<provider>.py` and registering it in
