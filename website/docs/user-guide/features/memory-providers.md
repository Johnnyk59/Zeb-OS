...
Zeb Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
zeb memory setup      # interactive picker + configuration
zeb memory status     # check what's active
zeb memory off        # disable external provider
```

You can also select the active memory provider via `zeb plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.zeb/config.yaml`:
...
When a memory provider is active, Zeb automatically:
...
**Setup Wizard:**
```bash
zeb memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `zeb honcho setup` command still works (it now redirects to `zeb memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$ZEB_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ZEB_HOME/honcho.json` > `~/.zeb/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/zeb-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zeb).
...
:::tip Migrating from `zeb honcho`
If you previously used `zeb honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Zeb profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Zeb profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Zeb profile. Host key `zeb` → default; `zeb.<profile>` for others. |
...
```bash
zeb profile create coder --clone
```

`--clone` creates a `zeb.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Zeb profile, creates host blocks for any profile without one, inherits settings from the default `zeb` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `zeb memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/zeb-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zeb).
...
# Then configure Zeb
zeb memory setup    # select "openviking"
# Or manually:
zeb config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.zeb/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.zeb/.env
```
...
`OPENVIKING_AGENT` is Zeb' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
zeb memory setup    # select "mem0" → "Platform"
# Or manually:
zeb config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.zeb/.env
```

**Setup (OSS):**
```bash
zeb memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
zeb memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
zeb memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
zeb memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
zeb memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.zeb/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.zeb/.env
```
...
**Config:** `$ZEB_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.zeb/.env`.
...
| `user_id` | `zeb-user` | User identifier |
| `agent_id` | `zeb` | Agent identifier |
...
**Switching modes:** Re-run `zeb memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
zeb memory setup    # select "hindsight"
# Or manually:
zeb config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.zeb/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p zeb ui start`

**Config:** `$ZEB_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `zeb` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Zeb Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/zeb-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
zeb memory setup    # select "holographic"
# Or manually:
zeb config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.zeb-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZEB_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
zeb memory setup    # select "retaindb"
# Or manually:
zeb config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.zeb/.env
```
...
# Then configure Zeb
zeb memory setup    # select "byterover"
# Or manually:
zeb config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ZEB_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=zeb) |
...
**Setup:**
```bash
zeb memory setup    # select "supermemory"
# Or manually:
zeb config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zeb/.env
```

**Config:** `$ZEB_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zeb` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `zeb-{identity}` → `zeb-coder`) to isolate memories per Zeb profile
...
| **Requires** | `pip install zeb-memori` + `zeb-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install zeb-memori
zeb-memori install
zeb config set memory.provider memori
zeb memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `zeb-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ZEB_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ZEB_HOME/` so each profile has its own credentials
