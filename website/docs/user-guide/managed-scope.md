...
`~/.zeb/config.yaml`, `~/.zeb/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/zeb`:

```text
/etc/zeb/
├── config.yaml     # managed config layer (wins over ~/.zeb/config.yaml)
└── .env            # managed env layer (wins over ~/.zeb/.env + shell)
```
...
The location can be relocated with the `ZEB_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ZEB_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Zeb.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ZEB_MANAGED_DIR=/opt/org/zeb-policy
```

:::warning
A user who can set `ZEB_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `zeb doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/zeb/config.yaml` (managed) | `/etc/zeb/.env` (managed) |
| 2 | `~/.zeb/config.yaml` (user) | `~/.zeb/.env` (user) |
...
```bash
zeb config        # shows a header naming the managed source + the pinned keys
zeb doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Zeb refuses and names the source:

```bash
$ zeb config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/zeb/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `zeb config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/zeb

# Pin some config values for every user on this machine
sudo tee /etc/zeb/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/zeb/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/zeb
sudo chmod 0644 /etc/zeb/config.yaml /etc/zeb/.env
```

Changes take effect on the next Zeb start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`zeb doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Zeb as `root`), managed scope is advisory.
