...
description: "Operate the Teams meeting summary pipeline via Zeb CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Zeb CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `zeb teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${ZEB_HOME:-~/.zeb}/.env`:
...
```bash
zeb teams-pipeline validate              # config snapshot — run first after any change
zeb teams-pipeline token-health          # Graph token status
zeb teams-pipeline token-health --force-refresh   # force a fresh token acquisition
zeb teams-pipeline list                  # recent meeting jobs
zeb teams-pipeline list --status failed  # only failed jobs
zeb teams-pipeline show <job-id>         # full detail of one job
zeb teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
zeb teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
zeb teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
zeb teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
zeb teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

zeb teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
zeb teams-pipeline delete-subscription <sub-id>
zeb teams-pipeline maintain-subscriptions            # renew near-expiry ones
zeb teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `zeb teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `zeb cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
