...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`zeb-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zeb-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Zeb terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Zeb itself, `model.provider: openai-codex` uses Zeb-managed Codex
OAuth from `~/.zeb/auth.json` after `zeb auth add openai-codex`. For the
...
When invoking the Codex CLI from a Zeb gateway/service context (for example,
