...
- Give it a title like "zeb-agent-&lt;machine-name>"
...
elif _zeb_env="${ZEB_HOME:-$HOME/.zeb}/.env"; [ -f "$_zeb_env" ] && grep -q "^GITHUB_TOKEN=" "$_zeb_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zeb_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
