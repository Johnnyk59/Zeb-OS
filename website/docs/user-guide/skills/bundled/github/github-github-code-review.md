...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _zeb_env="${ZEB_HOME:-$HOME/.zeb}/.env"; [ -f "$_zeb_env" ] && grep -q "^GITHUB_TOKEN=" "$_zeb_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zeb_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Zeb Agent\",
    \"comments\": [
...
```bash
source "${ZEB_HOME:-$HOME/.zeb}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Zeb Agent. Code looks clean — good test coverage, no security concerns."
