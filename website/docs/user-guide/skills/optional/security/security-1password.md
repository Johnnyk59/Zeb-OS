...
| | |
|---|---|
| Source | Optional — install with `zeb skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Zeb Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Zeb terminal calls (desktop app flow only)
...
### Service Account (recommended for Zeb)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZEB_HOME:-~/.zeb}/.env` (the skill will prompt for this on first load).
...
## Zeb Execution Pattern (desktop app flow)

Zeb terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zeb-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zeb-op.sock"
