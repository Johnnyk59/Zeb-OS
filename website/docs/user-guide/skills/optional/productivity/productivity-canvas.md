...
| | |
|---|---|
| Source | Optional — install with `zeb skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Zeb Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ZEB_HOME:-~/.zeb}/.env`:
...
```bash
CANVAS="python $ZEB_HOME/skills/productivity/canvas/scripts/canvas_api.py"
