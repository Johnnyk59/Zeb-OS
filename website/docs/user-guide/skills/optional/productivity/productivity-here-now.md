...
| | |
|---|---|
| Source | Optional — install with `zeb skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZEB_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZEB_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZEB_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client zeb
```
...
```bash
PUBLISH="${ZEB_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client zeb
```
...
```bash
DRIVE="${ZEB_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `zeb`)    |
