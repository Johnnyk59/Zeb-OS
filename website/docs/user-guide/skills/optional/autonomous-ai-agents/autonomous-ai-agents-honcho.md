...
description: "Configure and use Honcho memory with Zeb -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Zeb -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zeb skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Zeb

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zeb profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zeb#running-honcho-locally-with-zeb

### Verify

```bash
zeb honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Zeb profile's AI peer
...
Honcho models conversations as interactions between **peers**. Zeb creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zeb instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zeb run |
| `global` | Single session across all directories |

Manual override: `zeb honcho map my-project-name`
...
Each Zeb profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zeb.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zeb.coder` host block in `honcho.json`
...
```bash
zeb honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Zeb when Honcho memory is active.
...
Config file: `$ZEB_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zeb honcho setup`. Ensure `memory.provider: honcho` is in `~/.zeb/config.yaml`.

### Memory not persisting across sessions
Check `zeb honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zeb profile create <name> --clone`. For existing profiles: `zeb honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zeb honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zeb honcho status` | Show resolved config, connection test, peer info for active profile |
| `zeb honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zeb honcho disable` | Disable Honcho for the active profile |
| `zeb honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zeb honcho peers` | Show peer identities across all profiles |
| `zeb honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zeb honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zeb honcho sessions` | List known directory-to-session-name mappings |
| `zeb honcho map <name>` | Map current working directory to a Honcho session name |
| `zeb honcho identity` | Seed AI peer identity or show both peer representations |
| `zeb honcho sync` | Create host blocks for all Zeb profiles that don't have one yet |
| `zeb honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zeb + Honcho |
| `zeb memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zeb memory status` | Show active memory provider and config |
| `zeb memory off` | Disable external memory provider |
