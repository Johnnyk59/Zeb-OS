...
| | |
|---|---|
| Source | Optional — install with `zeb skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Zeb or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zeb. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zeb/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zeb/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zeb integration, either:

- configure the server in `~/.zeb/config.yaml` using the `native-mcp` skill, or
...
### Zeb cannot see the deployed server

The server-building part may be correct while the Zeb config is not. Load the `native-mcp` skill and configure the server in `~/.zeb/config.yaml`, then restart Zeb.
