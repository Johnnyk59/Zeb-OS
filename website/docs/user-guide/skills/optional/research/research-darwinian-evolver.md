...
| | |
|---|---|
| Source | Optional — install with `zeb skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Zeb Agent |
| License | MIT |
...
import upstream classes into Zeb itself.
...
```bash
mkdir -p ~/.zeb/cache/darwinian-evolver && cd ~/.zeb/cache/darwinian-evolver
...
```bash
cd ~/.zeb/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.zeb/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.zeb/skills/research/darwinian-evolver
DE_DIR=~/.zeb/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Zeb core.
   Custom driver scripts under `~/.zeb/skills/...` are user-side and fine.
