/**
 * Zeb Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zeb/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZEB_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZEB_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zeb-kanban-dot-triage",
    todo: "zeb-kanban-dot-todo",
    ready: "zeb-kanban-dot-ready",
    running: "zeb-kanban-dot-running",
    blocked: "zeb-kanban-dot-blocked",
    done: "zeb-kanban-dot-done",
    archived: "zeb-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zeb-task";
  ...
  const DOCS_URL = "https://zeb-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://zeb-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "zeb.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zeb-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zeb-kanban-md-code\1/i.test(attrs)) {
      return ' class="zeb-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zeb-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zeb-kanban-md",
  ...
  // standard `drop` event and our `zeb-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zeb-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zeb-kanban-column--drop");
          if (target) target.classList.add("zeb-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zeb-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zeb-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zeb-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZEB_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zeb-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zeb-kanban-attention",
        "zeb-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zeb-kanban-attention-bar" },
        h("span", { className: "zeb-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zeb-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zeb-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zeb-kanban-attention-row",
                  "zeb-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zeb-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zeb-kanban-attention-row-id" }, task.id),
                h("span", { className: "zeb-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zeb-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zeb-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zeb-kanban-diag-action-btn",
      action.suggested ? "zeb-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zeb-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zeb-kanban-drawer-comment-row input, .zeb-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zeb-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zeb-kanban-diag", sevClass) },
      h("div", { className: "zeb-kanban-diag-header" },
        h("span", { className: "zeb-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zeb-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zeb-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zeb-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zeb-kanban-diag-data-row" },
                  h("span", { className: "zeb-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zeb-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zeb-kanban-diag-data-row" },
                h("span", { className: "zeb-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zeb-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zeb-kanban-diag-reassign-row" },
            h("span", { className: "zeb-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zeb-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zeb-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zeb-kanban-diag-msg",
              msg.ok ? "zeb-kanban-diag-msg--ok" : "zeb-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zeb-kanban-section" },
      h("div", { className: "zeb-kanban-section-head-row" },
        h("span", { className: "zeb-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zeb-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zeb-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zeb-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zeb-kanban-docs-link",
      title: "Open Zeb Kanban docs in a new tab",
      "aria-label": "Zeb Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zeb kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zeb-kanban-msg-ok" : "zeb-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zeb-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zeb-kanban-boardswitcher" },
      h("div", { className: "zeb-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zeb-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zeb-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zeb-kanban-dialog-title" },
        ...
        h("div", { className: "zeb-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Zeb profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zeb-kanban-bulk" },
      h("span", { className: "zeb-kanban-bulk-count" },
      ...
      h("div", { className: "zeb-kanban-bulk-priority",
      ...
      h("div", { className: "zeb-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Zeb profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zeb-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeb-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zeb-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zeb-kanban-trash",
        dragOver ? "zeb-kanban-trash--drop" : "",
        props.draggingTaskId ? "zeb-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zeb-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zeb-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zeb-kanban-card")) return true;
      if (target.closest && target.closest(".zeb-kanban-column-add")) return true;
      if (target.closest && target.closest(".zeb-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zeb-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zeb-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zeb-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "zeb-kanban-columns",
        isScrollable ? "zeb-kanban-columns--scrollable" : "",
        isPanning ? "zeb-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeb-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zeb-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zeb-kanban-column",
        dragOver ? "zeb-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zeb-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zeb-kanban-col-check",
        ...
        h("span", { className: cn("zeb-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zeb-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zeb-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zeb-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zeb-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zeb-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zeb-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zeb-kanban-lane" },
                  h("div", { className: "zeb-kanban-lane-head" },
                    h("span", { className: "zeb-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zeb-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zeb-kanban-card--stale-red";
    if (age >= tier.amber) return "zeb-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zeb-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zeb-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zeb-kanban-card",
        props.selected ? "zeb-kanban-card--selected" : "",
        props.failed ? "zeb-kanban-card--failed" : "",
        props.draggingSource ? "zeb-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zeb-kanban-card-content" },
          h("div", { className: "zeb-kanban-card-row" },
            h("label", {
              className: "zeb-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zeb-kanban-card-check",
            ...
            h("span", { className: "zeb-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zeb kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zeb-kanban-warning-badge",
                    "zeb-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "zeb-kanban-progress",
                    progress.done === progress.total ? "zeb-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zeb-kanban-needs-assignee",
          ...
          h("div", { className: "zeb-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zeb-kanban-card-row zeb-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zeb-kanban-assignee",
                            title: `Assigned to Zeb profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zeb-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zeb-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zeb-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "zeb-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Zeb profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Zeb profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zeb-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zeb-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zeb-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zeb-kanban-drawer-close",
        ...
        data ? h("div", { className: "zeb-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zeb-kanban-section" },
      h("div", { className: "zeb-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zeb-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zeb-kanban-drawer-body" },
      h("div", { className: "zeb-kanban-drawer-title" },
        h("span", { className: cn("zeb-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "zeb-kanban-section" },
        h("div", { className: "zeb-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "zeb-kanban-section" },
        h("div", { className: "zeb-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zeb-kanban-comment" },
                h("div", { className: "zeb-kanban-comment-head" },
                  h("span", { className: "zeb-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zeb-kanban-comment-ago" },
      ...
      h("div", { className: "zeb-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "zeb-kanban-event",
              isDiag ? "zeb-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zeb-kanban-event-header" },
                  h("span", { className: "zeb-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zeb-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zeb-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zeb-kanban-event-header-plain" },
                  h("span", { className: "zeb-kanban-event-kind" }, e.kind),
                  h("span", { className: "zeb-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zeb-kanban-event-phantom-row" },
                  h("span", { className: "zeb-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zeb-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zeb-kanban-event-payload" },
    ...
    return h("div", { className: "zeb-kanban-section" },
      h("div", { className: "zeb-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zeb-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zeb-kanban-run--${r.outcome || r.status || "ended"}`
          : "zeb-kanban-run--active";
        return h("div", { key: r.id, className: cn("zeb-kanban-run", outcomeClass) },
          h("div", { className: "zeb-kanban-run-head" },
            h("span", { className: "zeb-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zeb-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zeb-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zeb-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zeb-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zeb-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zeb-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zeb-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zeb-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zeb-kanban-pre zeb-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zeb-kanban-meta-row" },
      h("span", { className: "zeb-kanban-meta-label" }, props.label),
      h("span", { className: "zeb-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zeb-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeb-kanban-meta-row" },
        h("span", { className: "zeb-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zeb-kanban-meta-value zeb-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeb-kanban-meta-row" },
        h("span", { className: "zeb-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zeb-kanban-meta-value zeb-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zeb-kanban-section" },
      h("div", { className: "zeb-kanban-section-head-row" },
        h("span", { className: "zeb-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zeb-kanban-section" },
      h("div", { className: "zeb-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zeb-kanban-deps-row" },
        h("span", { className: "zeb-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zeb-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zeb-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zeb-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zeb-kanban-dep-chip-x",
      ...
      h("div", { className: "zeb-kanban-deps-row" },
        h("span", { className: "zeb-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zeb-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zeb-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zeb-kanban-section" },
      ...
      h("div", { className: "zeb-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zeb-kanban-home-sub zeb-kanban-home-sub--on"
              : "zeb-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__ZEB_PLUGINS__ && typeof window.__ZEB_PLUGINS__.register === "function") {
    window.__ZEB_PLUGINS__.register("kanban", KanbanPage);
  }
})();
