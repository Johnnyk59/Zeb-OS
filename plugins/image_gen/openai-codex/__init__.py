...
Output is saved as PNG under ``$ZEB_HOME/cache/images/``. Source images for
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `zeb auth codex` (or `zeb setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`zeb auth codex` (or `zeb setup` → Codex) to sign in."
                ),
