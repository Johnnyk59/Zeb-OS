# Zeb Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: zeb-achievements plugin
...
### `GET /api/plugins/zeb-achievements/achievements`
...
### `POST /api/plugins/zeb-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/zeb-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/zeb-achievements/overview`
...
- Persist plugin data under:
  - `~/.zeb/plugins/zeb-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.zeb/plugins/zeb-achievements/scan_checkpoint.json`
