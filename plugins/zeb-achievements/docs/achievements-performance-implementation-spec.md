# Zeb Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.zeb/plugins/zeb-achievements/state.json` (existing unlock state)
  - `~/.zeb/plugins/zeb-achievements/scan_snapshot.json` (new)
  - `~/.zeb/plugins/zeb-achievements/scan_checkpoint.json` (new)
