...
Behavioral settings live in `$ZEB_HOME/mem0.json` (set them via `zeb memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.zeb/.env`.
...
| `user_id` | `zeb-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Zeb session and call `mem0_search` — it connects to your server.
...
```bash
zeb memory setup mem0 --mode oss \
...
```bash
zeb memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZEB_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.zeb/mem0_qdrant"}}
  }
}
```
...
```bash
zeb memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ZEB_HOME/mem0.json`).
