...
def _save_mem0_json(zeb_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(zeb_home) / "mem0.json"
    existing = {}
...
def _setup_platform(zeb_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "zeb-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "zeb"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.zeb/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.zeb/.env (or unset it) or Zeb will keep "
    ...
    provider.save_config(provider_config, zeb_home)

    if env_writes:
        _write_env(Path(zeb_home) / ".env", env_writes)
...
def _setup_selfhosted(zeb_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "zeb-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "zeb")
...
def _setup_oss(zeb_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(zeb_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "zeb-user")
    ...
    if env_writes:
        _write_env(Path(zeb_home) / ".env", env_writes)
    _save_mem0_json(zeb_home, {"mode": "oss", "user_id": user_id, "agent_id": "zeb", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, zeb_home: str) -> str:
...
_PGVECTOR_CONTAINER = "zeb-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "zeb"
...
def _setup_oss_interactive(zeb_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], zeb_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], zeb_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'zeb-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "zeb-user")

    agent_id = input("  Agent ID [zeb]: ").strip()
    agent_id = agent_id or "zeb"
    ...
    if env_writes:
        _write_env(Path(zeb_home) / ".env", env_writes)
    _save_mem0_json(zeb_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(zeb_home: str, config: dict) -> None:
    """Entry point called by zeb memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(zeb_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(zeb_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(zeb_home, config, flags)
        return
