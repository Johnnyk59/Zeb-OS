...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=zeb"
...
def _load_supermemory_config(zeb_home: str) -> dict:
    config = _default_config()
    config_path = Path(zeb_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zeb_home: str) -> None:
    config_path = Path(zeb_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "zeb"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Zeb writes into the "Zeb" Space in the Supermemory
        ...
        merged = {"sm_source": "zeb", **(metadata or {})}
...
def _resolve_container_tag_for_setup(zeb_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(zeb_home)
...
def _probe_supermemory_connection(api_key: str, zeb_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(zeb_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(zeb_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zeb memory setup`.
        # All other options are documented for $ZEB_HOME/supermemory.json
    ...
    def save_config(self, values, zeb_home):
        ...
        _save_supermemory_config(sanitized, zeb_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, zeb_home)
    ...
    def post_setup(self, zeb_home: str, config: dict) -> None:
        ...
        from zeb_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(zeb_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from zeb_constants import get_zeb_home
        self._zeb_home = kwargs.get("zeb_home") or str(get_zeb_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zeb_home)
