...
- Supermemory API key from [app.supermemory.ai/integrations?connect=zeb](http://app.supermemory.ai/integrations?connect=zeb)
...
Config file: `$ZEB_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zeb` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zeb-{identity}` → `zeb-coder`). |
...
All Supermemory API calls send `x-sm-source: zeb`, and document writes stamp
`metadata.sm_source: zeb`. This is a **functional routing key, not telemetry**:
it groups Zeb-written memories into a dedicated "Zeb" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zeb profile:

```json
{
  "container_tag": "zeb-{identity}"
}
```

For a profile named `coder`, this resolves to `zeb-coder`. The default profile resolves to `zeb-default`. Without `{identity}`, all profiles share the same container.
