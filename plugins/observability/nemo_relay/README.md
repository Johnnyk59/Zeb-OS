...
Optional Zeb observability plugin that maps Zeb observer hooks to
...
not replace Zeb Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Zeb emit NeMo Relay lifecycle events for
the work Zeb already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Zeb Agent can:

- Preserve Zeb execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
zeb plugins enable observability/nemo_relay
```

The `ZEB_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ZEB_HOME` that the
agent run will use:

```bash
env ZEB_HOME=/tmp/zeb-nemo-relay-test \
  zeb plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ZEB_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ZEB_HOME` is the Zeb profile/config home used by both
`zeb plugins enable ...` and the later `zeb chat ...` run. If unset,
Zeb uses the user's default home, usually `~/.zeb`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ZEB_HOME=/tmp/zeb-nemo-relay-test
zeb plugins enable observability/nemo_relay
zeb chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `zeb` command you run is built from the
...
uv run zeb plugins enable observability/nemo_relay
uv run zeb chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/zeb_agent-*.whl
...
The plugin can configure exporters directly from `ZEB_NEMO_RELAY_*`
...
```bash
export ZEB_NEMO_RELAY_ATOF_ENABLED=1
export ZEB_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ZEB_NEMO_RELAY_ATIF_ENABLED=1
export ZEB_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ZEB_NEMO_RELAY_ATOF_FILENAME`
- `ZEB_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ZEB_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ZEB_NEMO_RELAY_ATIF_AGENT_NAME`
- `ZEB_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ZEB_NEMO_RELAY_ATIF_MODEL_NAME`
- `ZEB_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export ZEB_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Zeb Agent"
agent_version = "local"
```

When `ZEB_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ZEB_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ZEB_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Zeb does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Zeb keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Zeb routes LLM and tool
...
For the full generic Zeb middleware contract, see
...
export ZEB_HOME=/tmp/zeb-nemo-relay-docs/zeb-home
mkdir -p "$ZEB_HOME"

cat > "$ZEB_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Zeb session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ZEB_NEMO_RELAY_ATOF_ENABLED=1
export ZEB_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zeb-nemo-relay-docs/subagent/atof
export ZEB_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ZEB_NEMO_RELAY_ATOF_MODE=overwrite
export ZEB_NEMO_RELAY_ATIF_ENABLED=1
export ZEB_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zeb-nemo-relay-docs/subagent/atif
export ZEB_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ZEB_NEMO_RELAY_ATIF_AGENT_NAME='Zeb Agent E2E'
export ZEB_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ZEB_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"zeb.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Zeb Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Zeb dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/zeb-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/zeb-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/zeb-nemo-relay-docs/workdir/beta.txt
cd /tmp/zeb-nemo-relay-docs/workdir

export ZEB_NEMO_RELAY_ATOF_ENABLED=1
export ZEB_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zeb-nemo-relay-docs/parallel/atof
export ZEB_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ZEB_NEMO_RELAY_ATOF_MODE=overwrite
export ZEB_NEMO_RELAY_ATIF_ENABLED=1
export ZEB_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zeb-nemo-relay-docs/parallel/atif
export ZEB_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ZEB_NEMO_RELAY_ATIF_AGENT_NAME='Zeb Agent E2E'
export ZEB_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Zeb sessions map to `agent` scopes.
- Zeb API request hooks map to `llm` scope start/end events.
- Zeb tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Zeb execution middleware to hand
...
```bash
export ZEB_NEMO_RELAY_PLUGINS_TOML=/tmp/zeb-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Zeb routes execution
...
Zeb provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Zeb provider adapter next_call(...)

Zeb tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Zeb tool dispatcher next_call(...)
```
...
middleware for a local Zeb run. This path requires a NeMo Relay runtime that
...
```bash
export ZEB_HOME=/tmp/zeb-middleware-test/zeb-home
mkdir -p "$ZEB_HOME" /tmp/zeb-middleware-test/nemo-relay
...
cat > /tmp/zeb-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/zeb-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/zeb-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Zeb Middleware E2E"
agent_version = "local"
...
export ZEB_NEMO_RELAY_PLUGINS_TOML=/tmp/zeb-middleware-test/nemo-relay/plugins.toml
