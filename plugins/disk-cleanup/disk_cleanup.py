"""disk_cleanup — ephemeral file cleanup for Zeb Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ZEB_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZEB_HOME and /tmp/zeb-*
Never touches: ~/.zeb/logs/ or any system directory.
"""
...
try:
    ...
    def get_zeb_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZEB_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zeb").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZEB_HOME/logs/``."""
    return get_zeb_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZEB_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZEB_HOME or ``/tmp/zeb-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zeb_home = get_zeb_home()
    try:
        path.resolve().relative_to(zeb_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zeb-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zeb-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "zeb-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ZEB_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ZEB_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        zeb_home = get_zeb_home()
        for parent in ("cron", "cronjobs"):
            base = zeb_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZEB_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZEB_HOME, but never recurse into known
    # durable state trees.  Some installs place the Zeb checkout, venv,
    # and desktop build under ZEB_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    zeb_home = get_zeb_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in zeb_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zeb_home = get_zeb_home()
    try:
        rel = path.resolve().relative_to(zeb_home)
        ...
        # Path isn't under ZEB_HOME (e.g. /tmp/zeb-*) — fall through.
        pass
