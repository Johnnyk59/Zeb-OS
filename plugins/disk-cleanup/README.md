...
Auto-tracks and cleans up ephemeral files created during Zeb Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZEB_HOME` and `/tmp/zeb-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZEB_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZEB_HOME` or `/tmp/zeb-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZEB_HOME/disk-cleanup/` is itself excluded
- `$ZEB_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
