...
Credential storage mirrors every other Zeb channel:

    * runtime SDK creds  -> ``~/.zeb/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.zeb/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a zeb dependency
...
# registers Zeb as its own client_id.
...
# Default name of the project Zeb provisions for the operator.
DEFAULT_PROJECT_NAME = "Zeb Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of zeb-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.zeb/auth.json`` honouring the active Zeb profile."""
    try:
        from zeb_constants import get_zeb_home
        return Path(get_zeb_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.zeb")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.zeb/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.zeb/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.zeb/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: zeb_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `zeb photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `zeb photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `zeb photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `zeb photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `zeb photon setup`)"
