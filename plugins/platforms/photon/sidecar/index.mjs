// Zeb Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Zeb): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Zeb -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zeb-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `zeb photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Zeb can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Zeb'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Zeb allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Zeb, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-zeb-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Zeb supplied them so a known-good
