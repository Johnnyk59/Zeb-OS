...
Small Node helper that bridges Zeb Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Zeb is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zeb-Sidecar-Token`)
...
The Zeb plugin's `zeb photon setup` command runs `npm install`
here automatically.
