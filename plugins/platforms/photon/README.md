...
This plugin connects Zeb Agent to iMessage (and other Spectrum
...
TypeScript-only, Zeb runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Zeb-Sidecar-Token`.
...
There is no separate `login` command; like every other Zeb channel,
...
Run `zeb photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.zeb/.env` (the same place every other
...
Management metadata lives in `~/.zeb/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `zeb photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `zeb photon status`, a DM and a group roundtrip,
