"""
Photon Spectrum (iMessage) platform adapter for Zeb Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Zeb dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `zeb photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `zeb setup` / status.
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `zeb update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``zeb photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Zeb defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Zeb-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Zeb checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `zeb photon setup`)"
            )
        # A `zeb update` that bumps the spectrum-ts pin rewrites
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Zeb-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Zeb plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `zeb photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: zeb photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `zeb gateway setup` alongside every other
    ...
    # Register CLI subcommands — `zeb photon ...`
