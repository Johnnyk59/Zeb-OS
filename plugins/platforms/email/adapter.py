"""
Email platform adapter for the Zeb gateway.

Allows users to interact with Zeb by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "zeb-agent" "version" "{_zeb_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `zeb config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `zeb gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Zeb Agent")
        ...
        msg_id = f"<zeb-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# zeb_cli/gateway.py, and the _send_email dispatch in
