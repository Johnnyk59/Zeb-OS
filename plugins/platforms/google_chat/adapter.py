...
# ``model_tools`` import time, so ``zeb status``, ``zeb chat``, etc.
...
# ``zeb_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Zeb is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a zeb MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ZEB_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _zeb_home = _get_zeb_home()
        except (ModuleNotFoundError, ImportError):
            _zeb_home = _Path.home() / ".zeb"
        self._thread_count_store = _ThreadCountStore(
            _zeb_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Zeb is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ZEB_HOME", str(_Path.home() / ".zeb"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ZEB_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a zeb MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Zeb is thinking…' marker message.
        ...
        "Zeb is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Zeb is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Zeb is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Zeb is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Zeb is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``zeb setup``.

    The setup wizard at ``zeb_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.zeb/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. zeb-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zeb/.env")
    print_info("Restart the gateway: zeb gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``zeb cron`` running as a
    separate process from ``zeb gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Zeb plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zeb-agent[google_chat]'",
        ...
        # the "Zeb is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Zeb is thinking…' marker message "
