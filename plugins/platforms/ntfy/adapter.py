"""ntfy platform adapter (Zeb plugin).
...
``plugins/platforms/ntfy/``. The Zeb plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "zeb-in"              # subscribe topic (incoming)
          publish_topic: "zeb-out"     # optional — defaults to topic
...
_ECHO_TAG = "zeb-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``zeb cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Zeb dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `zeb gateway status` without
