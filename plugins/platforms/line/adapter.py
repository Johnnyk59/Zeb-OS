"""
LINE Messaging API platform adapter for Zeb Agent.
...
to Zeb Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZEB_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zeb_home = Path(get_zeb_home()).resolve()
        except Exception:
            zeb_home = Path.home().joinpath(".zeb").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``zeb status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zeb status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zeb setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zeb/.env`` via ``zeb_cli.config``.
    """
    print()
    ...
    try:
        from zeb_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zeb_cli.config not available; set LINE_* vars manually in ~/.zeb/.env")
        return
