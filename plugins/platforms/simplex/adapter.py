"""SimpleX Chat platform adapter (Zeb plugin).
...
This adapter ships as a Zeb platform plugin under
``plugins/platforms/simplex/``. The Zeb plugin loader scans the
...
discoverable and ``zeb setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Zeb — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ZEB_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zeb setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.zeb/.env`` via
    ``zeb_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "zeb_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.zeb/.env"
        )
        return
