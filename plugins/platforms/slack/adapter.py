...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `zeb slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_zeb_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Zeb does not currently need to route them into the agent loop.
            ...
            # /model work directly without /zeb prefix). A single regex
            ...
            # manifest (see `zeb slack manifest`). In Socket Mode, Slack
            ...
            from zeb_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zeb$")

            @self._app.command(_slash_pattern)
            async def handle_zeb_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Zeb handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Zeb session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zeb_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "zeb_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "zeb_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zeb_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zeb_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zeb_confirm_once": "once",
            "zeb_confirm_always": "always",
            "zeb_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zeb_approve_once": "once",
            "zeb_approve_session": "session",
            "zeb_approve_always": "always",
            "zeb_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zeb <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zeb
        ...
        if slash_name in {"zeb", ""}:
            # Legacy /zeb <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zeb_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/zeb   ``).
        ...
        # "Running /cmd…" ack shown by handle_zeb_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/zeb <question>" must produce public replies so
...
# ``zeb_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``zeb_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ZEB_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from zeb_cli.slack_cli import _build_full_manifest
            from zeb_constants import get_zeb_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Zeb",
                bot_description="Your Zeb agent on Slack",
            )
            target = Path(get_zeb_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `zeb slack manifest --write` anytime to refresh after "
                "Zeb adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zeb update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://zeb-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``zeb_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zeb-agent[slack]'",
        # Interactive setup wizard — replaces zeb_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in zeb_cli/gateway.py.
