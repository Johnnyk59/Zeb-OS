...
``{title, url, description, position}`` rows every other Zeb web
provider produces.
...
prefers Zeb-managed xAI Grok OAuth (via ``zeb auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.zeb/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``zeb tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `zeb auth` to sign in with "
