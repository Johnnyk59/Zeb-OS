...
Let the zeb agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zeb runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zeb meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zeb meet` CLI |
...
| `cli.py` | `zeb meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZEB_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zeb meet node {run,list,approve,remove,status,ping}` |
...
zeb meet install                                      # pip + Chromium
zeb meet setup                                        # preflight
zeb meet auth                                         # optional
zeb meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zeb meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zeb/.env
zeb meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zeb meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zeb meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zeb/.env
zeb meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zeb will **not** switch your system audio input automatically — the
...
zeb plugins enable google_meet
zeb meet node run --display-name my-mac --host 0.0.0.0 --port 18789
