...
session-scoped state file under ``$ZEB_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ZEB_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_zeb_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this zeb install, leave it first —
    ...
    env = os.environ.copy()
    env["ZEB_MEET_URL"] = url
    env["ZEB_MEET_OUT_DIR"] = str(out)
    env["ZEB_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZEB_MEET_HEADED"] = "1"
    if auth_state:
        env["ZEB_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZEB_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZEB_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZEB_MEET_MODE"] = mode
    if realtime_model:
        env["ZEB_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZEB_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZEB_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZEB_MEET_REALTIME_KEY"] = realtime_api_key
