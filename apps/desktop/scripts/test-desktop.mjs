...
// no longer bundles the Zeb Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Zeb.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Zeb'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Zeb.exe'),
...
// Default ZEB_HOME for non-sandboxed runs -- matches main.cjs's
// resolveZebHome(). On Windows it's %LOCALAPPDATA%\zeb; elsewhere
// it's ~/.zeb. The fresh-install sandbox launchFresh() sets its own
// ZEB_HOME and never touches this.
const DEFAULT_ZEB_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'zeb')
  }
  return path.join(os.homedir(), '.zeb')
})()
const VENV_ROOT = path.join(DEFAULT_ZEB_HOME, 'zeb-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'zeb-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZEB_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Zeb-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Zeb-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Zeb-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Zeb-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZEB_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZEB_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const zebHome = path.join(sandbox, 'zeb-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(zebHome, { recursive: true })
  ...
  env.ZEB_DESKTOP_CWD = cwd
  env.ZEB_DESKTOP_IGNORE_EXISTING = '1'
  env.ZEB_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZEB_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZEB_HOME = zebHome
  delete env.ZEB_DESKTOP_ZEB
  delete env.ZEB_DESKTOP_ZEB_ROOT
  ...
  console.log(`  ZEB_HOME: ${zebHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(zebHome, 'zeb-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Zeb Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zeb_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'zeb-agent', 'zeb_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Zeb
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZEB_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZEB_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
