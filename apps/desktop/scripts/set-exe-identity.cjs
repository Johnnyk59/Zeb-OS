#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Zeb icon + version metadata onto the
// built Zeb.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Zeb.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `zeb desktop`, the installer's
...
// Stamp the Zeb icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Zeb icon + identity stamped')
}
