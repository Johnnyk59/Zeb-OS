 ...
 *
 * Stamps the Zeb icon + identity onto the packed Windows Zeb.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `zeb desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Zeb')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Zeb'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Zeb.exe keeps the stock Electron icon`)
  }
}
