 ...
 * product name (`Zeb`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Zeb'
 *
 * This is a hard failure with no obvious cause for the user — `zeb desktop`
 ...
 * (the mac-unpacked Zeb.app bundle) and Windows (win-unpacked), so we
