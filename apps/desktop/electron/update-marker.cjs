...
function markerPath(zebHome) {
  return path.join(zebHome, '.zeb-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(zebHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(zebHome)
  let raw
  try {
 ...
 *
 * The Tauri-based zeb-setup.exe takes several seconds to initialise its
 ...
 */
function writeUpdateMarker(zebHome, pid, { now = Date.now } = {}) {
  const file = markerPath(zebHome)
