...
const { canImportZebCli, verifyZebCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeZebHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.ZEB_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ZEB_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ZEB_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[zeb] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[zeb] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('zeb:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[zeb] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[zeb] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[zeb] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ZEB_HOME — the user-facing root for everything Zeb-related. Mirrors
// scripts/install.ps1's $ZebHome and scripts/install.sh's $ZEB_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\zeb (matches install.ps1)
//   macOS / Linux: ~/.zeb (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.zeb directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\zeb yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ZEB_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ZEB_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.zeb / %LOCALAPPDATA%\zeb.
function resolveZebHome() {
  if (process.env.ZEB_HOME) return normalizeZebHomeRoot(process.env.ZEB_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'zeb-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ZEB_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\zeb and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ZEB_HOME')
    if (fromRegistry) return normalizeZebHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'zeb')
    const legacy = path.join(app.getPath('home'), '.zeb')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.zeb setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.zeb')
}

const ZEB_HOME = resolveZebHome()

function zebManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_zeb_node_dirs() in
  // zeb_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ZEB_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithZebManagedNode(...entries) {
  return [...zebManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ZEB_ROOT — the canonical mutable Zeb install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ZEB_ROOT = path.join(ZEB_HOME, 'zeb-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ZEB_ROOT, 'venv')
...
// boots, going straight to `resolveZebBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ZEB_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ZEB_ROOT, '.zeb-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Zeb profile the desktop launches its
// local backend as. When set, startZeb() passes `zeb --profile <name>
// dashboard …`, which deterministically pins ZEB_HOME (see
// _apply_profile_override in zeb_cli/main.py) and bypasses the sticky
// ~/.zeb/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors zeb_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// zebDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ZEB_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by zeb_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ZEB_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (zeb_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ZEB_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ZEB_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'zeb:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.zeb) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.zeb')
}
// Seed the native About panel with the live Zeb version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `zeb update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveZebVersion(),
...
const MEDIA_PROTOCOL = 'zeb-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(zeb-media://…)`.
...
// (the desktop's launch profile) stays managed by zebProcess +
// connectionPromise + startZeb(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ZEB_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ZEB_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startZeb() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning zeb serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Zeb backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  zebLog.push(...lines)
  if (zebLog.length > 300) {
    zebLog.splice(0, zebLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-zeb-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('zeb:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the zeb:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:bootstrap:event', ev)
}
...
// The Tauri updater writes ZEB_HOME/.zeb-update-in-progress for the whole
...
// (`force_kill_other_zeb`, taskkill /IM zeb.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ZEB_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Zeb will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ZEB_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `zeb` — must not shadow `zeb.cmd`/`zeb.exe`.
...
function unwrapWindowsVenvZebCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^zeb(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  // died mid-`pip install` still has python.exe + zeb.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Zeb at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
    return null
  }

  return {
    label: `existing Zeb Python at ${python}`,
    command: python,
    args: ['-m', 'zeb_cli.main', ...backendArgs],
    bootstrap: false,
...
// spawns `zeb serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `zeb` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'zeb_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ZEB_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isZebSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'zeb_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ZEB_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Zeb deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Zeb' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\zeb\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zeb', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'zeb', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal ZEB_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\zeb\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zeb', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'zeb', 'git', 'bin', 'git.exe'))
  }
...
function recentZebLog() {
  return zebLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's zeb root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ZEB_ROOT.
// ZEB_DESKTOP_ZEB_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ZEB_DESKTOP_ZEB_ROOT && path.resolve(process.env.ZEB_DESKTOP_ZEB_ROOT),
    !IS_PACKAGED && isZebSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isZebSourceRoot(ACTIVE_ZEB_ROOT) ? ACTIVE_ZEB_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ZEB_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('zeb:updates:progress', merged)
  }
}
...
// ZEB_HOME/zeb-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_zeb_home). That binary owns
// ALL repo mutation — running `zeb update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'zeb-setup.exe' : 'zeb-setup'
  const candidate = path.join(ZEB_HOME, name)
...
// Path to the venv shim whose lock decides whether `zeb update` can write
// fresh entry points. On Windows this is the file the running backend
// `zeb.exe` holds open; on POSIX it's never mandatory-locked.
function venvZebShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'zeb.exe')
    : path.join(updateRoot, 'venv', 'bin', 'zeb')
}
...
// only signals the direct child, so on Windows a backend `zeb.exe` that
// spawned its own grandchildren (a `zeb` REPL, a pty terminal session, the
...
// `zebProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `zeb.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a zeb
// REPL, a pty terminal, the gateway) keep `zeb.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (zebProcess && Number.isInteger(zebProcess.pid)) pids.push(zebProcess.pid)
  ...
  if (zebProcess && !zebProcess.killed) {
    try {
      zebProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvZebShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (zebProcess && Number.isInteger(zebProcess.pid)) stragglers.push(zebProcess.pid)
    for (const entry of backendPool.values()) {
...
// `zeb update`). Instead we spawn the staged Zeb-Setup binary with
// --update and quit, so it can run `zeb update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri zeb-setup. Unlike Windows
      ...
      // whole update itself: `zeb update` (backend) + `zeb desktop
    ...
    if (!updater) {
      ...
      // `zeb desktop`, never the Tauri installer that self-copies
      // zeb-setup.exe into ZEB_HOME). They DO have a working `zeb`
      ...
      // checkout they're on — bare `zeb update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'zeb update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `zeb update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `zeb update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, zebRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Zeb — this window will close and the updater will open. Don’t reopen Zeb yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // zeb.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running zeb, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Zeb install open ' +
        '(a second Zeb window or a terminal running zeb?). Close it and retry.'
    ...
    // `zeb update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ZEB_HOME,
      env: {
        ...process.env,
        ZEB_HOME,
        PATH: pathWithZebManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(ZEB_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvZeb = path.join(venvBin, IS_WINDOWS ? 'zeb.exe' : 'zeb')
  ...
  // not just the `zeb.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvZeb) || fileExists(path.join(updateRoot, '.zeb-bootstrap-complete'))
...
// Resolve the zeb CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `zeb` on PATH.
function resolveZebCliBinary(updateRoot) {
  const venvZeb = path.join(updateRoot, 'venv', 'bin', 'zeb')
  if (fileExists(venvZeb)) return venvZeb
  return findOnPath('zeb') || null
}
...
// macOS/Linux in-app update: backend (`zeb update`) + OS-aware GUI rebuild
// (`zeb desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const zeb = resolveZebCliBinary(updateRoot)
  if (!zeb) {
    emitUpdateProgress({ stage: 'manual', message: 'zeb update', percent: null })
    return { ok: true, manual: true, command: 'zeb update', zebRoot: updateRoot }
  }

  // Put the Zeb-managed Node and the venv on PATH so `zeb desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\zeb\node, not node\bin.
  const env = {
    ZEB_HOME,
    PATH: pathWithZebManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `zeb update` reaps stale `zeb serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ZEB_DESKTOP_CHILD_PID and excludes
  ...
  if (zebProcess && Number.isInteger(zebProcess.pid)) {
    desktopChildPids.push(zebProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ZEB_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Zeb (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(zeb, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'zeb update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'zeb update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(zeb, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Zeb to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `zeb desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Zeb…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `zeb-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Zeb to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Zeb desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Zeb to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Zeb.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Zeb.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Zeb to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.zeb-update-new"; then
    rm -rf "$DST.zeb-update-old" 2>/dev/null || true
    mv "$DST" "$DST.zeb-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.zeb-update-new" "$DST"
    rm -rf "$DST.zeb-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Zeb to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ZEB_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isZebSourceRoot(ACTIVE_ZEB_ROOT) &&
    fileExists(venvPython) &&
    canImportZebCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ZEB_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `zeb
...
function resolveWebDist() {
  const override = process.env.ZEB_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ZEB_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: zeb desktop --force-build`
  )
  return candidates[0]
}
...
function resolveZebCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Zeb.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveZebCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ZEB_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Zeb at ${ACTIVE_ZEB_ROOT}`,
    command,
    args: ['-m', 'zeb_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      zebHome: ZEB_HOME,
      pythonPathEntries: [ACTIVE_ZEB_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ZEB_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveZebBackend(backendArgs) {
  // 1. Explicit override -- ZEB_DESKTOP_ZEB_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ZEB_DESKTOP_ZEB_ROOT && path.resolve(process.env.ZEB_DESKTOP_ZEB_ROOT)
  if (overrideRoot && isZebSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Zeb source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `zeb` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isZebSourceRoot.)
  if (!IS_PACKAGED && isZebSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Zeb source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ZEB_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\zeb\zeb-agent (Windows) or ~/.zeb/zeb-agent.
  ...
  //    to spawning zeb. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `zeb update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `zeb` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ZEB_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ZEB_DESKTOP_IGNORE_EXISTING !== '1') {
    let zebCommand = null
    const zebOverride = process.env.ZEB_DESKTOP_ZEB

    if (zebOverride) {
      const resolvedOverride = findOnPath(zebOverride)
      if (resolvedOverride) {
        zebCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(zebOverride, { isWsl: IS_WSL })) {
        zebCommand = zebOverride
      } else {
        rememberLog(`Ignoring Windows Zeb override under WSL: ${zebOverride}`)
      }
    } else {
      zebCommand = findOnPath('zeb')
    }

    if (zebCommand) {
      if (looksLikeDesktopAppBinary(zebCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Zeb CLI: ${zebCommand}`)
        zebCommand = null
      }
    }

    if (zebCommand) {
      const unwrapped = unwrapWindowsVenvZebCommand(zebCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `zeb` shim
      ...
      const shellForProbe = isCommandScript(zebCommand)
      if (verifyZebCli(zebCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvZebCommand(zebCommand, backendArgs) || {
            label: `existing Zeb CLI at ${zebCommand}`,
            command: zebCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
      ...
      rememberLog(
        `Ignoring existing Zeb CLI at ${zebCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed zeb_cli module via system Python.
  ...
  if (python) {
    ...
    // having zeb_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\zeb\zeb-agent\venv.
    if (canImportZebCli(python)) {
      return {
        kind: 'python',
        label: `installed zeb_cli module via ${python}`,
        command: python,
        args: ['-m', 'zeb_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: zeb_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveZebBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Zeb Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveZebBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Zeb install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Zeb recovery was handed off to Zeb Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      zebHome: ZEB_HOME,
      logRoot: path.join(ZEB_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Zeb install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Zeb bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ZEB_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startZeb() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // zeb:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveZebBackend(backend.args))
  }
  ...
  if (!isZebSourceRoot(ACTIVE_ZEB_ROOT)) {
    throw new Error(
      `Zeb install at ${ACTIVE_ZEB_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Zeb' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\zeb\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `zeb` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Zeb on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isZebSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Zeb venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Zeb at ${ACTIVE_ZEB_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Zeb runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Zeb backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Zeb backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Zeb backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Zeb-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('zeb:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveZebCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:preview-file-changed', payload)
}
...
async function waitForZeb(baseUrl, token) {
  ...
  throw new Error(`Zeb backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Zeb...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('zeb:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('zeb:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('zeb:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
//
// Hosted Zeb gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``zeb_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / zeb #37247).
//     Both are set as HttpOnly cookies (``zeb_session_at`` ~15 min,
//     ``zeb_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:zeb-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Zeb backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root ZEB_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ZEB_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ZEB_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by ZEB_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (zeb #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Zeb gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Zeb gateway is selected, but no session token is saved. ' +
...
//   2. env override (ZEB_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ZEB_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ZEB_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ZEB_DESKTOP_REMOTE_URL is set but ZEB_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Zeb backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.ZEB_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Zeb
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startZeb())
  ...
  // connect to Zeb gateway". Mirror the renderer's connect here so the test
...
function resetZebConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(zebProcess)
...
// startZeb() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetZebConnection() nulls zebProcess.
  const dying = zebProcess && !zebProcess.killed ? zebProcess : null
  resetZebConnection()
...
// profile to startZeb() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startZeb() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForZeb(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: zebLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ZEB_HOME env (see _apply_profile_override
  // step 3 in zeb_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveZebBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const zebCwd = resolveZebCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Zeb backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: zebCwd,
        ZEB_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ZEB_DESKTOP: '1',
        ZEB_WEB_DIST: webDist,
        ...(readyFile ? { ZEB_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Zeb backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Zeb backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Zeb backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForZeb(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Zeb backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_zeb_home() recreates the deleted profile directory.
...
async function startZeb() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startZeb() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Zeb backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Zeb backend at ${remote.baseUrl}`, 24)
      await waitForZeb(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Zeb backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.zeb/active_profile file) and
    // resolves ZEB_HOME the same way `zeb -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Zeb runtime', 28)
    const backend = await ensureRuntime(resolveZebBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const zebCwd = resolveZebCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Zeb backend via ${backend.label}`, 84)
    rememberLog(`Starting Zeb backend via ${backend.label}`)
    ...
    zebProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ZEB_HOME for the child so Python's get_zeb_home()
          // resolves to the SAME location our resolveZebHome() picked. Without
          // this pin, Python falls back to ~/.zeb on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\zeb, which differs from C:\Users\<u>\.zeb.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ZEB_HOME via setx; the desktop
    ...
    zebProcess.stdout.on('data', rememberLog)
    zebProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    zebProcess.once('error', error => {
      rememberLog(`Zeb backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Zeb backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    zebProcess.once('exit', (code, signal) => {
      rememberLog(`Zeb backend exited (${signal || code})`)
      zebProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Zeb backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Zeb backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentZebLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Zeb backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(zebProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Zeb backend to become ready', 90)
    await Promise.race([waitForZeb(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null zebProcess when the child dies.
      childAlive: () => zebProcess !== null && zebProcess.exitCode === null && !zebProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Zeb backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Zeb is
...
// pushes pet state over IPC (zeb:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via zeb:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Zeb app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Zeb icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see zeb:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Zeb in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('zeb:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startZeb().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('zeb:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Zeb…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('zeb:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetZebConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Zeb backend failed liveness probe; dropping stale connection.')
    resetZebConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('zeb:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('zeb:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('zeb:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('zeb:window:openNewSession', async () => {
...
ipcMain.handle('zeb:zoom:get', event => {
...
ipcMain.on('zeb:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
  if (!window || window.isDestroyed()) return
...
ipcMain.handle('zeb:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('zeb:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('zeb:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('zeb:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('zeb:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('zeb:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('zeb:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('zeb:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('zeb:pet-overlay:control', payload)
})
ipcMain.handle('zeb:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startZeb() call restarts the
...
ipcMain.handle('zeb:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startZeb() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('zeb:bootstrap:cancel', async () => {
...
ipcMain.handle('zeb:boot-progress:get', async () => bootProgressState)
ipcMain.handle('zeb:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('zeb:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('zeb:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('zeb:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('zeb:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('zeb:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('zeb:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('zeb:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('zeb:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('zeb:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ZEB_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('zeb:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('zeb:requestMicrophoneAccess', async () => {
...
ipcMain.handle('zeb:api', async (_event, request) => {
  ...
  // backend calls ensure_zeb_home() which recreates the profile directory,
...
ipcMain.handle('zeb:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('zeb:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('zeb:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('zeb:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('zeb:readFileText', async (_event, filePath) => {
...
ipcMain.handle('zeb:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('zeb:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('zeb:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('zeb:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('zeb:saveClipboardImage', async () => {
...
ipcMain.handle('zeb:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('zeb:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('zeb:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('zeb:titlebar-theme', (_event, payload) => {
...
ipcMain.on('zeb:native-theme', (_event, mode) => {
...
ipcMain.on('zeb:translucency', (_event, payload) => {
...
ipcMain.handle('zeb:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('zeb:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveZebCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('zeb:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveZebCwd()
}))

ipcMain.handle('zeb:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('zeb:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('zeb:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('zeb:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('zeb:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('zeb:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: zebLog.slice(-200) }))
...
function terminalShellCommand() {
  // ZEB_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ZEB_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Zeb' TUI into
  ...
  env.TERM_PROGRAM = 'Zeb'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a zeb/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ZEB_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ZEB_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `zeb:terminal:${id}:${suffix}`
}
...
ipcMain.handle('zeb:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('zeb:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('zeb:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('zeb:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('zeb:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('zeb:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('zeb:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('zeb:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('zeb:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('zeb:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('zeb:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('zeb:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('zeb:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('zeb:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('zeb:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('zeb:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('zeb:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('zeb:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('zeb:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('zeb:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Zeb.')
  }
...
ipcMain.handle('zeb:terminal:write', (_event, id, data) => {
...
ipcMain.handle('zeb:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zeb:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('zeb:updates:check', async () =>
...
ipcMain.handle('zeb:updates:apply', async (_event, payload) =>
...
ipcMain.handle('zeb:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('zeb:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Zeb version (the one `release.py` bumps in
// zeb_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Zeb version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveZebVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'zeb_cli', '__init__.py')
...
// Re-resolve the live Zeb version and push it into the native About panel
// just before showing it, so an in-place `zeb update` is reflected without
...
ipcMain.handle('zeb:version', async () => ({
  appVersion: resolveZebVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  zebRoot: resolveUpdateRoot()
}))
...
// the actual removal via `zeb uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (zeb_cli/uninstall.py + zeb_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ZEB_ROOT
  ...
  const fallback = () => ({
    zeb_home: ZEB_HOME,
    agent_installed: isZebSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'zeb_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ZEB_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Zeb agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import zeb_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ZEB_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `zeb-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `zeb-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('zeb:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('zeb:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('zeb:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('zeb:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// zeb:// deep links (e.g. zeb://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ZEB_PROTOCOL = 'zeb'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ZEB_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // zeb://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('zeb:deep-link', payload)
...
ipcMain.handle('zeb:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${ZEB_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ZEB_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ZEB_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `zeb://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching zeb:// URL is in our own argv.
