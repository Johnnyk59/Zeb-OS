...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zeb-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zeb-runner-test',
    sourceRepoRoot: null,
    zebHome: '/tmp/zeb-runner-test',
    logRoot: '/tmp/zeb-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zeb-agent', 'scripts')
