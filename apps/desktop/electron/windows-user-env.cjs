...
// even though a fresh shell — and the Zeb CLI — sees it immediately. The
// desktop's ZEB_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\zeb. Reading
