 ...
 * the three user-facing uninstall modes to the `zeb uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m zeb_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zeb_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zeb_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Zeb.app/Contents/MacOS/Zeb  → …/Zeb.app
 *   Windows: …\Zeb\Zeb.exe                 → …\Zeb  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Zeb.app/Contents/MacOS/Zeb → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Zeb.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Zeb.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Zeb$/i.test(dir) || /[\\/]zeb-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/zeb
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zeb_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, zebHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import zeb_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Zeb install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZEB_HOME=${String(zebHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
