 ...
 * Background
 * ----------
 * After `zeb update` + `zeb desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ZEB_DESKTOP_* is preserved
// plus ZEB_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ZEB_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ZEB_DESKTOP_']
