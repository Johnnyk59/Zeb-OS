...
// Backend subcommand routing for the desktop-managed Zeb process.
//
// The desktop app launches its own headless backend via `zeb serve` — it
...
// app hasn't updated yet, or an older `zeb` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Zeb profile to pin via `--profile`.
 */
 ...
 * `-m zeb_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zeb_cli/subcommands/dashboard.py` source registers
