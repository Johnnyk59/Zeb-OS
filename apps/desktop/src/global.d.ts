...
declare global {
  interface Window {
    zebDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ZebConnection>
      ...
      profile: {
        ...
        // backend under the new ZEB_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZebApiRequest) => Promise<T>
      notify: (payload: ZebNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ZebReadFileTextResult>
      selectPaths: (options?: ZebSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZebPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZebPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ZebTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZebReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZebGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZebGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZebRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZebReviewScope, baseRef?: null | string) => Promise<ZebReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZebReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZebReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZebTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZebTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZebWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ZebPreviewFileChanged) => void) => () => void
...
export interface ZebTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZebTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zeb update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zebRoot?: string
...
export interface ZebConnection {
  baseUrl: string
...
export interface ZebTitleBarTheme {
  background: string
  foreground: string
}

export interface ZebWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ZebApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ZebNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZebPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZebReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZebPreviewWatch {
  id: string
  path: string
}
...
export interface ZebGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZebRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZebRepoStatus {
  branch: null | string
...
// turn began.
export type ZebReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZebReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZebReviewList {
  files: ZebReviewFile[]
...
export interface ZebReviewShipInfo {
  ghReady: boolean
  pr: ZebReviewPr | null
}

export interface ZebReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZebReadDirResult {
  entries: ZebReadDirEntry[]
  error?: string
}

export interface ZebPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZebSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
