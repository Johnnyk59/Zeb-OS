...
/** Why a known Zeb command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Zeb profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Zeb Agent version', surface: exec() },
...
function isKnownZebSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zeb' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZebSlashCommand(normalized)
}
