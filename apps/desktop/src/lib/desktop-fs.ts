import type {
  ZebConnection,
  ZebReadDirResult,
  ZebReadFileTextResult,
  ZebSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ZebConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<ZebReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZebReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZebReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZebReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ZebSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
