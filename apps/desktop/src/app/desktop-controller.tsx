...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../zeb'
...
import { useZebConfig } from './session/hooks/use-zeb-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.zebDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.zebDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.zebDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.zebDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // zeb:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.zebDesktop?.onDeepLink?.(payload => {
    ...
    void window.zebDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.zebDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshZebConfig, sttEnabled, voiceMaxRecordingSeconds } = useZebConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshZebConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshZebConfig])
