import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@zeb/shared'
import { useEffect, useRef } from 'react'

import type { ZebConnection } from '@/global'
import { ZebGateway } from '@/zeb'
...
import type { RpcEvent } from '@/types/zeb'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zebDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZebGateway | null) => void
  refreshZebConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.zebDesktop

    const publish = (next: ZebConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zeb...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Zeb…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Zeb gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshZebConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshZebConfig()
