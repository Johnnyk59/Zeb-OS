...
// bug that drives the REAL useGatewayBoot hook + REAL ZebGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshZebConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { zebDesktop?: unknown }).zebDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { zebDesktop?: unknown }).zebDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForZeb) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startZeb()'s remote branch awaits waitForZeb() for 45s before it
    ...
    ;(window as { zebDesktop?: unknown }).zebDesktop = desktop
    ...
    // After ~45s waitForZeb gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Zeb backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
