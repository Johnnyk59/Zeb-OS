...
const branchActionLabel = (branch: ZebGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `zeb/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<ZebGitBranch[]>([])
  ...
  const convert = async (branch: ZebGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
