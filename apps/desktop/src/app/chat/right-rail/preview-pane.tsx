        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.zebDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Zeb to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.zebDesktop?.watchPreviewFile ||
      !window.zebDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.zebDesktop.onPreviewFileChanged(payload => {
    ...
    void window.zebDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.zebDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.zebDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:zeb-preview')
