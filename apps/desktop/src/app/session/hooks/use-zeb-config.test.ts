...
import { getZebConfig } from '@/zeb'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useZebConfig } from './use-zeb-config'

vi.mock('@/zeb', () => ({
  getZebConfig: vi.fn(),
  getZebConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zeb.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZebConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZebConfig>>)

describe('useZebConfig refreshZebConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshZebConfig()
    })
