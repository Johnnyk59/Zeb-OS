...
import { getZebConfig, getZebConfigDefaults } from '@/zeb'
...
interface ZebConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useZebConfig({ activeSessionIdRef, refreshProjectBranch }: ZebConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshZebConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getZebConfig(), getZebConfigDefaults().catch(() => ({}))])
  ...
  return { refreshZebConfig, sttEnabled, voiceMaxRecordingSeconds }
}
