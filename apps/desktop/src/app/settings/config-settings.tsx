...
import { getElevenLabsVoices, getZebConfigSchema, saveZebConfig } from '@/zeb'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ZebConfigRecord } from '@/types/zeb'

import { setZebConfigCache, useZebConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ZebConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ZebConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZebConfigRecord()
  ...
  const {
    ...
    queryKey: ['zeb-config-schema'],
    queryFn: getZebConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: ZebConfigRecord) => {
