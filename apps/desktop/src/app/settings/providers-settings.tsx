...
import { disconnectOAuthProvider, listOAuthProviders } from '@/zeb'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/zeb'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.zebDesktop?.terminal)
...
//      in zeb_cli/provider_catalog.py) — the SAME provider identity
//      `zeb model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Zeb can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Zeb can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Zeb never deletes creds another tool owns behind a silent API call.
