...
const getZebConfigRecord = vi.fn()
const saveZebConfig = vi.fn()
...
vi.mock('@/zeb', () => ({
  ...
  getZebConfigRecord: () => getZebConfigRecord(),
  saveZebConfig: (config: unknown) => saveZebConfig(config)
}))
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'zeb-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['zeb-4', 'zeb-4-mini'],
        authenticated: true,
        capabilities: { 'zeb-4': { reasoning: true, fast: true } }
      },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'zeb-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'zeb-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'deepseek', model: 'deepseek-chat', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getZebConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveZebConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getZebConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveZebConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['zeb-4'],
          authenticated: true,
          capabilities: { 'zeb-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'zeb-4' }]
    })
