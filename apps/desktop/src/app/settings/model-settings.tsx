...
import { invalidateZebConfig, setZebConfigCache, useZebConfigRecord } from '../hooks/use-config-record'
...
// Zeb' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Zeb default (medium), shown as Medium.
...
// backend now surfaces the full `zeb model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in zeb_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useZebConfigRecord()
  const setConfig = setZebConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `zeb model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Zeb runs the flow for you.`}
          </p>
        )}
