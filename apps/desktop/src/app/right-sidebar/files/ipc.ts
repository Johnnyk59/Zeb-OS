...
import type { ZebReadDirEntry, ZebReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ZebReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ZebReadDirEntry) {
...
async function filterIgnored(entries: ZebReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZebReadDirResult> {
  if (!window.zebDesktop) {
