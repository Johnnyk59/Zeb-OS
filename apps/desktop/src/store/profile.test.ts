...
const { getProfiles } = await import('@/zeb')
...
const remoteConn = (over: Partial<ZebConnection> = {}): ZebConnection =>
  ({ baseUrl: 'https://zeb-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZebConnection

const localConn = (over: Partial<ZebConnection> = {}): ZebConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZebConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZebConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { zebDesktop: { getConnection } })
