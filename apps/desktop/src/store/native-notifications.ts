...
const STORAGE_KEY = 'zeb:native-notifications'
...
// "Backgrounded" = the user isn't on Zeb. `document.hidden` only flips when
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.zebDesktop
