...
import type { ZebGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/zeb'
...
const PROJECT_SCOPE_KEY = 'zeb.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Zeb gateway is not connected')
  }
...
// repo, returning where Zeb should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZebGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zebDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zebDesktop?.writeClipboard?.(path)
  }
}
