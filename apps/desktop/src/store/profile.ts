...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/zeb'
...
import type { ProfileInfo } from '@/types/zeb'
...
// /api/profiles/active `current`). "default" is the root ~/.zeb. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'zeb.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'zeb.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.zebDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZEB_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zebDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.zebDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zeb.desktop.showAllProfiles'
...
// Switch to the default (root ~/.zeb) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zebDesktop?.touchBackend?.(target).catch(() => undefined)
}
