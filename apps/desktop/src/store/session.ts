...
import type { ZebConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/zeb'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'zeb.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'zeb.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zeb.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'zeb.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zeb.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'zeb.desktop.lastSessionId'
...
function workspaceCwdKey(connection: ZebConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.zebDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.zebDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ZebConnection | null>(null)
...
export const setConnection = (next: Updater<ZebConnection | null>) => updateAtom($connection, next)
