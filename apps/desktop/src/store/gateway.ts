import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@zeb/shared'
...
const isOpen = (gateway: ZebGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ZebGateway | null>(null)
...
let primaryGateway: ZebGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ZebGateway | null, profile = 'default'): void {
...
export function activeGateway(): ZebGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ZebGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
