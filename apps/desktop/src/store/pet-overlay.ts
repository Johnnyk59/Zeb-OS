 ...
 * app's bounds and stays visible while Zeb is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zeb.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zeb.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zebDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zebDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zebDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zebDesktop?.petOverlay?.close()
}
