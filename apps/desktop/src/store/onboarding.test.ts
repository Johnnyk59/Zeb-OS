...
import type { OAuthProvider } from '@/types/zeb'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('zeb-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('zeb-desktop-onboarded-v1')).toBe('1')
  })
