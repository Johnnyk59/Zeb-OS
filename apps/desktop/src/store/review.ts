...
import type { ZebReviewFile, ZebReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Zeb' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zeb.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zeb.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zeb.desktop.reviewTreeMode'
const SELECTED_KEY = 'zeb.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZebReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZebReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zebDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZebReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zebDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zebDesktop?.openExternal?.(url)
    }
