...
describe('Zeb REST session helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'zebDesktop')
  })
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getZebConfig, '/api/config'],
      [getZebConfigDefaults, '/api/config/defaults'],
