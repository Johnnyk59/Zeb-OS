...
import { formatRefValue, zebDirectiveFormatter } from './directive-text'
...
describe('zebDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zebDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zebDirectiveFormatter.parse('@file:src/main.tsx the entry point')
