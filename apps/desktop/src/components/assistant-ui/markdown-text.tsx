...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.zebDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.zebDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.zebDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.zebDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.zebDesktop && text && normalizeExternalUrl(text) === target) {
