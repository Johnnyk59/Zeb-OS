...
const ZEB_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ZebRefType = (typeof ZEB_REF_TYPES)[number]
...
const ICON_PATHS: Record<ZebRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ZebRefType] ?? ICON_FALLBACK
...
const ZEB_DIRECTIVE_RE = new RegExp(
...
export const zebDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZEB_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ZebRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ZebRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...zebDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Zeb directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zebDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zebDesktop?.readFileDataUrl(id)
