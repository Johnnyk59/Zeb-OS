...
const SKIN_KEY = 'zeb-desktop-theme-v2'
const MODE_KEY = 'zeb-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'zeb-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'zeb-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'zeb-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.zebTheme = skinName
  root.dataset.zebMode = rendered
  ...
  window.zebDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('zeb-boot-background', chromeBg)
    window.localStorage.setItem('zeb-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.zebThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.zebDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Zeb backend theme on connect. */
