...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zeb doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zeb_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `zeb model` uses — instead of
...
export type ZebConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `zeb model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors zeb_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Zeb, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `zeb tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Zeb); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/zeb/update/check` — the backend's own update state.
