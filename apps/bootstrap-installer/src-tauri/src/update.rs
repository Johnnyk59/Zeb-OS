...
//!
//! Driven when the installer is launched as `Zeb-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Zeb desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `zeb update`
//!      or repair bootstrap can race locked files),
//!   2. run `zeb update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in zeb_cli/main.py),
//!   3. run `zeb desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `zeb update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_zeb) and
...
/// `zeb update` exit code meaning "another zeb process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// zeb_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `zeb update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let zeb_home = crate::paths::zeb_home();
    let install_root = zeb_home.join("zeb-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_zeb`
    ...
    let zeb = resolve_zeb(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the zeb CLI under {}. Is Zeb installed? \
    ...
    // async on Windows. If it still holds the venv shim, `zeb update`
    ...
    // ---- stage 2: zeb update -----------------------------------------
    // Pass --branch so `zeb update` targets the branch this installer was
    ...
    // without --branch, `zeb update` switches the checkout to `main` (a
    ...
    // --force skips `zeb update`'s Windows running-exe guard (which would
    ...
    // time `zeb update` runs there is no legitimate zeb.exe to protect,
    // and the guard would only produce a false "Zeb is still running" stop.
    //
    ...
    // probe only checks the zeb.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Zeb windows" message.
    ...
    // Retry-once for the update-boundary crash. `zeb update` lazily imports
    ...
    // second `zeb update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Zeb" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Zeb is still running. Close all Zeb windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "zeb update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::zeb_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: zeb desktop --build-only ----------------------------
    // `zeb update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `zeb update` already does above, and `zeb update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `zeb desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Zeb manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_zeb_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Zeb to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend zeb.exe (or the desktop Zeb.exe
            ...
            // force-kill every Zeb.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Zeb still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_zeb();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_zeb(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Zeb.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Zeb.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `zeb.exe` other than this process. Windows-only; a no-op
...
/// Spawn `zeb <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv zeb shim under an install root, regardless of existence.
fn venv_zeb(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("zeb.exe")
    } else {
        install_root.join("venv").join("bin").join("zeb")
    }
}

/// Resolve the zeb CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `zeb` on PATH.
fn resolve_zeb(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_zeb(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "zeb.exe" } else { "zeb" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "ZEB_HOME".to_string(),
        zeb_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        zeb_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_zeb_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Zeb.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.zeb-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.zeb-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::zeb_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_zeb_is_under_install_root() {
        let root = Path::new("/x/zeb-agent");
        let shim = venv_zeb(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_zeb(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/zeb-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".zeb-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Zeb.app"]),
            Some(PathBuf::from("/Applications/Zeb.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Zeb.app");
        let tmp = base.join("Zeb.app.zeb-update-new");
        let old = base.join("Zeb.app.zeb-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Zeb.app");
        let tmp = base.join("Zeb.app.zeb-update-new"); // intentionally absent
        let old = base.join("Zeb.app.zeb-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Zeb.app");
        let tmp = base.join("Zeb.app.zeb-update-new"); // absent
        let old = base.join("Zeb.app.zeb-update-old");
        write_marker(&target, "OLD");
