...
//!
//! Mirrors `zeb_constants.get_zeb_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\zeb
//!   macOS:   ~/.zeb
//!   Linux:   ~/.zeb  (override via $ZEB_HOME)
//!
//! NOTE (macOS): Python's get_zeb_home(), scripts/install.sh, and the
//! Electron desktop's resolveZebHome() ALL use ~/.zeb on macOS — there
...
/// Returns the canonical Zeb home directory, respecting $ZEB_HOME if set.
pub fn zeb_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ZEB_HOME") {
    ...
    {
        // %LOCALAPPDATA%\zeb — matches scripts/install.ps1's $ZebHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("zeb");
        }
    }

    // macOS + Linux + fallback: ~/.zeb (matches Python get_zeb_home(),
    // install.sh, and the Electron desktop's resolveZebHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".zeb");
    }
...
/// ZEB_HOME so it survives repo checkout deletion (unlike anything under
/// zeb-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\zeb\zeb-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_zeb`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ZEB_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ZEB_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    zeb_home().join(".zeb-update-in-progress")
}
...
pub fn copy_self_to_zeb_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ZEB_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ZEB_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".zeb-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ZEB_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[zeb-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ZEB_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_zeb_home() -> String {
    zeb_home().to_string_lossy().into_owned()
}
