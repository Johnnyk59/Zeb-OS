...
export const $zebHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.zeb/logs/bootstrap-installer.log')
    $zebHome.set('~/.zeb')
  ...
  try {
    const [logPath, zebHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zeb_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zeb" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Zeb-Setup.exe --update);
...
export async function launchZebDesktop(): Promise<void> {
  ...
  await invoke('launch_zeb_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Zeb repository' },
