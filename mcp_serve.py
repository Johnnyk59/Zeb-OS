"""
Zeb MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zeb-specific extra)

Usage:
    zeb mcp serve
    zeb mcp serve --verbose
...
logger = logging.getLogger("zeb.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZEB_HOME."""
    try:
        from zeb_constants import get_zeb_home
        return get_zeb_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZEB_HOME", Path.home() / ".zeb")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zeb_constants import get_zeb_home
        directory_file = get_zeb_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZEB_HOME", Path.home() / ".zeb")
...
class EventBridge:
    ...
    This is the Zeb equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zeb_constants import get_zeb_home
            db_file = get_zeb_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZEB_HOME", Path.home() / ".zeb")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zeb MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zeb",
        instructions=(
            "Zeb Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zeb MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
