...
# Do not write .pyc files at runtime: /opt/zeb is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/zeb/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when zeb
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# zeb process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
RUN set -eu; \
    ...
    # 'Zeb WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via ZEB_UID at runtime
RUN useradd -u 10000 -m -d /opt/data zeb
...
#
# ui-tui/packages/zeb-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/zeb-ink/ ui-tui/packages/zeb-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as zeb-ink above).
...
# lock on the @zeb/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/zeb/.venv at first
...
# gives the non-root zeb user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link zeb-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/zeb are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the zeb user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/zeb/bin && \
    cp /opt/zeb/docker/zeb-exec-shim.sh /opt/zeb/bin/zeb && \
    chmod 0755 /opt/zeb/bin/zeb && \
    printf 'docker\n' > /opt/zeb/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $ZEB_HOME. $ZEB_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``zeb update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the zeb user via
# `s6-setuidgid zeb` in its run script. If ZEB_UID is unset, services
# run as the default zeb user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `zeb dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the ZEB_GIT_SHA build-arg to
# /opt/zeb/.zeb_build_sha at build time, and have
# zeb_cli/build_info.py read it at runtime.  Both `zeb dump` and
...
ARG ZEB_GIT_SHA=
RUN if [ -n "${ZEB_GIT_SHA}" ]; then \
        printf '%s\n' "${ZEB_GIT_SHA}" > /opt/zeb/.zeb_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-zeb + dashboard.
...
# `exec zeb`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $ZEB_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/zeb/docker/stage2-hook.sh\n' \
...
# ---------- Runtime ----------
ENV ZEB_WEB_DIST=/opt/zeb/zeb_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/zeb/ui-tui/dist/entry.js`) and skip the
...
ENV ZEB_TUI_DIR=/opt/zeb/ui-tui
...
# is seeded + chowned to the zeb user by docker/stage2-hook.sh and lives
...
ENV ZEB_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> zeb ...` they default to root, and any file the
# command writes under $ZEB_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/zeb/bin/zeb, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid zeb`
# when invoked as root. Non-root callers (supervised processes,
# `--user zeb`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/zeb/.venv/bin/zeb). See the shim source for
# the opt-out env var (ZEB_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> zeb ...` and any
# subprocess that doesn't activate the venv first still find zeb.
#
# /opt/zeb/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/zeb/bin:/opt/zeb/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. zeb
# subcommand vs. no-args), drops to the zeb user via s6-setuidgid,
